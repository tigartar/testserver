   ...
   protected void freeChunk(int x, int y) throws IOException {
      if (this.chunks[x][y] != null) {
         if (this.chunks[x][y].isDirty()) {
            ...
            ((Buffer)this.byteBuffer).position(ib.position() * 4);
   ...
   public static WorldMap getWorldMap(File file) throws IOException {
      if (file.exists()) {
         ...
         if (file.length() < 12L) {
            ...
            } else if (file.length() != (long)(width * height * 16384 + 12)) {
               throw new IOException(
                  "Found the map file, but it was the wrong size. (found " + file.length() + ", expected " + (width * height * 16384 + 12) + ")"
               );
            } else {
