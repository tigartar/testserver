...
public abstract class GuardPlan implements CreatureTemplateIds, TimeConstants, MiscConstants, MonetaryConstants {
   ...
   public final long getTimeLeft() {
      try {
         ...
         logger.log(Level.WARNING, this.villageId + ", " + var2.getMessage(), (Throwable)var2);
      }
   ...
   public double calculateUpkeep(boolean calculateFraction) {
      ...
      return (double)monthlyCost * 2.0667989417989417E-4;
   }
   ...
   public static final long getCostForGuards(int numGuards) {
      return Servers.localServer.isChallengeOrEpicServer()
         ? (long)(numGuards * 10000 + (numGuards - 1) * numGuards / 2 * 100 * 50)
         : (long)numGuards * Villages.GUARD_UPKEEP;
   }
   ...
   public final long getMonthlyCost() {
      if (!Servers.localServer.isUpkeep()) {
         ...
         try {
            ...
            logger.log(Level.WARNING, "Guardplan for village " + this.villageId + ": Village not found. Deleting.", (Throwable)var4);
            this.delete();
            return 10000L;
         }
      }
   ...
   private void pollGuards() {
      if (this.type != 0) {
         try {
            ...
            logger.log(Level.WARNING, "No village for guardplan with villageid " + this.villageId, (Throwable)var15);
         }
      } else {
         try {
            Village village = this.getVillage();
            ...
            logger.log(Level.WARNING, "No village for guardplan with villageid " + this.villageId, (Throwable)var13);
         }
      }
   }
   ...
   private void putGuardInWorld(Creature guard) {
      try {
         ...
         try {
            ...
            logger.log(Level.WARNING, "Guard: " + guard.getWurmId() + ": " + var4.getMessage(), (Throwable)var4);
         } catch (NoSuchCreatureException var5) {
            logger.log(Level.WARNING, "Guard: " + guard.getWurmId() + ": " + var5.getMessage(), (Throwable)var5);
            this.getVillage().deleteGuard(guard, false);
         } catch (NoSuchPlayerException var6) {
            logger.log(Level.WARNING, "Guard: " + guard.getWurmId() + ": " + var6.getMessage(), (Throwable)var6);
         } catch (Exception var7) {
            logger.log(Level.WARNING, "Failed to return village guard: " + var7.getMessage(), (Throwable)var7);
         }
         ...
         if (this.moneyLeft - upkeep <= 0L) {
            ...
            if (tl < 3600000L) {
               try {
                  this.getVillage()
                     .broadCastAlert(
                        "The village is disbanding within the hour. You may add upkeep money to the village coffers at the token immediately.", (byte)2
                     );
                  this.getVillage().broadCastAlert("Any traders who are citizens of " + this.getVillage().getName() + " will disband without refund.");
               } catch (NoSuchVillageException var9) {
                  logger.log(Level.WARNING, "No Village? " + this.villageId, (Throwable)var9);
               }
               ...
               if (System.currentTimeMillis() - this.lastSentWarning > 3600000L) {
                  ...
                  try {
                     this.getVillage()
                        .broadCastAlert("The village is disbanding within 24 hours. You may add upkeep money to the village coffers at the token.", (byte)2);
                     this.getVillage().broadCastAlert("Any traders who are citizens of " + this.getVillage().getName() + " will disband without refund.");
                  } catch (NoSuchVillageException var8) {
                     logger.log(Level.WARNING, "No Village? " + this.villageId, (Throwable)var8);
                  }
               }
               ...
               try {
                  ...
                  logger.log(Level.WARNING, "No Village? " + this.villageId, (Throwable)var7);
               }
            }
   ...
   public final void changePlan(int newPlan, int newNumberOfGuards) {
      ...
      if (changeInGuards < 0) {
         try {
            ...
            logger.log(Level.WARNING, "Village lacking for plan " + this.villageId, (Throwable)var8);
         }
      }
   }
   ...
   public final void fixGuards() {
      try {
         ...
         logger.log(Level.WARNING, "Village lacking for plan " + this.villageId, (Throwable)var3);
      }
   }
