...
public final class DbVillageRole extends VillageRole implements VillageStatus, Comparable<DbVillageRole> {
   ...
   public DbVillageRole(
      ...
      boolean aMayInvite,
      boolean aMayDestroy,
      boolean aMayManageRoles,
      boolean aMayExpand,
      boolean aMayLockFences,
      boolean aMayPassFences,
      boolean aIsDiplomat,
      boolean aMayAttackCitizens,
      boolean aMayAttackNonCitizens,
      boolean aMayFish,
      boolean aMayCutOldTrees,
      byte aStatus,
      int appliedToVillage,
      boolean aMayPushPullTurn,
      boolean aMayUpdateMap,
      boolean aMayLead,
      boolean aMayPickup,
      boolean aMayTame,
      boolean aMayLoad,
      boolean aMayButcher,
      boolean aMayAttachLock,
      boolean aMayPickLocks,
      long appliedToPlayer,
      int aSettings,
      int aMoreSettings,
      int aExtraSettings
   ) throws IOException {
   ...
   DbVillageRole(
      ...
      int aVillageId,
      String aRoleName,
      boolean aMayTerraform,
      boolean aMayCuttrees,
      boolean aMayMine,
      boolean aMayFarm,
      boolean aMayBuild,
      boolean aMayHire,
   ...
   void create() throws IOException {
      ...
      try {
         ...
         ps = dbcon.prepareStatement(
            "INSERT INTO VILLAGEROLE (VILLAGEID,NAME ,MAYTERRAFORM ,MAYCUTTREE ,MAYMINE ,MAYFARM ,MAYBUILD ,MAYHIRE,MAYINVITE,MAYDESTROY,MAYMANAGEROLES, MAYEXPAND,MAYLOCKFENCES, MAYPASSFENCES,DIPLOMAT, MAYATTACKCITIZ, MAYATTACKNONCITIZ,MAYFISH,MAYCUTOLD, STATUS,VILLAGEAPPLIEDTO,MAYPUSHPULLTURN,MAYUPDATEMAP,MAYLEAD,MAYPICKUP,MAYTAME,MAYLOAD,MAYBUTCHER,MAYATTACHLOCK,MAYPICKLOCKS,PLAYERAPPLIEDTO,SETTINGS,MORESETTINGS,EXTRASETTINGS) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
            1
         );
   ...
   public void save() throws IOException {
      ...
      try {
         dbcon = DbConnector.getZonesDbCon();
         ps = dbcon.prepareStatement(
            "UPDATE VILLAGEROLE SET VILLAGEID=?,NAME=?,MAYTERRAFORM=?,MAYCUTTREE=?,MAYMINE=?,MAYFARM=?,MAYBUILD=?,MAYHIRE=?,MAYINVITE=?,MAYDESTROY=?,MAYMANAGEROLES=?,MAYEXPAND=?,MAYLOCKFENCES=?,MAYPASSFENCES=?,DIPLOMAT=?,MAYATTACKCITIZ=?,MAYATTACKNONCITIZ=?,MAYFISH=?,MAYCUTOLD=?,STATUS=?,VILLAGEAPPLIEDTO=?,MAYPUSHPULLTURN=?,MAYUPDATEMAP=?,MAYLEAD=?,MAYPICKUP=?,MAYTAME=?,MAYLOAD=?,MAYBUTCHER=?,MAYATTACHLOCK=?,MAYPICKLOCKS=?,PLAYERAPPLIEDTO=?,SETTINGS=?,MORESETTINGS=?,EXTRASETTINGS=? WHERE ID=?"
         );
         ps.setInt(1, this.villageid);
         ps.setString(2, this.name);
         ps.setBoolean(3, this.mayTerraform);
         ...
         logger.log(Level.WARNING, "Failed to save role " + this.name + ": " + var8.getMessage(), (Throwable)var8);
         throw new IOException(var8);
      } finally {
         DbUtilities.closeDatabaseObjects(ps, rs);
         DbConnector.returnConnection(dbcon);
   ...
   public void setMayHire(boolean hire) throws IOException {
      if (this.mayHire != hire) {
         ...
         try {
            ...
            logger.log(Level.WARNING, "Failed to set data for role with id " + this.id, (Throwable)var8);
            throw new IOException(var8);
         } finally {
   ...
   public void delete() throws IOException {
      ...
      try {
         ...
         logger.log(Level.WARNING, "Failed to delete role with id " + this.id, (Throwable)var7);
