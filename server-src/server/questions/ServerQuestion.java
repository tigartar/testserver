...
public final class ServerQuestion extends Question implements CounterTypes {
   private final List<ServerEntry> serverEntries = new LinkedList<>();
   private final List<ServerEntry> transferEntries = new LinkedList<>();
   ...
   public void sendQuestion() {
      ...
      if (this.getResponder().getPower() > 0) {
         ...
         ServerEntry[] entries;
         if (!Servers.localServer.entryServer && !Servers.isThisATestServer()) {
            entries = Servers.getAllNeighbours();
         } else {
      ...
      if (this.getResponder().getPower() >= 5) {
         ...
         buf.append(
            "label{type=\"bolditalic\";text=\"Type\"}label{type=\"bolditalic\";text=\"Name\"}label{type=\"bolditalic\";text=\"PvP\"}label{type=\"bolditalic\";text=\"Epic\"}label{type=\"bolditalic\";text=\"Home\"}label{type=\"bolditalic\";text=\"Chaos\"}"
         );
