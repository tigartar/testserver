...
extends Question {
    ...
    private final List<Creature> memberlist = new LinkedList<Creature>();
    ...
    public TeamManagementQuestion(Creature aResponder, String aTitle, String aQuestion, boolean _mayInvite, long aTarget, boolean manage, boolean removeSelf) throws NoSuchCreatureException, NoSuchPlayerException {
        ...
        this.founding = this.getResponder().getTeam() == null;
    ...
    public void answer(Properties answers) {
        ...
        if (this.sendToResponder) {
                ...
                if (did != null) {
                    try {
                        ...
                        if (kick != null && kick.equals("true")) {
                            ...
                            if ((invite = this.getAnswer().getProperty("invite")) != null && invite.equals("true") && !c.isTeamLeader()) {
            ...
            boolean join;
            boolean bl = join = this.getAnswer().getProperty("join") != null && this.getAnswer().getProperty("join").equals("true");
            if (this.invited == null || !this.invited.hasLink()) {
                asker.getCommunicator().sendNormalServerMessage("The team invitation is lost - the creature can no longer be found.");
            } else if (!join) {
        ...
        if (this.sendToResponder) {
            if (this.managing) {
                ...
                for (int x = 0; x < members.length; ++x) {
                    ...
                    if (!members[x].mayInviteTeam()) continue;
                    buf.append(" (invite)");
                }
                buf.append("\"}};");
            ...
            Team team = this.getResponder().getTeam();
