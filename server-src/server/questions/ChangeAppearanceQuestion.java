...
public final class ChangeAppearanceQuestion extends Question {
   ...
   private void handleGenderChange() {
      if (this.getResponder().getSex() != this.gender) {
         ...
         try {
            ...
            logger.warning("Error setting face for player " + player.getName() + ": " + var3.getMessage());
            return;
         }
         ...
         this.getResponder()
            ...
            .sendSafeServerMessage(
               "You feel a strange sensation as Vynora's power alters your body. You are now " + (this.gender == 1 ? "female" : "male") + ".", (byte)2
            );
      } else {
   ...
   private void sendConfirmation() {
      if (this.mirror.getAuxData() != 1) {
         ...
         buf.append(
            "harray{text{text=''}}text{type='bold';text='Are you sure? This mirror will not allow you to make this choice again.'}harray{text{text=''}}"
         );
   ...
   public void sendQuestion() {
      if (this.mirror.getAuxData() != 1) {
         ...
         buf.append(this.getBmlHeader());
         buf.append(
            "harray{text{text=''}}text{text='Before you may change your appearance, you must choose to select a new gender or keep your current one.'}harray{text{text=''}}text{type='bold';text='What will your gender be?'}"
         );
   ...
   private final String maleOption() {
      return this.getResponder().getSex() == 0
         ? "harray{text{text=''}radio{ group='gender'; id='male';text='Male (current)';selected='true'}}"
         : "harray{text{text=''}radio{ group='gender'; id='male';text='Male'}}";
   }

   private final String femaleOption() {
      return this.getResponder().getSex() == 1
         ? "harray{text{text=''}radio{ group='gender'; id='female';text='Female (current)';selected='true'}}"
         : "harray{text{text=''}radio{ group='gender'; id='female';text='Female'}}";
   }
}
