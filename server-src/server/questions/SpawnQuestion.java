...
public final class SpawnQuestion extends Question {
   ...
   public void sendQuestion() {
      ...
      if (Servers.localServer.EPIC && Servers.localServer.getKingdom() != this.getResponder().getKingdomId()) {
         ...
         for(ServerEntry s : Servers.getAllServers()) {
            if (s.EPIC
               && s.isAvailable(0, this.getResponder().isPaying())
               && s.getId() != Servers.localServer.id
               && (s.getKingdom() == 0 || s.getKingdom() == this.getResponder().getKingdomId())
               && (this.getResponder().isPaying() || !s.ISPAYMENT)) {
               ++x;
               buf.append(",");
      ...
      this.getResponder()
         .getCommunicator()
         .sendBml(300, Servers.localServer.isChallengeServer() ? 500 : 300, true, true, buf.toString(), 200, 200, 200, this.title);
   }
   ...
   Spawnpoint getSpawnpoint(int aIndex) {
      return this.spawnpoints.isEmpty() ? null : this.spawnpoints.get(aIndex);
   }
}
