...
extends Question {
            ...
            if (nums != null && nums.length() > 0) {
                ...
                try {
                    ...
                    try {
                        ...
                        float maxDist = 4.0f;
                        if (topParent.isVehicle() && (vehicle = Vehicles.getVehicle(topParent)) != null) {
                        ...
                        int bnums = 0;
                        bnums = bulkitem.getRealTemplate() != null && bulkitem.getRealTemplate().isCombine() ? (int)Math.ceil(bulkitem.getBulkNumsFloat(false)) : bulkitem.getBulkNums();
                        Item toInsert = null;
                        ...
                        if (bnums >= i) {
                            ...
                            if (template != null) {
                                ...
                                if (max) {
                                    ...
                                    i = Math.min(i, bnums);
                                } else if (!(this.getResponder().canCarry(tweight * i) || targetInventory != null && targetInventory.isBulkContainer() || i <= 1 && bulkitem.getWeightGrams() < volume)) {
                                ...
                                for (int created = 0; created < i; ++created) {
                                    try {
                                        int weight;
                                        block61: {
                                            weight = bulkitem.getWeightGrams() - weightReduced;
                                            float percent = 1.0f;
                                            if (weight < volume) {
                                            ...
                                            if (this.moveTarget == 0L) {
                                                ...
                                                if (targetInventory.isBulkContainer()) {
                                                    try {
                                                        ...
                                                        message = "The %s will not fit in the %s.";
                                                    ...
                                                    catch (NoSuchPlayerException message2) {
                                                        break block61;
                                                    }
                                                    catch (NoSuchCreatureException message2) {
                                                        break block61;
                                                    }
                                                }
                                                if (targetInventory.testInsertItem(toInsert) && targetInventory.mayCreatureInsertItem()) {
                                                    ...
                                                    message = "There is not enough space for any more items.";
    ...
    public void sendQuestion() {
        Item temp;
        try {
            temp = this.moveTarget > 0L ? Items.getItem(this.moveTarget) : null;
        }
