...
implements MonetaryConstants {
    ...
    public void addReferrer(String receiver) {
        try {
            ...
            logger.log(Level.WARNING, e.getMessage() + " " + receiver + " from " + this.getResponder().getName(), e);
                ...
                try {
                    if (PlayerInfoFactory.acceptReferer(wurmid, pinf.wurmId, money)) {
                        try {
                            ...
                            logger.log(Level.WARNING, "An error occurred wurmid: " + wurmid + ", awarderName: " + awarderName + ", money: " + money + " - " + ex.getMessage(), ex);
        ...
        if (((Player)this.getResponder()).getPaymentExpire() > 0L && !this.getResponder().hasFlag(63)) {
            ...
            if (this.referrals.size() > 0) {
                for (Map.Entry<String, Byte> entry : this.referrals.entrySet()) {
                    ...
                    if (referralType != 0 || (a = answers.getProperty(name + "group")) == null) continue;
                    ...
                    if (!a.equals(name + "time")) continue;
                    if (Servers.isRealLoginServer()) {
    ...
    public void sendQuestion() {
        ...
        this.referrals = Servers.isRealLoginServer() ? PlayerInfoFactory.getReferrers(this.target) : new LoginServerWebConnection().getReferrers(this.getResponder(), this.target);
