...
public class GmSetEnchants extends Question {
   ...
   public GmSetEnchants(Creature aResponder, Item aTarget) {
      ...
      Arrays.sort((Object[])this.spells);
   }
            ...
            for(int x = 0; x < this.spells.length; ++x) {
               ...
               if (newsel != oldsel || oldsel && newpow != oldpow) {
                  ...
                  if (newsel) {
                     ...
                     this.getResponder()
                        .getLogger()
                        .log(Level.INFO, " enchanting " + this.spells[x].getName() + " " + this.item.getName() + ", " + this.item.getWurmId() + ", " + newpow);
                  }
               }
            }
   ...
   public void sendQuestion() {
      ...
      if (this.getResponder().getPower() >= 4) {
         ...
         buf.append(
            "table{rows=\""
               + this.spells.length
               + "\";cols=\"4\";label{text=\"\"};text{type=\"bold\";text=\"Power\"};text{type=\"bold\";text=\"Name\"};text{type=\"bold\";text=\"Description\"};"
         );
         ...
         for(int x = 0; x < this.spells.length; ++x) {
            ...
            buf.append(
               "checkbox{id=\"newsel"
                  ...
                  + "\"};"
                  + (
                     this.spells[x].singleItemEnchant
                        ? "text{type=\"italic\";text=\"(none)\"};"
                        : "input{id=\"newpow" + x + "\";maxchars=\"" + maxChars + "\";text=\"" + pow + "\"};"
                  )
                  + "label{text=\""
                  + this.spells[x].getName()
                  + "\"};label{text=\""
                  + this.spells[x].getDescription()
                  + "\"};"
            );
         }
         ...
         buf.append(
            "text{text=\"Can add or change or remove enchants to specific powers, it maybe necessary to remove an enchant before modifying its power. If the enchant requires a power, then if none is specified it will default to 50, also \"}"
         );
