...
public class ConchQuestion extends Question {
   ...
   private static final boolean sendAchievementNeeded(Creature responder, StringBuilder buf, Item conch) {
      ...
      if (conch.getAuxData() == 10 || ach.getAchievement(1) == null) {
         return sendSixthBml(responder, buf, 1, conch);
      } else if (ach.getAchievement(52) != null) {
         conch.setData(14L);
         return false;
      } else {
         return conch.getAuxData() != 12 && ach.getAchievement(52) != null ? false : sendSeventhBml(responder, buf, 52, conch);
      }
   }
   ...
   private static final boolean sendItemNeeded(Creature responder, StringBuilder buf, Item conch) {
      if (responder.getKingdomTemplateId() == 2) {
         ...
         if (conch.getAuxData() == 8 || !responder.hasAbility(Abilities.getAbilityForItem(798, responder))) {
            return sendFifthBml(responder, buf, 798, conch);
         }
      } else {
         if (conch.getAuxData() == 0 || !responder.hasAbility(Abilities.getAbilityForItem(807, responder))) {
            return sendFirstBml(responder, buf, 807, conch);
         }
         ...
         if (conch.getAuxData() == 4 || !responder.hasAbility(Abilities.getAbilityForItem(810, responder))) {
            return sendThirdBml(responder, buf, 810, conch);
         }

         if (conch.getAuxData() == 6 || !responder.hasAbility(Abilities.getAbilityForItem(809, responder))) {
            return sendFourthBml(responder, buf, 809, conch);
         }
   ...
   public void sendQuestion() {
      ...
      if (this.getResponder().getPower() > 0) {
         ...
         addBml(this.getResponder(), this.conch, buf);
   ...
   public void sendQuestionAfter() {
      ...
      if (this.getResponder().getPower() > 0) {
         ...
         if (this.conch.getAuxData() == 1) {
            ...
            sendAfterSixthBml(this.getResponder(), buf, this.conch);
         } else if (this.conch.getAuxData() == 13) {
            sendAfterSeventhBml(this.getResponder(), buf, this.conch);
         } else if (this.conch.getAuxData() == 15) {
            addFinalBml(this.getResponder(), buf, this.conch);
         } else {
   ...
   private static final void addBml(Creature responder, Item conch, StringBuilder buf) {
      if (!sendItemNeeded(responder, buf, conch) && !sendAchievementNeeded(responder, buf, conch)) {
         Achievements ach = Achievements.getAchievementObject(responder.getWurmId());
   ...
   private static final boolean addFinalBml(Creature responder, StringBuilder buf, Item conch) {
      if (isThisAdventureServer()) {
         buf.append("text{text=\"The " + addSpiritVoiceType(responder) + " says: \"}text{text=\"\"}");
         ...
         buf.append(
            "text{text=\"We hope you don't feel cheated by us. We really need help and hope that your deity will find a way to return us to Valrei.\"}text{text=\"\"}"
         );
         buf.append("text{text=\"We will show you the way now.'\"}text{text=\"\"}");
         sendSignalToEntrance(responder, conch);
      }
   ...
   private static final boolean sendFirstBml(Creature responder, StringBuilder buf, int templateId, Item conch) {
      ...
      buf.append("text{text=\"A faint " + addSpiritVoiceType(responder) + " is heard over the muffled sounds of the conch shell: \"}text{text=\"\"}");
      ...
      if (responder.hasAbility(Abilities.getAbilityForItem(templateId, responder))) {
         buf.append(
            "text{text=\"You already are "
               + Abilities.getAbilityString(Abilities.getAbilityForItem(templateId, responder))
               + ". That will speed things up!'\"}text{text=\"\"}"
         );
         conch.setAuxData((byte)2);
      } else {
         buf.append(
            "text{text=\"The item will teach you how to become "
               + Abilities.getAbilityString(Abilities.getAbilityForItem(templateId, responder))
               + ".\"}text{text=\"\"}"
         );
   ...
   public final void sendAfterFirstBml(Creature responder, StringBuilder buf, Item conch) {
      ...
      if (item == null) {
         ...
         buf.append(
            "text{text=\"'Look in the sky. We have created a colored spirit light for you to follow', the "
               + addSpiritVoiceType(responder)
               + " whispers.\"}text{text=\"\"}"
         );
      }
   ...
   private static final boolean sendSecondBml(Creature responder, StringBuilder buf, int templateId, Item conch) {
      conch.setData1(templateId);
      conch.setAuxData((byte)2);
      buf.append("text{text=\"As before, a " + addSpiritVoiceType(responder) + " is heard from the shell: \"}text{text=\"\"}");
      ...
      if (responder.hasAbility(Abilities.getAbilityForItem(templateId, responder))) {
         buf.append(
            "text{text=\"As you already are "
               + Abilities.getAbilityString(Abilities.getAbilityForItem(templateId, responder))
               + " we will show you the next tome.'\"}text{text=\"\"}"
         );
         conch.setAuxData((byte)4);
      } else {
         buf.append(
            "text{text=\"For now, let us show you where you can become "
               + Abilities.getAbilityString(Abilities.getAbilityForItem(templateId, responder))
               + ".'\"}text{text=\"\"}"
         );
      }
   ...
   public final void sendAfterSecondBml(Creature responder, StringBuilder buf, Item conch) {
      ...
      if (item == null) {
         ...
         this.sendSignalToItemTemplate(item);
         buf.append(
            "text{text=\"'We have created a colored spirit light for you to follow', the " + addSpiritVoiceType(responder) + " whispers.\"}text{text=\"\"}"
         );
      }
   ...
   private static final boolean sendThirdBml(Creature responder, StringBuilder buf, int templateId, Item conch) {
      conch.setData1(templateId);
      conch.setAuxData((byte)4);
      buf.append("text{text=\"Again, the " + addSpiritVoiceType(responder) + " whispers from the shell: \"}text{text=\"\"}");
      ...
      buf.append(
         "text{text=\"The gods wanted different and handed down a Key to the Heavens, well aware of the impact this would have on the friendship.\"}text{text=\"\"}"
      );
      ...
      if (responder.hasAbility(Abilities.getAbilityForItem(templateId, responder))) {
         buf.append(
            "text{text=\"You already are "
               + Abilities.getAbilityString(Abilities.getAbilityForItem(templateId, responder))
               + ". That will make the journey quicker.'\"}text{text=\"\"}"
         );
         conch.setAuxData((byte)6);
      } else {
         buf.append(
            "text{text=\"We will tell you more when you are "
               + Abilities.getAbilityString(Abilities.getAbilityForItem(templateId, responder))
               + ".'\"}text{text=\"\"}"
         );
      }
   ...
   public final void sendAfterThirdBml(Creature responder, StringBuilder buf, Item conch) {
      ...
      if (item == null) {
         ...
         this.sendSignalToItemTemplate(item);
         buf.append("text{text=\"'Look to the skies. The spirit light should guide you, says the " + addSpiritVoiceType(responder) + ".\"}text{text=\"\"}");
      }
   ...
   private static final boolean sendFourthBml(Creature responder, StringBuilder buf, int templateId, Item conch) {
      conch.setData1(templateId);
      conch.setAuxData((byte)6);
      buf.append("text{text=\"From the shell, a " + addSpiritVoiceType(responder) + " continues the story: \"}text{text=\"\"}");
      buf.append(
         "text{text=\"'As Ceyer, Zampooklidin and Brightberry discussed who should use the Key and ascend to Valrei a stranger approached.\"}text{text=\"\"}"
      );
      buf.append(
         "text{text=\"It was Malinkaan, a powerful Arch Mage who had found out about the Key. Rumours have it that Vynora gave him divination.\"}text{text=\"\"}"
      );
      ...
      buf.append(
         "text{text=\"Thoughts started to grow in Zampooklidins mind that he now had the right to the key, no doubt without the influence of Libila.\"}text{text=\"\"}"
      );
      if (responder.hasAbility(Abilities.getAbilityForItem(templateId, responder))) {
         buf.append(
            "text{text=\"You are a "
               + Abilities.getAbilityString(Abilities.getAbilityForItem(templateId, responder))
               + " already indeed. We will show you the next place.'\"}text{text=\"\"}"
         );
         conch.setAuxData((byte)8);
      } else {
         buf.append(
            "text{text=\"Now you should become "
               + Abilities.getAbilityString(Abilities.getAbilityForItem(templateId, responder))
               + " as well.'\"}text{text=\"\"}"
         );
      }
   ...
   public final void sendAfterFourthBml(Creature responder, StringBuilder buf, Item conch) {
      ...
      if (item == null) {
         ...
         this.sendSignalToItemTemplate(item);
         buf.append("text{text=\"'Find the spirit light we have created', advices the " + addSpiritVoiceType(responder) + ".\"}text{text=\"\"}");
      }
   ...
   private static final boolean sendFifthBml(Creature responder, StringBuilder buf, int templateId, Item conch) {
      ...
      conch.setAuxData((byte)8);
      buf.append(
         "text{text=\"'Darkness ascended on the lands as Zampooklidin declared war on Ceyer, and soon Brightberrys people craved the Key as well for their loss.'\"}text{text=\"\"}"
      );
      buf.append("text{text=\"'Ceyer was too proud to disregard his cowardness', the " + addSpiritVoiceType(responder) + " declares.\"}text{text=\"\"}");
      buf.append("text{text=\"'He decided that someone else was worthy of the Key, but not even Zampooklidin.\"}text{text=\"\"}");
      buf.append(
         "text{text=\"As he hid the key, he put a magical enchantment on the container which now only the most worthy of persons can open.\"}text{text=\"\"}"
      );
      ...
      if (responder.hasAbility(Abilities.getAbilityForItem(templateId, responder))) {
         buf.append(
            "text{text=\"You are a "
               + Abilities.getAbilityString(Abilities.getAbilityForItem(templateId, responder))
               + " already indeed. We would ask two things more from you before we show where to find the container.'\"}text{text=\"\"}"
         );
      } else {
         buf.append(
            "text{text=\"After you become "
               + Abilities.getAbilityString(Abilities.getAbilityForItem(templateId, responder))
               + " we will ask two small matters more before we show you the way to the container'.\"}text{text=\"\"}"
         );
      }
   ...
   public final void sendAfterFifthBml(Creature responder, StringBuilder buf, Item conch) {
      ...
      if (item == null) {
         ...
         this.sendSignalToItemTemplate(item);
         buf.append(
            "text{text=\"'This spirit light will show you to the last thing you need to find.', says the "
               + addSpiritVoiceType(responder)
               + ".\"}text{text=\"\"}"
         );
      }
   ...
   private static final boolean sendSixthBml(Creature responder, StringBuilder buf, int achievementId, Item conch) {
      ...
      buf.append(
         "text{text=\"'It is a test we do to honour Ceyer since he himself understood his cowardness', the "
            + addSpiritVoiceType(responder)
            + " explains.\"}text{text=\"\"}"
      );
   ...
   public static final void sendAfterSixthBml(Creature responder, StringBuilder buf, Item conch) {
      sendSignalToHota(conch, responder);
      buf.append(
         "text{text=\"There should be a light guiding your way', the "
            + addSpiritVoiceType(responder)
            + " says. 'Let us show you the way to the area where the Hunt Of The Ancients take place.'\"}text{text=\"\"}"
      );
   ...
   private static final boolean sendSeventhBml(Creature responder, StringBuilder buf, int achievementId, Item conch) {
      ...
      conch.setAuxData((byte)12);
      buf.append(
         "text{text=\"'The hidden entrance to the place where the container with Ceyer's Key lies hidden is one task away now', concludes "
            + addSpiritVoiceType(responder)
            + ".\"}text{text=\"\"}"
      );
      ...
      buf.append(
         "text{text=\"If you are to ascend to Valrei, you must be aware of how those missions will effect you up there. We have noticed you have a Valrei map available.\"}text{text=\"\"}"
      );
   ...
   public static final void sendAfterSeventhBml(Creature responder, StringBuilder buf, Item conch) {
      buf.append(
         "text{text=\"There will be no light this time', says the "
            + addSpiritVoiceType(responder)
            + ". 'We will save our energy for the final push.'\"}text{text=\"\"}"
      );
   ...
   public static final boolean isThisAdventureServer() {
      ...
      } catch (NoSuchItemException var1) {
      }
   ...
   public static final void sendSignalToEntrance(Creature responder, Item conch) {
      if (isThisAdventureServer() && responder.isPlayer()) {
   ...
   public static final void sendSignalToHota(Item conch, Creature responder) {
      ...
      if (responder.isPlayer()) {
         ...
         if (hota != null) {
            ...
            try {
               posz = Zones.calculateHeight((float)(cx * 4), (float)(cy * 4), true);
   ...
   private final Item getItemToSend(int templateId) {
      LinkedList<Item> availItems = new LinkedList<>();
      ...
      if (availItems.size() == 1) {
         return availItems.get(0);
      } else if (availItems.size() > 0) {
         int num = Server.rand.nextInt(availItems.size());
         return availItems.get(num);
      } else {
         return null;
      }
   }
   ...
   public final void sendSignalToItemTemplate(Item item) {
      if (this.getResponder().isPlayer() && item != null) {
         ...
         this.getResponder()
            .getCommunicator()
            .sendNormalServerMessage("'Oops', the shell declares. 'We couldn't find the item.. You have to do some deity missions..'");
      }
   }
}
