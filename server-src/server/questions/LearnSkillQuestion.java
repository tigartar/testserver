...
extends Question {
    ...
    public void sendQuestion() {
        ...
        try {
            receiver = WurmId.getType(this.target) == 1 || WurmId.getType(this.target) == 0 ? Server.getInstance().getCreature(this.target) : this.getResponder();
            Skills skills = receiver.getSkills();
            for (int x = 0; x < templates.length; ++x) {
                if (x > 0) {
                    buf.append(",");
                }
                int sk = ((SkillTemplate)templates[x]).getNumber();
                try {
                    Skill skill = skills.getSkill(sk);
                    String affs = "*****".substring(0, skill.affinity);
                    buf.append(((SkillTemplate)templates[x]).getName() + " " + affs + " (" + skill.getKnowledge() + ")");
                    continue;
                }
