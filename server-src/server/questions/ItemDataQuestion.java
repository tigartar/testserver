      ...
      try {
         ...
         if (this.getResponder().getPower() >= 4) {
            ...
            if (it.isInLunchbox() && lunchbox != null) {
               decayTick = it.getLastMaintained() - it.getDecayTime() * (long)(lunchbox.getRarity() / 4 + 2) - 1L;
            } else {
            ...
            buf.append(
               "harray{label{text=\"(Set to\"};label{text=\""
                  + decayTick
                  + "\"hover=\"this number can be copied using copy line\"};label{text=\"for a decay tick!)\"}}"
            );
         }
