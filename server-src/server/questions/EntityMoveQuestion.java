...
public final class EntityMoveQuestion extends Question {
   ...
   public void answer(Properties answers) {
      if (this.getResponder().getKarma() < 5000) {
         this.getResponder()
            .getCommunicator()
            .sendNormalServerMessage("You do not have enough karma to commune with " + this.getResponder().getDeity().getName() + ".");
      } else {
                        ...
                        (new Thread(this.getResponder().getName() + "-guides-" + deity.getName() + "-Thread") {
                              ...
                              public final void run() {
                                 ...
                                 if (success) {
                                    ...
                                    success = lsw.requestDeityMove(
                                       EntityMoveQuestion.this.deityToGuide, hexnum, EntityMoveQuestion.this.getResponder().getName()
                                    );
                                    ...
                                    if (success) {
                                       EntityMoveQuestion.logger
                                          .log(Level.INFO, EntityMoveQuestion.this.getResponder().getName() + " guides " + deity.getName());
                                       EntityMoveQuestion.this.getResponder()
                                          .getCommunicator()
                                          .sendSafeServerMessage("... and " + deity.getName() + " heeds your advice!");
                                       EntityMoveQuestion.this.getResponder().modifyKarma(-5000);
                                    } else {
                                       EntityMoveQuestion.this.getResponder()
                                          .getCommunicator()
                                          .sendNormalServerMessage("... but fail to penetrate the ether to Valrei.");
                                       EntityMoveQuestion.logger
                                          .log(
                                             Level.INFO,
                                             EntityMoveQuestion.this.getResponder().getName() + " guiding but connection to " + deity.getName() + " broken."
                                          );
                                    }
                                 } else {
                                    try {
                                    ...
                                    EntityMoveQuestion.logger
                                       .log(Level.INFO, EntityMoveQuestion.this.getResponder().getName() + " guiding ignored by " + deity.getName() + ".");
                                 }
                              }
                           })
                           .start();
   ...
   public void sendQuestion() {
      ...
      if (this.getResponder().getDeity() != null) {
         ...
         buf.append(
            "text{text=\"There is 70% chance that you succed in getting your deities attention, and the cost will be 5000 karma if you do.\"}text{text=\"\"}"
         );
   ...
   public final void sendHexQuestion() {
      ...
      if (currentInt != null) {
         this.currentHex = EpicServerStatus.getValrei().getMapHex(currentInt);
         if (this.currentHex != null) {
            this.neighbours = this.currentHex.getNearMapHexes();

            for(Integer i : this.neighbours) {
               MapHex maphex = EpicServerStatus.getValrei().getMapHex(i);
               if (maphex != null) {
