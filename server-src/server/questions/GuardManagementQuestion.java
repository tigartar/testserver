      ...
      if (this.getResponder().getCitizenVillage() != null) {
         if (this.getResponder().getCitizenVillage().plan != null) {
            ...
            if (this.getResponder().getCitizenVillage().isCitizen(this.getResponder())) {
               buf.append(
                  "text{text=\"The size of "
                     + this.getResponder().getCitizenVillage().getName()
                     + " is "
                     + this.getResponder().getCitizenVillage().getDiameterX()
                     + " by "
                     + this.getResponder().getCitizenVillage().getDiameterY()
                     + ".\"}"
               );
               buf.append(
                  "text{text=\"The perimeter is "
                     + (5 + this.getResponder().getCitizenVillage().getPerimeterSize())
                     + " and it has "
                     + plan.getNumHiredGuards()
                     + " guards hired.\"}"
               );
            }
            ...
            buf.append(
               "text{text=\"The cost for hiring the guards is a one-time summoning fee that is not returned in case you decide to lower the amount of guards.\"};"
            );
            buf.append("text{text=\"\"}");
            ...
            buf.append(
               "text{text=\"How many guards do you wish to have? You currently have "
                  + plan.getNumHiredGuards()
                  + " and may hire up to "
                  + GuardPlan.getMaxGuards(this.getResponder().getCitizenVillage())
                  + ".\"};input{text=\""
                  + plan.getNumHiredGuards()
                  + "\";id=\"hired\"}; "
            );
