...
extends Question {
    ...
    public void answer(Properties answers) {
        ...
        if (val != null && val.equals("true")) {
            ...
            if (this.getResponder().isTeleporting()) {
                ...
                logger.log(Level.INFO, this.getResponder().getName() + " SUICIDE " + this.getResponder().getName() + " at coords: " + this.getResponder().getTileX() + ", " + this.getResponder().getTileY());
                this.getResponder().getCommunicator().sendAlertServerMessage("Using an old Kelatchka Nomad-trick you once heard of, you swallow your tongue and quickly fall down dead.");
                Server.getInstance().broadCastAction(this.getResponder().getName() + " falls down dead, having swallowed " + this.getResponder().getHisHerItsString() + " tongue.", this.getResponder(), 5);
    ...
    public void sendQuestion() {
        ...
        buf.append("text{text='Committing suicide may be helpful when you are stuck in a mine or fenced in. At low levels you will quickly regain the skill you lose when doing so, and more experienced players should know better than to risk becoming stuck anyways.'}");
        ...
        try {
            ...
            if (this.getResponder().isPlayer() && !this.getResponder().isPaying() && body.getKnowledge() < 20.0 && body.minimum - body.getKnowledge() > (double)0.05f && body.minimum - body.getKnowledge() <= 0.06) {
                lastDeath = true;
            }
        }
