...
public final class SimplePopup extends Question {
   ...
   public void sendQuestion(String sendButtonText) {
      ...
      if (this.missionId > 0L) {
         ...
         buf.append(
            "input{id=\"answer\";enabled=\"false\";maxchars=\""
               + this.papyrusData.getInscription().length()
               ...
               + ","
               + WurmColor.getColorBlue(this.papyrusData.getPenColour())
               + "\";text=\""
               + this.papyrusData.getInscription()
               + "\"}"
         );
