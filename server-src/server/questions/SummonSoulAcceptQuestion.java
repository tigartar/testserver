...
public class SummonSoulAcceptQuestion extends Question {
   ...
   public void answer(Properties answers) {
      ...
      if (accept) {
         ...
         this.getResponder().setTeleportPoints((float)this.summonX, (float)this.summonY, this.summonLayer, this.summonFloor);
   ...
   public void sendQuestion() {
      try {
         ...
         buf.append(
            "text{text='"
               + asker2.getName()
               + " would like to summon you to their location.'}text{text=''}text{text=''}text{text='Would you like to accept?'}"
         );
