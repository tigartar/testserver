...
implements QuestionTypes {
    ...
    private List<Kingdom> klist = new LinkedList<Kingdom>();
    ...
    public KingdomMembersQuestion(Creature aResponder, long aTarget, byte kingdomId) {
        super(aResponder, winTitle, question, 131, aTarget);
    ...
    public void answer(Properties answers) {
        ...
        if (val != null && (val = answers.getProperty(filterText)) != null) {
            this.filter = val;
        ...
        if (this.getResponder().getPower() > 0) {
            ...
            if ((val = answers.getProperty(gmtool)) != null) {
                val = answers.getProperty(selectGrp);
                if (val == null) {
                    return;
                }
        ...
        if ((val = answers.getProperty(next)) != null) {
            KingdomMembersQuestion kmq = new KingdomMembersQuestion(this.getResponder(), this.target, this.kingdom);
        ...
        if (King.isKing(this.getResponder().getWurmId(), this.kingdom.getId())) {
            ...
            if (val != null && (val = answers.getProperty(toExpel)).compareTo(this.playerInfo.getName()) == 0) {
            ...
            if (val != null) {
                ...
                answers.forEach((BiConsumer<? super Object, ? super Object>)((BiConsumer<Object, Object>)(k, v) -> this.handleAppointment(k.toString(), v.toString(), apps)));
                return;
            }
        }
    }
    ...
    private void sendKingdomList() {
        StringBuilder buf = new StringBuilder(this.getHeader(question));
        ...
        for (int i = 0; i < kingdoms.length; ++i) {
            if (kingdoms[i].getId() == 4 || kingdoms[i].getId() == 0) continue;
        ...
        this.getResponder().getCommunicator().sendBml(250, 150, true, true, buf.toString(), RGB[0], RGB[1], RGB[2], winTitle);
    }

    private PlayerInfo[] filterList(PlayerInfo[] memberArray) {
        ArrayList<PlayerInfo> memberList = new ArrayList<PlayerInfo>();
        for (int x = 0; x < memberArray.length; ++x) {
            if (!PlayerInfoFactory.wildCardMatch(memberArray[x].getName(), this.filter)) continue;
    ...
    private void sendMemberList() {
        ...
        this.getResponder().getCommunicator().sendBml(600, 250 + rows * 13, true, true, buf.toString(), RGB[0], RGB[1], RGB[2], winTitle);
    }
    ...
    private final void appendMember(int x, StringBuilder buf) {
        ...
        buf.append("radio{id='" + this.members[this.currentIndex + x].wurmId + "'; group='" + selectGrp + "'};");
    ...
    private void sendExpelConfirmation() {
        ...
        this.getResponder().getCommunicator().sendBml(320, 225, true, true, buf.toString(), RGB[0], RGB[1], RGB[2], winTitle);
    }
    ...
    private void sendAppointWindow() {
        ...
        this.getResponder().getCommunicator().sendBml(620, 460, true, true, buf.toString(), RGB[0], RGB[1], RGB[2], winTitle);
    }

    private void addTitleStrings(Appointments a, StringBuilder buf) {
        int x;
        for (x = 0; x < a.availableTitles.length; ++x) {
            if (Appointments.getMaxAppointment(this.kingdom.getId(), x) == 0) continue;
        ...
        if (a.availableTitles.length % 3 != 0) {
            for (x = 0; x < 3 - a.availableTitles.length % 3; ++x) {
    ...
    private void addOrderStrings(Appointments a, StringBuilder buf) {
        int x;
        for (x = 0; x < a.availableOrders.length; ++x) {
            if (Appointments.getMaxAppointment(this.kingdom.getId(), x) == 0) continue;
            int oId = x + 30;
            ...
            if (a.getAvailOrdersForId(oId) > 0) {
        ...
        if (a.availableOrders.length % 3 != 0) {
            for (x = 0; x < 3 - a.availableOrders.length % 3; ++x) {
                buf.append("label{text=''};");
            }
        }
    }
    ...
    private void addOfficeStrings(Appointments a, StringBuilder buf) {
        int x;
        for (x = 0; x < a.officials.length; ++x) {
            String oName;
            int oId = x + 1500;
            Appointment o = a.getAppointment(oId);
            if (a.officials[x] == this.player.getWurmId()) {
                ...
                buf.append("checkbox{id='" + oId + "'; unconfirm=''; unquestion=\"" + conf + "\"; selected='true'}; label{color='" + green + "'; text=\"" + o.getNameForGender(this.player.getSex()) + "\"};");
                continue;
            }
            if (a.isOfficeSet(oId)) {
                oName = PlayerInfoFactory.getPlayerName(a.officials[x]);
            ...
            if (a.officials[x] > 0L) {
                ...
                buf.append("checkbox{id='" + oId + "'; hover=\"Current: " + oName + "\"; confirm=''; question=\"" + conf + "\"}; label{color='" + orange + "'; hover=\"Current: " + oName + "\"; text=\"" + o.getNameForGender(this.player.getSex()) + "\"};");
                continue;
            }
            buf.append("checkbox{id='" + oId + "'}; label{text=\"" + o.getNameForGender(this.player.getSex()) + "\"};");
        }
        if (a.officials.length % 3 != 0) {
            for (x = 0; x < 3 - a.officials.length % 3; ++x) {
                buf.append("label{text=''};");
            }
        }
    }
