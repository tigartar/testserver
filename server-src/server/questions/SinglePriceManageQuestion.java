...
public final class SinglePriceManageQuestion extends Question {
   ...
   public void sendQuestion() {
      if (this.target == -10L) {
         ...
         buf.append(
            "label{text=\" \"};label{text='Gold'};label{text=\" \"};label{text='Silver'};label{text=\" \"};label{text='Copper'};label{text=\" \"};label{text='Iron'};"
         );
         buf.append(
            "label{text=\" \"};input{maxchars=\"2\";id=\"gold\";text=\"0\"};label{text=\" \"};input{maxchars=\"2\";id=\"silver\";text=\"0\"};label{text=\" \"};input{maxchars=\"2\";id=\"copper\";text=\"0\"};label{text=\" \"};input{maxchars=\"2\";id=\"iron\";;text=\"0\"}"
         );
         buf.append("}");
         ...
         try {
            ...
            if (item.getOwnerId() == this.getResponder().getWurmId()) {
               ...
               buf.append(
                  "label{text=\""
                     ...
                     + "\"};harray{label{text=\" \"};input{maxchars=\"3\";id=\""
                     + wid
                     + "gold\";text=\""
                     + change.getGoldCoins()
                     + "\"};};harray{label{text=\" \"};input{maxchars=\"3\";id=\""
                     + wid
                     + "silver\";text=\""
                     + change.getSilverCoins()
                     + "\"};};harray{label{text=\" \"};input{maxchars=\"3\";id=\""
                     + wid
                     + "copper\";text=\""
                     + change.getCopperCoins()
                     + "\"};};harray{label{text=\" \"};input{maxchars=\"3\";id=\""
                     + wid
                     + "iron\";text=\""
                     + change.getIronCoins()
            ...
            logger.log(Level.WARNING, this.getResponder().getName(), (Throwable)var6);
         }
      }
   }
