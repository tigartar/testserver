      ...
      try {
         ...
         buf.append(
            "checkbox{id=\"unlimitC\""
               ...
               + " citizens.\"}text{text=\"Your upkeep costs are doubled as long as you have more than that amount of citizens.\"}"
         );
         if (this.selecting) {
         ...
         for(int x = 0; x < citizens.length; ++x) {
            if (this.allowedLetters.indexOf(citizens[x].getName().substring(0, 1).toLowerCase()) >= 0) {
               ...
               VillageRole role = citizens[x].getRole();
               ...
               if (role.getStatus() != 4 && role.getStatus() != 5 && role.getStatus() != 6) {
                  ...
                  PlayerState cState = PlayerInfoFactory.getPlayerState(citizens[x].getId());
                  if (cState == null) {
                  ...
                  Wagoner wagoner = Wagoner.getWagoner(citizens[x].getId());
                  buf.append(
                     "label{text=\""
                        + citizens[x].getName()
                        + "\"}label{text=\"role:\"};label{text=\""
                        + citizens[x].getRole().getName()
                        + "\"};label{text=\""
         ...
         logger.log(Level.WARNING, "Failed to locate village/homestead deed with id " + this.target, (Throwable)var16);
         ...
         logger.log(Level.WARNING, "Failed to locate the village/homestead for the deed with id " + this.target, (Throwable)var17);
