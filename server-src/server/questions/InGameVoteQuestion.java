...
public class InGameVoteQuestion extends Question {
            ...
            switch(this.part) {
               case 0:
                  ...
                  if (this.questionId != -1) {
                     ...
                     } else if (!hasPower && !p.hasVoted(this.questionId)) {
                        nextPart = 2;
                        windowTitle = "Vote";
                        header = vq.getQuestionTitle();
                     } else {
               ...
               case 2:
                  ...
                  } else if (!Servers.isThisLoginServer() && !this.isLoginAvailable()) {
                     this.getResponder()
                        .getCommunicator()
                        .sendNormalServerMessage("Login server is currently down, so your vote cannot be registered, please try again later.");
                  } else {
      ...
      try {
         ...
         switch(this.part) {
            case 0:
               ...
               if (vqs.length <= 0) {
                  ...
                  buf.append(
                     "table{rows=\""
                        + (vqs.length + 2)
                        + "\";cols=\"5\";label{text=\"\"};label{type=\"bold\";text=\"Question\"};label{type=\"bold\";text=\"Vote Start\"};label{type=\"bold\";text=\"Vote End\"};label{type=\"bold\";text=\""
                        + (cols == 5 ? "Voted?" : "")
                        + "\"};"
                  );
                  ...
                  for(VoteQuestion vq : vqs) {
                     ...
                     buf.append(
                        (colour.length() == 0 ? "label{text=\"\"}" : "radio{group=\"qid\";id=\"" + vq.getQuestionId() + "\"};")
                           + "label{"
                           + colour
                           + "text=\""
                           + vq.getQuestionTitle()
                           ...
                           + colour
                           + "text=\""
                           + Tickets.convertTime(vq.getVoteStart())
                           + "\"};label{"
                           + colour
                           + "text=\""
                           + Tickets.convertTime(vq.getVoteEnd() - 1000L)
                           + "\"};label{"
                           + colour
                           + "text=\""
                           + (cols == 5 ? strVoted : "")
                  ...
                  buf.append(
                     "radio{group=\"qid\";id=\"-1\";selected=\"true\";hidden=\"true\"};label{text=\"\"};label{text=\"\"};label{text=\"\"};label{text=\"\"};"
                  );
                  buf.append("}");
               }
            ...
            case 2:
               ...
               buf.append(
                  "text{type=\"bold\";text=\"Note: Votes will be used as guidelines to assess player sentiment. There is no guarantee that any majority vote will result in the implementation or removal of anything.\"}"
               );
               height += 32;
               break;
            case 4:
         ...
         this.getResponder().getCommunicator().sendNormalServerMessage("Exception:" + var18.getMessage());
      }
   ...
   private boolean isLoginAvailable() {
      return Servers.isThisLoginServer() ? true : Servers.loginServer.isAvailable(5, true);
   }
}
