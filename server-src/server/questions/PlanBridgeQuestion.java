...
public class PlanBridgeQuestion extends Question {
   ...
   private final String[] spansBrick = new String[]{
      "", "E", "aA", "aDA", "abBA", "abCBA", "ESabBA", "ESabCBA", "aASabCBA", "ESabCBASE", "ESabCBASaA", "abCBASabCBA"
   };
   private final String[] archWood = new String[]{
      "",
      "C",
      "aA",
      "aCA",
   ...
   public PlanBridgeQuestion(
      Creature aResponder,
      int aTargetFloorLevel,
      Point aStart,
      Point aEnd,
      byte aDir,
      int aWidth,
   ...
   public PlanBridgeQuestion(
      Creature aResponder, int aTargetFloorLevel, String aQuestion, Point aStart, Point aEnd, byte aDir, int aWidth, int aLength, int aPage
   ) {
            ...
            if (this.page == 0) {
               ...
               if (!this.arched && this.length > 5) {
                  this.getResponder()
                     .getCommunicator()
                     .sendOpenPlanWindow(
                        this.bridgeName, this.dir, (byte)this.length, (byte)this.width, this.start, this.end, this.bridgeType, this.bridgePlan
                     );
                  return;
               }

               PlanBridgeQuestion pbq = new PlanBridgeQuestion(
                  this.getResponder(),
                  this.targetFloorLevel,
                  this.start,
                  this.end,
                  this.dir,
                  this.width,
               ...
               if (this.steepnessSelected <= 20) {
                  PlanBridgeMethods.planBridge(
                     this.getResponder(), this.dir, this.bridgeType, this.arched, this.bridgePlan, this.steepness, this.start, this.end, this.bridgeName
                  );
               } else {
   ...
   public void sendQuestion() {
      ...
      String bridge = "";
      String bridgeArea = "Planned bridge area is "
         + this.length
         + " tile"
         + (this.length == 1 ? "" : "s")
         + " long and "
      ...
      if (this.width != 1) {
         ...
         String[] reply = PlanBridgeMethods.isBuildingOk(
            BridgeConstants.BridgeMaterial.ROPE.getCode(), this.dir, onSurface, this.start, this.startFloorlevel, this.end, this.endFloorlevel
         );
         this.fail = reply[0];
         this.reason = reply[1];
      }
      ...
      if (this.width > 2) {
      ...
      } else if (this.length > var29) {
         this.fail = "Too Long";
         this.reason = "Wood bridges are restricted to " + var29 + " tiles long";
      } else if (this.hasLowSkill(requiredSkill, 10, var29, false)) {
         this.fail = "Low Skill";
         if (requiredSkill.getKnowledge(0.0) < 10.0) {
         ...
         String[] reply = PlanBridgeMethods.isBuildingOk(
            BridgeConstants.BridgeMaterial.WOOD.getCode(), this.dir, onSurface, this.start, this.startFloorlevel, this.end, this.endFloorlevel
         );
         this.fail = reply[0];
         this.reason = reply[1];
         doneWoodCheck = true;
      ...
      if (this.width > 3) {
         ...
         this.fail = "Too Long";
         this.reason = "Brick bridges are restricted to " + var29 + " tiles long";
      } else if (this.hasLowSkill(requiredSkill, 30, var29, false)) {
         this.fail = "Low Skill";
         ...
         String[] reply = PlanBridgeMethods.isBuildingOk(
            BridgeConstants.BridgeMaterial.BRICK.getCode(), this.dir, onSurface, this.start, this.startFloorlevel, this.end, this.endFloorlevel
         );
         this.fail = reply[0];
         this.reason = reply[1];
         doneStoneCheck = true;
      ...
      if (this.width > 3) {
         ...
         this.fail = "Too Long";
         this.reason = "Marble bridges are restricted to " + var29 + " tiles long";
      } else if (this.hasLowSkill(requiredSkill, 40, var29, false)) {
         this.fail = "Low Skill";
         ...
         if (doneStoneCheck) {
            ...
            String[] reply = PlanBridgeMethods.isBuildingOk(
               BridgeConstants.BridgeMaterial.MARBLE.getCode(), this.dir, onSurface, this.start, this.startFloorlevel, this.end, this.endFloorlevel
            );
            this.fail = reply[0];
            this.reason = reply[1];
            stoneFail = reply;
      ...
      if (this.fail.length() > 0) {
         bridges.append(
            this.addBridgeEntry(BridgeConstants.BridgeMaterial.MARBLE, false, "Flat " + BridgeConstants.BridgeMaterial.MARBLE.getTextureName(), bridge)
         );
         bridges.append(
            this.addBridgeEntry(BridgeConstants.BridgeMaterial.SLATE, false, "Flat " + BridgeConstants.BridgeMaterial.SLATE.getTextureName(), bridge)
         );
         bridges.append(
            this.addBridgeEntry(
               BridgeConstants.BridgeMaterial.ROUNDED_STONE, false, "Flat " + BridgeConstants.BridgeMaterial.ROUNDED_STONE.getTextureName(), bridge
            )
         );
         bridges.append(
            this.addBridgeEntry(BridgeConstants.BridgeMaterial.POTTERY, false, "Flat " + BridgeConstants.BridgeMaterial.POTTERY.getTextureName(), bridge)
         );
         bridges.append(
            this.addBridgeEntry(BridgeConstants.BridgeMaterial.SANDSTONE, false, "Flat " + BridgeConstants.BridgeMaterial.SANDSTONE.getTextureName(), bridge)
         );
         bridges.append(
            this.addBridgeEntry(BridgeConstants.BridgeMaterial.RENDERED, false, "Flat " + BridgeConstants.BridgeMaterial.RENDERED.getTextureName(), bridge)
         );
      } else {
         bridges.append(
            this.addBridgeEntry(
               BridgeConstants.BridgeMaterial.MARBLE, "Flat " + BridgeConstants.BridgeMaterial.MARBLE.getTextureName(), this.getBrickSpan(this.length)
            )
         );
         bridges.append(
            this.addBridgeEntry(
               BridgeConstants.BridgeMaterial.SLATE, "Flat " + BridgeConstants.BridgeMaterial.SLATE.getTextureName(), this.getBrickSpan(this.length)
            )
         );
         bridges.append(
            this.addBridgeEntry(
               BridgeConstants.BridgeMaterial.ROUNDED_STONE,
               "Flat " + BridgeConstants.BridgeMaterial.ROUNDED_STONE.getTextureName(),
               this.getBrickSpan(this.length)
            )
         );
         bridges.append(
            this.addBridgeEntry(
               BridgeConstants.BridgeMaterial.POTTERY, "Flat " + BridgeConstants.BridgeMaterial.POTTERY.getTextureName(), this.getBrickSpan(this.length)
            )
         );
         bridges.append(
            this.addBridgeEntry(
               BridgeConstants.BridgeMaterial.SANDSTONE, "Flat " + BridgeConstants.BridgeMaterial.SANDSTONE.getTextureName(), this.getBrickSpan(this.length)
            )
         );
         bridges.append(
            this.addBridgeEntry(
               BridgeConstants.BridgeMaterial.RENDERED, "Flat " + BridgeConstants.BridgeMaterial.RENDERED.getTextureName(), this.getBrickSpan(this.length)
            )
         );
      }
      ...
      if (!this.getResponder().isOnSurface()) {
         ...
         if (res.failed()) {
            ...
            bridges.append(
               this.addBridgeEntry(BridgeConstants.BridgeMaterial.WOOD, true, "Arched " + BridgeConstants.BridgeMaterial.WOOD.getTextureName(), bridge)
            );
            bridges.append(
               this.addBridgeEntry(BridgeConstants.BridgeMaterial.BRICK, true, "Arched " + BridgeConstants.BridgeMaterial.BRICK.getTextureName(), bridge)
            );
            bridges.append(
               this.addBridgeEntry(BridgeConstants.BridgeMaterial.MARBLE, true, "Arched " + BridgeConstants.BridgeMaterial.MARBLE.getTextureName(), bridge)
            );
            bridges.append(
               this.addBridgeEntry(BridgeConstants.BridgeMaterial.SLATE, true, "Arched " + BridgeConstants.BridgeMaterial.SLATE.getTextureName(), bridge)
            );
            bridges.append(
               this.addBridgeEntry(
                  BridgeConstants.BridgeMaterial.ROUNDED_STONE, true, "Arched " + BridgeConstants.BridgeMaterial.ROUNDED_STONE.getTextureName(), bridge
               )
            );
            bridges.append(
               this.addBridgeEntry(BridgeConstants.BridgeMaterial.POTTERY, true, "Arched " + BridgeConstants.BridgeMaterial.POTTERY.getTextureName(), bridge)
            );
            bridges.append(
               this.addBridgeEntry(
                  BridgeConstants.BridgeMaterial.SANDSTONE, true, "Arched " + BridgeConstants.BridgeMaterial.SANDSTONE.getTextureName(), bridge
               )
            );
            bridges.append(
               this.addBridgeEntry(BridgeConstants.BridgeMaterial.RENDERED, true, "Arched " + BridgeConstants.BridgeMaterial.RENDERED.getTextureName(), bridge)
            );
         }
      }
      ...
      if (this.fail.length() == 0) {
         ...
         if (this.width > 2) {
            ...
            this.fail = "Too Long";
            this.reason = "Arched wood bridges are restricted to " + var29 + " tiles long";
         } else if (this.hasLowSkill(requiredSkill, 20, var29, true)) {
            this.fail = "Low Skill";
            if (requiredSkill.getKnowledge(0.0) < 10.0) {
         ...
         if (this.width > 3) {
            ...
            this.fail = "Too Long";
            this.reason = "Arched brick bridges are restricted to " + var29 + " tiles long";
         } else if (this.hasLowSkill(requiredSkill, 50, var29, true)) {
            this.fail = "Low Skill";
         ...
         if (this.width > 3) {
            ...
            this.fail = "Too Long";
            this.reason = "Arched marble bridges are restricted to " + var29 + " tiles long";
         } else if (this.hasLowSkill(requiredSkill, 60, var29, true)) {
            this.fail = "Low Skill";
         ...
         bridges.append(
            this.addBridgeEntry(BridgeConstants.BridgeMaterial.SANDSTONE, true, "Arched " + BridgeConstants.BridgeMaterial.SANDSTONE.getTextureName(), bridge)
         );
         bridges.append(
            this.addBridgeEntry(BridgeConstants.BridgeMaterial.RENDERED, true, "Arched " + BridgeConstants.BridgeMaterial.RENDERED.getTextureName(), bridge)
         );
   ...
   private String addSagEntry(int testSag) {
      ...
      if (isOk) {
         ...
         buf.append(
            "harray{image{src=\"img.gui.bridge.north\";size=\"12,12\";text=\""
               + this.reason
   ...
   private boolean ropeHeightsOk(String bridge, int testSag) {
      this.fail = "";
      this.reason = "";
      int[] hts = PlanBridgeMethods.calcHeights(
         this.getResponder(), this.dir, BridgeConstants.BridgeMaterial.ROPE.getCode(), true, bridge, testSag, this.start, this.end
      );
   ...
   private String addBridgeEntry(BridgeConstants.BridgeMaterial bridgeMaterial, boolean aArched, String aTypeName, String bridge) {
      ...
      if (this.fail.length() > 0) {
         ...
         buf.append(
            "harray{image{src=\"img.gui.bridge.blank\";size=\"2,34\";text=\"\"};radio{group=\"bridgereply\";id=\""
               + bridgeMaterial.getCode()
               + ","
               + aArched
               + ","
               + bridge
               ...
               + aTypeName
               + "\"};"
               + "image{src=\"img.gui.bridge.blank\";size=\"2,34\";text=\"\"}"
               + "};"
         );
   ...
   private String addArchs(int alength, boolean isStone) {
      if (alength == 2) {
         ...
         if (alength == 3) {
            ...
            return alength < 9 ? this.makeArch("ab", alength, 'F', "BA") : this.makeArch("ESab", alength, 'F', "BASE");
         }
      } else {
   ...
   private boolean hasLowSkill(Skill requiredSkill, int minSkill, int maxLength, boolean slidingScale) {
      if (requiredSkill.getKnowledge(0.0) < (double)minSkill) {
         ...
         if (slidingScale) {
            float as = (float)Math.min(requiredSkill.getKnowledge(0.0), 99.0);
            float k = 90.0F / (99.0F - (float)minSkill);
            float a = 90.0F - (as - (float)minSkill) * k;
            float b = (float)Math.toRadians((double)a);
            float c = (float)Math.sin((double)b);
            float d = 1.0F - c;
            float r = d * (float)(maxLength - 5) + 5.0F;
            if (r < (float)this.length) {
               return true;
            }
         }
   ...
   public void sendQuestionPage2() {
      ...
      if (this.arched && this.bridgeType != BridgeConstants.BridgeMaterial.ROPE.getCode() && this.steepnessSelected > 20) {
         ...
         buf.append(
            "label{text=\"Once satisified with name"
               + (this.bridgeType == BridgeConstants.BridgeMaterial.ROPE.getCode() ? " and saggyness" : (this.arched ? " and steepness for arch" : ""))
               + ", select 'Finalise'\"}"
         );
      ...
      String sBridgeType = BridgeConstants.BridgeMaterial.fromByte(this.bridgeType).getName().toLowerCase();
      String sTitle = "Name your "
         + (this.bridgeType == BridgeConstants.BridgeMaterial.ROPE.getCode() ? "" : (this.arched ? "arched " : "flat "))
         + sBridgeType
         + " bridge";
