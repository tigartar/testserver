   ...
   public void answer(Properties answers) {
      ...
      if (this.type == 0) {
         ...
         if (this.type == 148) {
            ...
            WagonerHistory wh = new WagonerHistory(
               this.getResponder(),
               this.wagoner,
               this.deliveryId,
               this.sortBy,
   ...
   public void sendQuestion() {
      ...
      switch(absSortBy) {
         ...
         case 7:
            Arrays.sort(deliveries, new Comparator<Delivery>() {
               public int compare(Delivery param1, Delivery param2) {
                  return param1.getWhenDelivered() < param2.getWhenDelivered() ? 1 * upDown : -1 * upDown;
               }
            });
      }
      ...
      buf.append(
         "table{rows=\"1\";cols=\"8\";label{text=\"\"};"
            ...
            + this.colHeader("Sender Costs", 5, this.sortBy)
            + this.colHeader("Receiver Costs", 6, this.sortBy)
            + this.colHeader("When Delivered", 7, this.sortBy)
      );
      ...
      for(Delivery delivery : deliveries) {
         buf.append(
            "label{text=\"\"};label{text=\""
               ...
               + delivery.getCrates()
               + "\"};label{text=\""
               + delivery.getSenderCostString()
               + "\"};label{text=\""
               + delivery.getReceiverCostString()
               + "\"};label{text=\""
               + delivery.getStringDelivered()
               + "\"};"
         );
      }
      ...
      buf.append(
         "harray{button{text=\"Filter\";id=\"filter\"};label{text=\" by \"}checkbox{id=\"waitaccept\";text=\"Waiting for accept  \""
            + (this.waitAccept ? ";selected=\"true\"" : "")
            + "};checkbox{id=\"inqueue\";text=\"In queue  \""
