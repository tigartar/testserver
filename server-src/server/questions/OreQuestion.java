...
extends Question {
    ...
    private final Map<Integer, Integer> oretiles = new HashMap<Integer, Integer>();
    ...
    public void answer(Properties answers) {
        if (this.type == 82) {
            ...
            if (this.rod.getWurmId() == this.target && (d1 = answers.getProperty("data1")) != null) {
                Integer newType;
                int index = 0;
                try {
                ...
                if ((newType = this.oretiles.get(index)) == null) {
                ...
                if (Tiles.decodeType(Server.caveMesh.getTile(this.tilex, this.tiley)) == Tiles.Tile.TILE_CAVE_WALL.id) {
                    ...
                    this.getResponder().getCommunicator().sendNormalServerMessage("The wall changes to purest " + Tiles.getTile((byte)nt).tiledesc + " and the rod vanishes!");
    ...
    public void sendQuestion() {
        ...
        for (int x = 0; x < tiles.length; ++x) {
            if (tiles[x] == null || !Tiles.isOreCave(tiles[x].id) || tiles[x].id == Tiles.Tile.TILE_CAVE_WALL_ORE_ADAMANTINE.id || tiles[x].id == Tiles.Tile.TILE_CAVE_WALL_ORE_GLIMMERSTEEL.id) continue;
