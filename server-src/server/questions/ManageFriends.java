...
public final class ManageFriends extends Question {
   ...
   public ManageFriends(Creature aResponder) {
      ...
      Arrays.sort((Object[])this.friends);
   }
   ...
   public void answer(Properties aAnswer) {
      ...
      if (this.type == 0) {
         ...
         if (this.type == 118) {
            ...
            String add = aAnswer.getProperty("add");
            ...
            if (update != null && update.equalsIgnoreCase("true")) {
               ...
               for(int i = 0; i < this.friends.length; ++i) {
                  ...
                  if (cat != null) {
                     ...
                     if (this.friends[i].getCatId() != catId || !this.friends[i].getNote().equals(note)) {
                        ((Player)this.getResponder()).updateFriendData(this.friends[i].getFriendId(), catId, note);
                        if (this.friends[i].getCatId() != catId) {
                           this.getResponder()
                              .getCommunicator()
                              .sendNormalServerMessage(
                                 this.friends[i].getName() + " is now in your category " + Friend.Category.catFromInt(catId).name() + "."
                              );
                        }
   ...
   public void sendQuestion() {
      ...
      for(Friend friend : this.friends) {
         ...
         buf.append(
            "harray{varray{image{src=\"img.gui.bridge.blank\";size=\"200,1\";text=\"\"}label{text=\""
               + pName
               + "\"}};radio{group=\"cat"
               + row
               + "\";id=\"3\";selected=\""
               + (cat == 3)
               + "\";text=\"Trusted \";hover=\"Trusted\"}radio{group=\"cat"
               + row
               + "\";id=\"2\";selected=\""
               + (cat == 2)
               + "\";text=\"Friend \";hover=\"Friend\"}radio{group=\"cat"
               + row
               + "\";id=\"1\";selected=\""
               + (cat == 1)
               + "\";text=\"Contact \";hover=\"Contact\"}radio{group=\"cat"
               + row
               + "\";id=\"0\";selected=\""
               + (cat == 0)
               + "\";text=\"Other \";hover=\"Other\"}harray{label{text=\" \"};button{id=\"rem"
               + row
               + "\";text=\"Remove\";confirm=\"You are about to remove "
               + pName
               + "  from your friends list.\";question=\"Do you really want to do that?\";hover=\"remove "
               + pName
      ...
      if (wffn.length() != 0) {
         if (this.player.askingFriend()) {
            ...
            buf.append(
               "harray{button{text=\"Send Reply\";id=\"reply\"};label{text=\" and add them to \"};dropdown{id=\"cat\";default=\"0\";options=\"Other,Contacts,Friends,Trusted\"}label{text=\" category.\"}};"
            );
         }
      } else {
         buf.append(
            "harray{button{text=\"Send Request\";id=\"add\"};label{text=\" to \"};input{maxchars=\"40\";id=\"addname\";onenter=\"add\"};label{text=\" so can add them to your \"};dropdown{id=\"addcat\";default=\"0\";options=\"Other,Contacts,Friends,Trusted\"}label{text=\" category.\"}};"
         );
      }
