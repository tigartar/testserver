   ...
   public void sendQuestion() {
      ...
      buf.append(
         "border{border{size=\"20,20\";null;null;label{type='bold';text=\""
            + this.question
            + "\"};harray{label{text=\" \"};button{text=\"Close\";id=\"close\"};label{text=\" \"}};null;}null;scroll{vertical=\"true\";horizontal=\"false\";varray{rescale=\"true\";passthrough{id=\"id\";text=\""
            + this.getId()
            + "\"}"
      );
      ...
      if (this.messageBoard.mayAccessHold(this.getResponder())) {
         ...
         if (publicNotices.length > 0) {
            Arrays.sort((Object[])publicNotices);

            for(VillageMessage vm : publicNotices) {
      ...
      if (this.messageBoard.mayAccessHold(this.getResponder()) && this.getResponder().getCitizenVillage() == this.village) {
         ...
         if (notices.length > 0) {
            ...
            Arrays.sort((Object[])notices);

            for(VillageMessage vm : notices) {
               buf.append(this.showMessage(vm));
            }
         } else {
         ...
         VillageMessage[] personals = VillageMessages.getVillageMessages(this.village.getId(), this.getResponder().getWurmId());
         if (personals.length > 0) {
            Arrays.sort((Object[])personals);

            for(VillageMessage vm : personals) {
               buf.append(this.showMessage(vm));
            }
         } else {
      ...
      if (info == null) {
         ...
         if (vr == null) {
            ...
            buf.append(
               "input{id=\"ans"
                  + vm.getPostedTime()
                  + "\";enabled=\"false\";maxchars=\""
                  + vm.getMessage().length()
                  + "\";maxlines=\"-1\";bgcolor=\"200,200,200\";color=\""
                  + WurmColor.getColorRed(vm.getPenColour())
                  + ","
                  + WurmColor.getColorGreen(vm.getPenColour())
                  + ","
                  + WurmColor.getColorBlue(vm.getPenColour())
                  + "\";text=\""
                  + vm.getMessage()
                  + "\"}"
            );
            if (vm.getToId() != -10L && vm.getToId() != -1L) {
               String delButton = "harray{label{text=\" \"};button{text=\"Delete\";id=\"del"
                  + vm.getPostedTime()
                  + "\"confirm=\"You are about to delete a personal message from "
                  + vm.getPosterName()
                  + ".\";question=\"Do you really want to do that?\"}label{text=\" \"};}";
               buf.append(
                  "border{size=\"20,20\";null;null;harray{label{text=\"From:"
                     + info.getName()
                     + "  \"};label{text=\"Role:"
                     + vr.getName()
                     + "  \"};label{text=\"When:"
               ...
               String delButton = "harray{label{text=\" \"};button{text=\"Delete\";id=\""
                  + id
                  + "\"confirm=\"You are about to delete a "
                  + note
                  + " posted by "
                  + vm.getPosterName()
                  + ".\";question=\"Do you really want to do that?\"}label{text=\" \"};}";
               ...
               buf.append(
                  "border{size=\"20,20\";null;null;harray{label{text=\"Posted by: "
                     + info.getName()
                     + "  \"};label{text=\"Role: "
                     + vr.getName()
                     + "  \"};label{text=\"When: "
