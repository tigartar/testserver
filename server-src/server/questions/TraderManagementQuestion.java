...
implements TimeConstants {
    ...
    public static void dismissMerchant(Creature dismisser, long target) {
        block17: {
            try {
                ...
                if (trader != null) {
                    if (!trader.isTrading()) {
                        ...
                        for (int x = 0; x < items.length; ++x) {
                            if (items[x].getTemplateId() != 300 || items[x].getData() != target) continue;
                            items[x].setData(-1, -1);
                            break;
                        }
                        if ((shop = Economy.getEconomy().getShop(trader)) != null) {
                            try {
                                ...
                                ArrayList<Item> largeItems = new ArrayList<Item>();
                                for (Item realItem : trader.getInventory().getAllItems(false)) {
                                    if (realItem.getTemplate() == null || realItem.getTemplate().isComponentItem() || backPack.insertItem(realItem, false)) continue;
                                    largeItems.add(realItem);
                                }
                                WurmMail mail = new WurmMail(0, backPack.getWurmId(), shop.getOwnerId(), shop.getOwnerId(), 0L, System.currentTimeMillis() + 60000L, System.currentTimeMillis() + (Servers.isThisATestServer() ? 3600000L : 14515200000L), Servers.localServer.id, false, false);
                                ...
                                for (Item i : largeItems) {
                                    WurmMail largeMail = new WurmMail(0, i.getWurmId(), shop.getOwnerId(), shop.getOwnerId(), 0L, System.currentTimeMillis() + 60000L, System.currentTimeMillis() + (Servers.isThisATestServer() ? 3600000L : 14515200000L), Servers.localServer.id, false, false);
                                ...
                                logger.log(Level.WARNING, e.getMessage() + " " + trader.getName() + " at " + trader.getTileX() + ", " + trader.getTileY(), e);
                            }
                        } else {
                ...
                if (dismisser == null) break block17;
    ...
    public void answer(Properties answers) {
        ...
        if (this.isDismissing) {
            ...
            if (Boolean.parseBoolean(val)) {
                TraderManagementQuestion.dismissMerchant(this.getResponder(), this.target);
            } else {
    ...
    private String contractQuestion() {
        ...
        try {
            ...
            if (traderId != -1L && (trader = Server.getInstance().getCreature(traderId)).isNpcTrader()) {
