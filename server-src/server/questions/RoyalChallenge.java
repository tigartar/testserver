...
implements TimeConstants {
    ...
    public void answer(Properties aAnswers) {
        King k;
        if (this.getResponder().isKing() && (k = King.getKing(this.getResponder().getKingdomId())) != null) {
            block17: {
                ...
                if (decline) {
                    ...
                    if (k.hasFailedAllChallenges()) {
                        this.getResponder().getCommunicator().sendAlertServerMessage("The people of " + Kingdoms.getNameFor(k.kingdom) + " may now vote you from the throne at the duelling ring.");
                        ...
                        Server.getInstance().broadCastNormal(this.getResponder().getName() + " may now be voted away from the throne within one week at the duelling stone.");
                ...
                if (!accept) {
                    ...
                    this.getResponder().getCommunicator().sendNormalServerMessage("Unless you answer this challenge within " + tl + " you will automatically have declined " + (k.getDeclinedChallengesNumber() + 1) + " challenges.");
                        ...
                        if (valhour != null && valhour.length() > 0) {
                            try {
                                ...
                                if (Servers.localServer.testServer) {
                                    this.getResponder().getCommunicator().sendSafeServerMessage("You have accepted the challenge and since this is the test server you must be at the duelling ring exactly in 2 minutes instead of " + day + " days and " + hour + " hours (which is " + (day * 24 + hour) + " hours away). You must stay there a bit more than half an hour until you receive a message.");
                                    time = System.currentTimeMillis() + 120000L;
                                } else {
                                    this.getResponder().getCommunicator().sendSafeServerMessage("You have accepted the challenge and must be at the duelling ring exactly in " + day + " days and " + hour + " hours (which is " + (day * 24 + hour) + " hours away). You must stay there a bit more than half an hour until you receive a message.");
                                }
                                k.setChallengeAccepted(time);
                                Message mess = new Message(this.getResponder(), 10, Kingdoms.getChatNameFor(this.getResponder().getKingdomId()), "<" + this.getResponder().getName() + "> has accepted the challenge and must be at the duelling ring exactly in " + day + " days and " + hour + " hours");
    ...
    public void sendQuestion() {
        ...
        if (this.getResponder().isKing()) {
            ...
            if (k != null) {
                ...
                buf.append("text{text='The duelling ring is an area where your people can fight and kill eachother without harm to their reputation.'};text{text=''}");
                ...
                buf.append("text{text='In case you leave the proximity or die within the proximity during this time you may be removed from the throne of " + Kingdoms.getNameFor(this.getResponder().getKingdomId()) + ".'}");
                buf.append("text{text=''}");
                buf.append("text{text='You may be challenged once per week and must respond within one week or you are considered to have declined. You may decline these challenges two times but the third time you are strongly adviced to accept. In case you accept the challenge the first week nobody may challenge you for two more weeks.'}");
                buf.append("text{text=''}");
                buf.append("text{text='If you fail to respond to or decline the third challenge you may also be removed from the throne if enough people vote at the duelling ring.'}");
                buf.append("text{text=''}");
                if (k.hasFailedToRespondToChallenge()) {
                    ...
                    long timeLeft = 604800000L + k.getChallengeDate() - System.currentTimeMillis();
                    String tl = Server.getTimeFor(timeLeft);
                    buf.append("text{text=\"Unless you answer this challenge within " + tl + " you will have declined " + (k.getDeclinedChallengesNumber() + 1) + " challenges.\"}");
