...
extends Question {
    ...
    public void answer(Properties aAnswers) {
        ...
        for (int x = 0; x < this.permitted.length; ++x) {
            ...
            if (val2 == null || !val2.equals("true")) continue;
            ++removed;
        ...
        if ((val = aAnswers.getProperty(key = "vperm")) != null && val.equals("true")) {
        ...
        if ((val = aAnswers.getProperty(key = "allperm")) != null && val.equals("true")) {
            if (!md.isAllowAll()) {
        ...
        if ((val = aAnswers.getProperty(key = "newperm")) != null && val.length() > 0) {
            if (md.getPermissionsPlayerList().size() < md.getMaxAllowed()) {
                long mdid = Players.getInstance().getWurmIdByPlayerName(val);
                if (mdid > 0L) {
                    kingdom = Players.getInstance().getKingdomForPlayer(mdid);
        ...
        if ((val = aAnswers.getProperty(key = "newcont")) != null && val.length() > 0) {
            long mdid = Players.getInstance().getWurmIdByPlayerName(val);
            if (mdid > 0L) {
                kingdom = Players.getInstance().getKingdomForPlayer(mdid);
                if (kingdom == this.getResponder().getKingdomId()) {
        ...
        if ((val = aAnswers.getProperty(key = "newname")) != null) {
            boolean bl = changed = md.setObjectName(val, this.getResponder()) || changed;
        }
        if (changed) {
            try {
                md.save();
