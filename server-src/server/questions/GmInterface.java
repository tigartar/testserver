...
implements TimeConstants {
    ...
    public void answer(Properties aAnswer) {
        ...
        if (this.type == 83 && (this.getResponder().getPower() >= 2 || this.getResponder().mayMute())) {
            ...
            if ((pname == null || pname.length() == 0) && (num = Integer.parseInt(pid = aAnswer.getProperty("ddname"))) > 0) {
            ...
            boolean bl = nameSpecified = pname != null && pname.length() > 0;
            if (nameSpecified) {
                ...
                if (this.getResponder().getPower() >= 2) {
                    ...
                    if (val != null && (summon = val.equals("true"))) {
                        if (pname.equalsIgnoreCase(this.getResponder().getName())) {
                            ...
                            if ((val = aAnswer.getProperty(key = "tiley")) != null && val.length() > 0) {
                                tiley = Integer.parseInt(val);
                            }
                            if ((val = aAnswer.getProperty(key = "surfaced")) != null && val.length() > 0) {
                                layer = (byte)(val.equals("true") ? 0 : -1);
                            }
                            QuestionParser.summon(pname, this.getResponder(), tilex, tiley, layer);
                        }
                    }
                    if ((val = aAnswer.getProperty(key = "locate")) != null && (locate = val.equals("true"))) {
            ...
            if (!this.doneSomething && this.getResponder().getPower() >= 2) {
                boolean gmtool;
                String key = "gmtool";
                val = aAnswer.getProperty("gmtool");
                if (val != null && (gmtool = val.equals("true"))) {
                    ...
                    if (optCount != 1) {
                        ...
                        this.doneSomething = true;
                        GmTool gt = new GmTool(this.getResponder(), toolType, toolSubType, toolWurmId, toolSearch, "", 50, 0);
                        gt.sendQuestion();
                    }
                }
    ...
    private void checkMute(String pname) {
        ...
        if (mutereasontb != null && mutereasontb.length() > 2) {
            ...
            try {
                ...
                switch (reason) {
                    case 0: {
                        muteReason = muteReasonOne;
                        break;
                    }
                    case 1: {
                        muteReason = muteReasonTwo;
                        break;
                    }
                    case 2: {
                        muteReason = muteReasonThree;
                        break;
                    }
                    case 3: {
                        muteReason = muteReasonFour;
                        break;
                    }
                    case 4: {
                        muteReason = muteReasonFive;
                        break;
                    }
                    case 5: {
                        muteReason = muteReasonSix;
                        break;
                    }
                    case 6: {
                        muteReason = muteReasonSeven;
                        break;
                    }
                    default: {
                ...
                logger.log(Level.WARNING, "Problem parsing the mute reason: " + muteReason + ". Responder: " + this.getResponder() + " due to " + nfe, nfe);
            }
        }
        String key = "mutewarn";
    ...
    private void checkBans(String pname) {
        ...
        if (val != null && val.equals("true") && expiry > System.currentTimeMillis()) {
            ...
            try {
                ...
                if (val != null && val.equals("true")) {
                    bannedip = true;
                    String bip = this.playerInfo.getIpaddress();
                    this.getResponder().getCommunicator().sendSafeServerMessage("You ban and kick " + pname + ". The server won't accept connections from " + bip + " anymore.", (byte)1);
                    ...
                    if (Servers.localServer.LOGINSERVER) {
                        ...
                        try {
                            ...
                            logger.log(Level.INFO, this.getResponder().getName() + " banning ip on login server failed: " + e.getMessage(), e);
                        }
                    }
                } else {
                ...
                if (!Servers.localServer.LOGINSERVER) {
                    try {
                        ...
                        this.getResponder().getCommunicator().sendAlertServerMessage("Failed to ban on login server:" + e.getMessage());
                        logger.log(Level.INFO, this.getResponder().getName() + " banning " + pname + " on login server failed: " + e.getMessage(), e);
                    }
                }
            }
            catch (IOException e) {
                this.getResponder().getCommunicator().sendAlertServerMessage("Failed to ban on local server:" + e.getMessage());
                logger.log(Level.INFO, this.getResponder().getName() + " banning " + pname + " on local server failed: " + e.getMessage(), e);
            }
        }
        key = "banip";
        val = this.getAnswer().getProperty(key);
        ...
        if (val != null && val.equals("true")) {
            ...
            try {
                ...
                logger.log(Level.INFO, this.getResponder().getName() + " fails to reset warnings for " + pname + '.', iox);
            }
        }
        key = "pardonip";
        val = this.getAnswer().getProperty(key);
        if (val != null && val.length() > 0) {
            try {
                    ...
                    if (bip != null) {
                        if (Players.getInstance().removeBan(ip)) {
                            ...
                            try {
                                ...
                                this.getResponder().getCommunicator().sendAlertServerMessage("Failed to remove ip ban on login server:" + e.getMessage());
                                logger.log(Level.INFO, this.getResponder().getName() + " removing ip ban " + bip + " on login server failed: " + e.getMessage(), e);
                            }
                        } else {
                ...
                logger.log(Level.WARNING, "Problem parsing the pardonip value: " + val + ". Responder: " + this.getResponder() + " due to " + nfe, nfe);
            }
        }
    }
    ...
    private final void logMgmt(String logString) {
        ...
        Message mess = new Message(this.getResponder(), 9, "MGMT", "<" + this.getResponder().getName() + "> " + logString);
    ...
    private final void log(String logString) {
        ...
        Message mess = new Message(this.getResponder(), 11, "GM", "<" + this.getResponder().getName() + "> " + logString);
        Server.getInstance().addMessage(mess);
    }
    ...
    public void sendQuestion() {
        ...
        if (this.getResponder().getPower() >= 2 || this.getResponder().mayMute()) {
            ...
            for (int x = 0; x < players.length; ++x) {
                if (((Creature)players[x]).getWurmId() == this.getResponder().getWurmId() || (long)((Player)players[x]).getPower() > this.getResponder().getWurmId() || this.getResponder().getPower() < 2 && (this.getResponder().getKingdomId() != ((Creature)players[x]).getKingdomId() || !((Player)players[x]).isActiveInChat())) continue;
                buf.append(",");
                this.playlist.add((Player)players[x]);
                buf.append(((Creature)players[x]).getName());
                if (((Player)players[x]).isMute()) {
                    buf.append(" (m)");
                }
                if (this.getResponder().getPower() >= 2 && ((Player)players[x]).getWarnings() > 0) {
                    buf.append(" - ");
                    buf.append(((Player)players[x]).getWarnings());
                }
                if (this.getResponder().getPower() < 2 || !((Player)players[x]).isActiveInChat()) continue;
                chattingBuild.append(((Creature)players[x]).getName() + " ");
            }
            ...
            buf.append(muteReasonOne);
            buf.append(", ");
            buf.append(muteReasonTwo);
            buf.append(", ");
            buf.append(muteReasonThree);
            buf.append(", ");
            buf.append(muteReasonFour);
            buf.append(", ");
            buf.append(muteReasonFive);
            buf.append(", ");
            buf.append(muteReasonSix);
            buf.append(", ");
            buf.append(muteReasonSeven);
            buf.append("\"}");
