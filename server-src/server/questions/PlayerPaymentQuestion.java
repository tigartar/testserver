                    ...
                    if (purchaseFirstTime && referredBy == 0L) {
                        try {
                            if (this.getResponder().chargeMoney(20000L)) {
                                ...
                                lsw.addPlayingTime(this.getResponder(), this.getResponder().getName(), 0, 30, "firstBuy" + (System.currentTimeMillis() - 1400000000000L) + Servers.localServer.name);
                                this.getResponder().getCommunicator().sendSafeServerMessage("Your request for playing time is being processed. It may take up to half an hour until the system is fully updated.");
                        ...
                        this.getResponder().getCommunicator().sendNormalServerMessage("You have already purchased this option once, if you still have not received your play time after 30 minutes, please contact /support.");
                    } else {
                    ...
                    if (money >= 100000L) {
                        try {
                            if (this.getResponder().chargeMoney(100000L)) {
                                ...
                                logger.log(Level.INFO, this.getResponder().getName() + " purchased 1 month premium time for " + 10L + " silver coins. " + 30000L + " iron added to king.");
                                break block25;
                            }
                        ...
                        this.getResponder().getCommunicator().sendNormalServerMessage("You need at least 10 silver in your account to purchase 30 days of premium game time.");
                    }
                    ...
                    if (money >= 50000L) {
                        try {
                            if (this.getResponder().chargeMoney(50000L)) {
                                ...
                                logger.log(Level.INFO, this.getResponder().getName() + " purchased 1 month premium time for " + 5L + " silver coins. " + -20000L + " iron added to king.");
                                break block25;
                            }
                            this.getResponder().getCommunicator().sendAlertServerMessage("Failed to charge you 10 silvers. Please try later.");
                        }
                        ...
                        this.getResponder().getCommunicator().sendNormalServerMessage("You need at least 5 silver in your account to purchase 15 days of premium game time.");
                    }
                } else {
                    this.getResponder().getCommunicator().sendNormalServerMessage("You decide not to buy any premium game time for now.");
                }
