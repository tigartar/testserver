...
public class InGameVoteSetupQuestion extends Question {
            ...
            switch(this.part) {
               case 0:
                  ...
                  if (this.questionId == 0) {
                     InGameVoteSetupQuestion igvsq = new InGameVoteSetupQuestion(this.getResponder(), "In game voting setup", "Add New Question", (byte)1);
                     igvsq.sendQuestion();
                  } else {
                     byte nextPart = 0;
               ...
               case 2:
                  ...
                  if (fail.length() > 0) {
                     InGameVoteSetupQuestion igvsq = new InGameVoteSetupQuestion(
                        this.getResponder(), this.title, "Error: " + fail, this.part, this.questionId, fail
                     );
                     igvsq.setData(
                        this.questionId,
                        this.qtitle,
                     ...
                     List<VoteServer> servers = new ArrayList<>();
   ...
   public void sendQuestion() {
      ...
      if (this.getResponder().getPower() >= 2) {
         try {
               ...
               case 0:
                  ...
                  buf.append(
                     "table{rows=\""
                        ...
                        + "\";cols=\""
                        + (4 + servers.length)
                        + "\";label{text=\"\"};label{type=\"bold\";text=\"Question\"};label{type=\"bold\";text=\"Vote Start\"};label{type=\"bold\";text=\"Vote End\"};"
                  );
                  ...
                  for(int x = 0; x < vqs.length; ++x) {
                     ...
                     buf.append(
                        "radio{group=\"qid\";id=\""
                           + vqs[x].getQuestionId()
                           + "\"};label{text=\""
                           + vqs[x].getQuestionTitle()
                           + "\""
                           + colour
                           + "};label{text=\""
                           + Tickets.convertTime(vqs[x].getVoteStart())
                           + "\""
                           + colour
                           + "};label{text=\""
                           + Tickets.convertTime(vqs[x].getVoteEnd() - 1000L)
                           + "\""
                           + colour
                           + "};"
                     );
            ...
            this.getResponder().getCommunicator().sendNormalServerMessage("Exception:" + var14.getMessage());
         }
      }
   }
   ...
   private String showQuestion() {
      ...
      buf.append(
         "harray{label{text=\"Option 1 [\"};label{color=\"100,100,255\";text=\"Mandatory\"};label{text=\"]:\"};input{id=\"opt1\";maxchars=\"50\";text=\""
            + this.opt1
            + "\""
            + disabled
            + "}}"
      ...
      buf.append(
         "harray{label{text=\"Option 2 [\"};label{color=\"100,100,255\";text=\"Mandatory\"};label{text=\"]:\"};input{id=\"opt2\";maxchars=\"50\";text=\""
            + this.opt2
            + "\""
            + disabled
            + "}}"
      ...
      buf.append(
         "harray{label{text=\"Multiple answers allowed?\"};checkbox{id=\"ma\";selected=\""
            + this.ma
            + "\""
            + disabled
            + "};label{text=\"   Premium Only?\"};checkbox{id=\"po\";selected=\""
            + this.po
            + "\""
            + disabled
            + "}}"
      ...
      buf.append(
         "harray{label{type=\"bold\";text=\"By Kingdoms: \"}label{text=\"JK?\"};checkbox{id=\"jk\";selected=\""
            + this.jk
            + "\""
            + disabled
            + "}label{text=\"MR?\"};checkbox{id=\"mr\";selected=\""
            + this.mr
            + "\""
            + disabled
            + "}label{text=\"HOTS?\"};checkbox{id=\"hots\";selected=\""
            + this.hots
            + "\""
            + disabled
            + "}label{text=\"Freedom?\"};checkbox{id=\"freedom\";selected=\""
            + this.freedom
            + "\""
            + disabled
            + "}}"
      ...
      for(int x = 0; x < entries.length; ++x) {
         ...
         if (entries[x].isAvailable(this.getResponder().getPower(), this.getResponder().isPaying())) {
            buf.append(
               "harray{label{color=\"100,255,100\";text=\""
                  + entries[x].getName()
                  + "?\"}checkbox{id=\"s"
                  + entries[x].getId()
                  + "\""
            ...
            buf.append(
               "harray{label{color=\"255,100,100\";text=\""
                  + entries[x].getName()
                  + "?\"}checkbox{id=\"s"
                  + entries[x].getId()
                  + "\""
      ...
      buf.append(
         "harray{label{text=\"Start:\"};input{id=\"day\";maxchars=\"2\";text=\""
            + this.startDay
            + "\""
            + disabled
            + "};label{text=\"/\"};"
            + strMonth
            + ";label{text=\"/\"};input{id=\"year\";maxchars=\"4\";text=\""
            + this.startYear
            + "\""
            + disabled
            + "};label{text=\" \"};"
   ...
   private static long daysBetween(Calendar startDate, Calendar endDate) {
      ...
      long daysBetween;
      for(daysBetween = 0L; date.before(endDate); ++daysBetween) {
         date.add(5, 1);
      }
