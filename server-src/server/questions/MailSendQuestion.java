...
public final class MailSendQuestion extends Question implements MonetaryConstants {
   ...
   public void answer(Properties answers) {
      ...
      if (!validateMailboxContents(this.items, this.mailbox)) {
         ...
         if (nosend != null && nosend.equals("true")) {
            ...
            if (name == null || name.length() <= 2) {
            ...
            } else if (LoginHandler.containsIllegalCharacters(name)) {
               ...
               long[] var10000 = new long[]{(long)Servers.localServer.id, pinf.wurmId};

               long[] info;
               try {
                  pinf.load();
                  var10000 = new long[]{(long)Servers.localServer.id, pinf.wurmId};
                  ...
                  if (kingdom != this.getResponder().getKingdomId()) {
                     ...
                     this.getResponder()
                        .getCommunicator()
                        .sendNormalServerMessage(pinf.getName() + " is with the " + kname + ". You may not trade with the enemy.");
                     return;
                  }
               ...
               if (info[1] > 0L && (int)info[0] > 0) {
                  ...
                  for(int x = 0; x < this.items.length; ++x) {
                     ...
                     if (this.items[x].getTemplateId() == 651) {
                        this.getResponder()
                           .getCommunicator()
                           .sendNormalServerMessage("The spirits refuse to touch the " + this.items[x].getName() + " since they don't know what's in it.");
                        return;
                     }
                  ...
                  MailSendConfirmQuestion msc = new MailSendConfirmQuestion(
                     this.getResponder(),
                     "Confirm the price",
   ...
   protected final boolean isContainerMailItemOk(Item item, Item container) {
      if (!item.isArtifact() && !item.isRoyal()) {
         ...
         this.getResponder()
            .getCommunicator()
            .sendAlertServerMessage("You can not mail the " + item.getName() + " (in the " + container.getName() + "). The spirits refuse to deal with it.");
         return false;
      }
   }
   ...
   public void sendQuestion() {
      ...
      buf.append(
         "text{type='bold';text='Use mailboxes with caution and inexpensive items until you rely on the spirits. They are still learning and take no responsibility for lost items.'};"
      );
      ...
      buf.append(
         "text{text='The costs are shown on the next screen. It is safe to click send here, but if you click send on the next screen, the goods will be sent.'};"
      );
