...
implements ItemMaterials {
    ...
    public void sendQuestion() {
        ...
        buf.append("table{rows=\"1\";cols=\"5\";");
        buf.append("label{text=\"Target:\"}input{id=\"tx\"; maxchars=\"6\";text=\"" + faid.getTargetPosX() + "\"}label{text=\",\"}input{id=\"ty\"; maxchars=\"6\";text=\"" + faid.getTargetPosY() + "\"}");
        ...
        buf.append("label{type=\"bold\";text=\"Body Strength:\"};label{text=\"" + faid.getBodyStrength() + "\"};label{text=\"" + fd.getBodyStrength() + "\"};label{type=\"bold\";text=\"Body Stamina:\"};label{text=\"" + faid.getBodyStamina() + "\"};label{text=\"" + fd.getBodyStamina() + "\"}");
        buf.append("label{type=\"bold\";text=\"Body Control:\"};label{text=\"" + faid.getBodyControl() + "\"};label{text=\"" + fd.getBodyControl() + "\"};label{type=\"bold\";text=\"Mind Speed:\"};label{text=\"" + faid.getMindSpeed() + "\"};label{text=\"" + fd.getMindSpeed() + "\"}");
        buf.append("}");
        buf.append("label{type=\"bolditalic\";text=\"Following cannot be changed, Fish Data from fish type id. \"}");
        buf.append("table{rows=\"1\";cols=\"6\";");
        buf.append("label{type=\"bold\";text=\"Name:\"};label{text=\"" + fd.getName() + "\"}label{text=\"\"};label{text=\"\"}label{type=\"bold\";text=\"Special:\"};label{" + this.showBoolean(fd.isSpecialFish()) + "};");
        buf.append("label{type=\"bold\";text=\"Surface:\"};label{" + this.showBoolean(fd.onSurface()) + "};label{type=\"bold\";text=\"Water:\"};label{" + this.showBoolean(fd.inWater()) + "}label{type=\"bold\";text=\"Pond:\"};label{" + this.showBoolean(fd.inPond()) + "};");
        buf.append("label{type=\"bold\";text=\"Lake:\"};label{" + this.showBoolean(fd.inLake()) + "}label{type=\"bold\";text=\"Sea:\"};label{" + this.showBoolean(fd.inSea()) + "};label{type=\"bold\";text=\"Shallows:\"};label{" + this.showBoolean(fd.inShallows()) + "}");
        buf.append("label{type=\"bold\";text=\"Min depth:\"};label{text=\"" + fd.getMinDepth() + "\"}label{type=\"bold\";text=\"Max depth:\"};label{text=\"" + fd.getMaxDepth() + "\"};label{type=\"bold\";text=\"Keeper weight:\"};label{text=\"" + fd.getMinWeight() + "\"}");
        buf.append("label{type=\"bold\";text=\"Use Pole:\"};label{" + this.showBoolean(fd.useFishingPole()) + "}label{type=\"bold\";text=\"Use Net:\"};label{" + this.showBoolean(fd.useFishingNet()) + "};label{type=\"bold\";text=\"Use Spear:\"};label{" + this.showBoolean(fd.useSpear()) + "}");
        buf.append("label{type=\"bold\";text=\"Use Basic Rod:\"};label{" + this.showBoolean(fd.useReelBasic()) + "}label{type=\"bold\";text=\"Use Fine Rod:\"};label{" + this.showBoolean(fd.useReelFine()) + "};label{type=\"bold\";text=\"Use Deep Water Rod:\"};label{" + this.showBoolean(fd.useReelWater()) + "}");
        buf.append("label{type=\"bold\";text=\"Use Professional Rod:\"};label{" + this.showBoolean(fd.useReelProfessional()) + "}label{type=\"bold\";text=\"\"};label{text=\"\"};label{type=\"bold\";text=\"\"};label{text=\"\"}");
        buf.append("}");
