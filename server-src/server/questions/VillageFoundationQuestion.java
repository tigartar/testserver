...
public final class VillageFoundationQuestion extends Question implements VillageStatus, ItemTypes, MonetaryConstants, CounterTypes {
   ...
   private void createIntro() {
      VillageFoundationQuestion vf = new VillageFoundationQuestion(
         this.getResponder(), "Settlement Application Form", "Welcome to the Settlement Application Form", this.target
      );
      if (vf != null) {
         this.copyValues(vf);
         vf.sequence = 0;
   ...
   private void createQuestion1() {
      VillageFoundationQuestion vf = new VillageFoundationQuestion(
         this.getResponder(), "Settlement Size", "Stage One - The size of your settlement", this.target
      );
      if (vf != null) {
         this.copyValues(vf);
         vf.sequence = 1;
   ...
   private void createQuestion2() {
      VillageFoundationQuestion vfq = new VillageFoundationQuestion(
         this.getResponder(),
         "The Deed Perimeter",
         "Congratulations! You have chosen a deed that will be " + this.diameterX + " tiles by " + this.diameterY + " tiles.",
         this.target
      );
      if (vfq != null) {
         this.copyValues(vfq);
   ...
   private void createQuestion4() {
      VillageFoundationQuestion vfq = new VillageFoundationQuestion(
         this.getResponder(), "Guards", "Congratulations! You have reserved the name '" + this.villageName + "'", this.target
      );
      if (vfq != null) {
         this.copyValues(vfq);
         vfq.sequence = 4;
   ...
   private void createQuestion5() {
      VillageFoundationQuestion vfq = new VillageFoundationQuestion(
         this.getResponder(), "Check your settings", "Check your settings and Found your settlement!", this.target
      );
      if (vfq != null) {
         this.copyValues(vfq);
         vfq.sequence = 5;
               ...
               for(int y = ya; y <= ye; ++y) {
                  ...
                  if (create) {
                     try {
                        ...
                        i.setPosXYZ(
                           (float)((x << 2) + 2), (float)((y << 2) + 2), Zones.calculateHeight((float)((x << 2) + 2), (float)((y << 2) + 2), true) + 5.0F
                        );
               ...
               if (v.mayChangeName()) {
                  if (this.villageName.length() >= 41) {
                     ...
                     this.error = this.error
                        + " * The name of the settlement would be ''"
                        + this.villageName
   ...
   private long getFoundingCharge() {
      return Servers.localServer.isFreeDeeds() ? 0L : this.getFoundingCost() + 30000L - (this.deed.getTemplateId() == 862 ? 0L : 100000L);
   }
      ...
      if (val != null && val.equals("true")) {
         ...
         try {
            if (!this.expanding) {
               ...
               long left = this.deed.getTemplateId() == 862 ? 30000L : 0L;
               if (toCharge < 0L) {
               ...
               if (charge && !((Player)this.getResponder()).chargeMoney(toCharge)) {
                  ...
                  this.getResponder()
                     .getCommunicator()
                     .sendAlertServerMessage("You do not have the required " + funds.getChangeString() + " available in your bank account.");
                  this.removeSettlementMarkers();
                  this.removePerimeterMarkers();
                  return false;
               }
               ...
               try {
                  Village v = Villages.createVillage(
                     Zones.safeTileX(this.tokenx - this.selectedWest),
                     Zones.safeTileX(this.tokenx + this.selectedEast),
                     Zones.safeTileY(this.tokeny - this.selectedNorth),
                     Zones.safeTileY(this.tokeny + this.selectedSouth),
                     this.tokenx,
                     this.tokeny,
                     this.villageName,
                     this.getResponder(),
                  ...
                  Server.getInstance()
                     .broadCastSafe("The settlement of " + this.villageName + " has just been founded by " + this.getResponder().getName() + ".");
                  this.getResponder()
                     .getCommunicator()
                     .sendSafeServerMessage("The settlement of " + this.villageName + " has been founded according to your specifications!");
                  ...
                  logger.log(Level.WARNING, "Failed to create settlement: " + var14.getMessage(), (Throwable)var14);
                  this.getResponder()
                     .getCommunicator()
                     .sendNormalServerMessage("Failed to locate a resource needed for that request. Please contact administration.");
                  return false;
               } catch (NoSuchCreatureException var15) {
                  logger.log(Level.WARNING, "Failed to create settlement: " + var15.getMessage(), (Throwable)var15);
                  this.getResponder()
                     .getCommunicator()
                     .sendNormalServerMessage("Failed to locate a resource needed for that request. Please contact administration.");
                  return false;
               } catch (IOException var16) {
                  logger.log(Level.WARNING, "Failed to create settlement:" + var16.getMessage(), (Throwable)var16);
                  this.getResponder()
                     .getCommunicator()
                     .sendNormalServerMessage("Failed to locate a resource needed for that request. Please contact administration.");
                  return false;
               } catch (NoSuchRoleException var17) {
                  logger.log(Level.WARNING, "Failed to create settlement:" + var17.getMessage(), (Throwable)var17);
                  this.getResponder()
                     .getCommunicator()
                     .sendNormalServerMessage("Failed to locate a role needed for that request. Please contact administration.");
                  return false;
               } catch (NoSuchItemException var18) {
                  logger.log(Level.WARNING, "Failed to create settlement:" + var18.getMessage(), (Throwable)var18);
               ...
               if (!this.permanent && moneyNeeded > 0L) {
                  long rest = getExpandMoneyNeededFromBank(moneyNeeded, oldvill);
                  if (rest > 0L) {
                     if (Servers.localServer.testServer) {
                        this.getResponder()
                           .getCommunicator()
                           .sendNormalServerMessage("We need " + moneyNeeded + ". " + rest + " must be taken from the bank.");
                     }
                     ...
                     if (Servers.localServer.testServer) {
                        this.getResponder()
                           .getCommunicator()
                           .sendNormalServerMessage("We charge " + moneyNeeded + " from the plan which has " + oldvill.plan.moneyLeft);
                     }
                  ...
                  if (!lsw.addMoney(this.getResponder().getWurmId(), this.getResponder().getName(), Math.abs(moneyNeeded), "Resize " + oldvill.getName())) {
                     change = false;
                     logger.log(
                        Level.INFO,
               ...
               if (change) {
                  ...
                  if (oldvill.getStartX() != this.tokenx - this.selectedWest
                     || oldvill.getStartY() != this.tokeny - this.selectedNorth
                     || oldvill.getEndX() != this.tokenx + this.selectedEast
                     || oldvill.getEndY() != this.tokeny + this.selectedSouth) {
                     ...
                     oldvill.setNewBounds(
                        Zones.safeTileX(oldvill.getTokenX() - this.selectedWest),
                        Zones.safeTileY(oldvill.getTokenY() - this.selectedNorth),
                        Zones.safeTileX(oldvill.getTokenX() + this.selectedEast),
                        Zones.safeTileY(oldvill.getTokenY() + this.selectedSouth)
                     );
                  }
            ...
            logger.log(Level.INFO, "Failed to create settlement:" + var20.getMessage(), (Throwable)var20);
   ...
   private boolean checkSize() {
      if (this.getResponder().getPower() < 3) {
         if (Features.Feature.TOWER_CHAINING.isEnabled()) {
            ...
            for(Item marker : chain.getChainMarkers()) {
               if ((marker.isChained() || this.getResponder().getKingdomId() == 4)
                  && marker.isGuardTower()
                  && Math.abs(this.tokenx - marker.getTileX()) <= 50
                  && Math.abs(this.tokeny - marker.getTileY()) <= 50) {
                  found = true;
                  break;
               }
            }
            ...
            if (!found) {
               this.getResponder()
                  .getCommunicator()
                  .sendSafeServerMessage("You must found the settlement within 50 tiles of an allied tower linked to the kingdom influence chain.");
               return false;
            }
         } else {
            boolean found = false;
      ...
      Map<Village, String> decliners = Villages.canFoundVillage(
         this.selectedWest,
         this.selectedEast,
         this.selectedNorth,
         this.selectedSouth,
      ...
      if (!decliners.isEmpty()) {
         ...
         if (checkFocusZones) {
            String focusZoneReject = Villages.isFocusZoneBlocking(
               this.selectedWest, this.selectedEast, this.selectedNorth, this.selectedSouth, this.tokenx, this.tokeny, this.initialPerimeter, true
            );
   ...
   public boolean checkDeedItem() {
      try {
         ...
         logger.log(Level.WARNING, "Failed to locate settlement deed with id " + this.target, (Throwable)var2);
   ...
   private boolean checkBlockingCreatures() {
      ...
      Object cret = Villages.isAggOnDeed(
         currVill, this.getResponder(), this.selectedWest, this.selectedEast, this.selectedNorth, this.selectedSouth, this.tokenx, this.tokeny, true
      );
      if (cret != null) {
         this.getResponder()
            .getCommunicator()
            .sendSafeServerMessage("You cannot found the settlement here, since there are dangerous aggressive creatures or dens in the area.");
         return false;
      } else {
         return true;
      }
   ...
   private boolean checkBlockingKingdoms() {
      if (this.getResponder().getPower() < 2) {
         ...
         return Zones.isKingdomBlocking(
            this.tokenx - this.selectedWest - 5 - this.initialPerimeter - mindist,
            this.tokeny - this.selectedNorth - 5 - this.initialPerimeter - mindist,
            this.tokenx + this.selectedEast + 5 + this.initialPerimeter + mindist,
            this.tokeny + this.selectedSouth + 5 + this.initialPerimeter + mindist,
            this.getResponder().getKingdomId(),
            eZoneStartX,
            eZoneStartY,
            eZoneEndX,
            eZoneEndY
   ...
   private boolean checkBlockingItems() {
      if (this.getResponder().getPower() < 2) {
         ...
         int maxsouth = Math.min(
            Zones.worldTileSizeY, this.tokeny + this.selectedSouth + (Servers.localServer.isChallengeServer() ? 20 : Kingdoms.minKingdomDist)
         );
         int maxwest = Math.max(0, this.tokenx - this.selectedWest - (Servers.localServer.isChallengeServer() ? 20 : Kingdoms.minKingdomDist));
         int maxeast = Math.min(
            Zones.worldTileSizeX, this.tokenx + this.selectedEast + (Servers.localServer.isChallengeServer() ? 20 : Kingdoms.minKingdomDist)
         );
         ...
         if (Zones.isWithinDuelRing(
            this.tokenx - this.selectedWest - 5 - this.initialPerimeter,
            this.tokeny - this.selectedNorth - 5 - this.initialPerimeter,
            this.tokenx + this.selectedEast + 5 + this.initialPerimeter,
            this.tokeny + this.selectedSouth + 5 + this.initialPerimeter
         )) {
   ...
   private boolean checkStructuresInArea() {
      if (this.getResponder().getPower() <= 1) {
         ...
         Structure[] surfStructures = Zones.getStructuresInArea(
            this.tokenx - this.selectedWest - 2,
            this.tokeny - this.selectedNorth - 2,
            this.tokenx + this.selectedEast + 2,
            this.tokeny + this.selectedSouth + 2,
            true
         );
         Structure[] caveStructures = Zones.getStructuresInArea(
            this.tokenx - this.selectedWest - 2,
            this.tokeny - this.selectedNorth - 2,
            this.tokenx + this.selectedEast + 2,
            this.tokeny + this.selectedSouth + 2,
         ...
         if (newStructures.size() > 0) {
            ...
            for(Structure lStructure : newStructures) {
               ...
               if (!ok && lStructure.isTypeHouse() && !lStructure.mayManage(this.getResponder())) {
                  this.getResponder()
                     .getCommunicator()
                     .sendSafeServerMessage("You need to have manage permissions for the structure " + lStructure.getName() + " to found the settlement here.");
                  return false;
               }
            }
         }
   ...
   private void addTileCost(StringBuilder buf) {
      ...
      if (!this.expanding) {
         ...
         try {
            ...
            if (Servers.localServer.isUpkeep()) {
               buf.append(
                  "text{text=\"The new monthly upkeep cost for the tiles will be "
                     + new Change((long)this.tiles * Villages.TILE_UPKEEP).getChangeString()
                     + ".\"}"
               );
            }
   ...
   private void addPerimeterCost(StringBuilder buf) {
      if (!this.expanding) {
         buf.append(
            "text{text=\"You have selected a perimeter of 5"
               + (Servers.localServer.isFreeDeeds() ? "" : " free")
               + " tiles plus "
               + this.initialPerimeter
         ...
         if (this.initialPerimeter > 0) {
            if (!Servers.localServer.isFreeDeeds()) {
               buf.append(
                  "text{text=\"The initial cost for the perimeter tiles will be "
                     + new Change((long)this.perimeterTiles * Villages.PERIMETER_COST).getChangeString()
                     + ".\"}"
               );
            }
            ...
            if (Servers.localServer.isUpkeep()) {
               buf.append(
                  "text{text=\"The monthly upkeep cost for the perimeter tiles will be "
                     + new Change((long)this.perimeterTiles * Villages.PERIMETER_UPKEEP).getChangeString()
                     + ".\"}"
               );
            }
         }
         ...
         try {
            ...
            if (diff > 0 && !Servers.localServer.isFreeDeeds()) {
               buf.append(
                  "text{text=\"The additional cost for the extra perimeter tiles will be "
                     + new Change((long)diff * Villages.PERIMETER_COST).getChangeString()
                     + ".\"}"
               );
            }
            ...
            if (Servers.localServer.isUpkeep()) {
               if (this.initialPerimeter > 0) {
                  buf.append(
                     "text{text=\"The new monthly upkeep cost for the perimeter tiles will be "
                        + new Change((long)this.perimeterTiles * Villages.PERIMETER_UPKEEP).getChangeString()
                        + ".\"}"
                  );
               } else if (diff < 0) {
      ...
      if (this.expanding) {
         try {
            ...
            for(Citizen lCitizen : citizens) {
               if (WurmId.getType(lCitizen.wurmId) == 0) {
                  ++curr;
               }
            }
      ...
      if (!this.expanding) {
         if (this.selectedGuards > 0) {
            if (Servers.localServer.isFreeDeeds()) {
               ...
               buf.append(
                  "text{text=\"You will hire "
                     + this.selectedGuards
                     + " guards for a cost of "
                     + new Change((long)this.selectedGuards * Villages.GUARD_COST).getChangeString()
                     + ".\"}"
               );
            }
         ...
         try {
            ...
            if (diff > 0) {
               if (Servers.localServer.isFreeDeeds()) {
                  ...
                  buf.append(
                     "text{text=\"You will hire "
                        ...
                        + new Change((long)diff * Villages.GUARD_COST).getChangeString()
                        + ".\"}"
                  );
               }
               ...
               if (Servers.localServer.isUpkeep()) {
                  buf.append(
                     "text{text=\"The new guard upkeep will be " + new Change(GuardPlan.getCostForGuards(this.selectedGuards)).getChangeString() + ".\"}"
                  );
               }
            } else {
   ...
   public void sendIntro() {
      ...
      buf.append(
         "text{type=\"italic\";text=\"This form will take you through the process of "
            + fs
            + " your settlement. You will be asked how large you want your settlement to be, if you want to buy a larger perimeter, what you want to call your new settlement and how many guards you wish to hire. Please be aware that there is no refund for purchased tiles.\"}"
      );
      buf.append("text{text=\"\"}");
      if (Servers.localServer.isChallengeOrEpicServer()) {
         buf.append(
            "text{type=\"bold\";color=\"255,0,0\";text=\"Note that deeds in the Epic and Challenge cluster may become harmed by natural events. It is rare but may happen.\"}"
         );
      ...
      buf.append("text{text=\"\"}");
      buf.append(
         "text{type=\"italic\";text=\"Each stage of the process is on its own page. This is so we can make some checks as we go along and also give you a running commentary as to cost.\"}"
      );
   ...
   public void sendQuestion() {
      ...
      if (!this.checkDeedItem()) {
         ...
         buf.append(
            "text{text=\"Please enter in the boxes below the distances in tiles between"
               + from
               + "and the border of your settlement. Example: 5, 5, 6, 6 will create a deed 11 tiles by 13 tiles. Minimum is 5.\"}"
         );
         buf.append("text{text=\"\"}");
         buf.append("text{text=\"");
         if (!Servers.localServer.isFreeDeeds()) {
      ...
      buf.append("text{text=\"\"}");
      buf.append(
         "harray {button{text=\"Survey Area\";id=\"submit\"}label{text=\" \";id=\"spacedlxg\"};button{text=\"Show Intro\";id=\"back\"};label{text=\" \";id=\"sacedlxg\"};button{text=\"Cancel\";id=\"cancel\"};};"
      );
      buf.append("text{text=\"\"}");
      buf.append("text{type=\"bold\";text=\"Help\"}");
      buf.append("text{text=\"\"}");
      buf.append(
         "text{type=\"italic\";text=\"This form will take you through the process of founding or resizing your settlement. You will be asked how large you want your settlement to be, if you want to buy a larger perimeter, what you want to call your new settlement and how many guards you wish to hire. You may wish to read the help articles on the Wiki for more information before you start.\"}"
      );
      buf.append("text{text=\"\"}");
      buf.append("text{type=\"bold\";text=\"What you need first:\"}");
      buf.append("text{text=\"\"}");
   ...
   public void sendQuestion2() {
      ...
      buf.append(
         "text{type=\"italic\";text=\"You may now move around and inspect the selected area while continuing with this form. The border has been marked for your convenience.\"}"
      );
      buf.append("text{text=\"\"}");
      ...
      buf.append("text{text=\"\"}");
      buf.append(
         "text{text=\"Please enter the number of tiles BEYOND the 5"
            + (Servers.localServer.isFreeDeeds() ? "" : " initial tiles that you get for free")
      ...
      buf.append("text{text=\"\"}");
      buf.append(
         "harray {button{text=\"Survey Area\";id=\"submit\"};label{text=\" \";id=\"spacedlxg\"};button{text=\"Back\";id=\"back\"};label{text=\" \";id=\"spacedlxg\"};button{text=\"Cancel\";id=\"cancel\"};};"
      );
      buf.append("text{text=\"\"}");
      buf.append("text{type=\"bold\";text=\"Help\"}");
      buf.append("text{text=\"\"}");
      buf.append(
         "text{type=\"italic\";text=\"The perimeter surrounding a settlement on all sides is purely to stop non-citizens from building or founding their own settlement - there are no other restrictions. You do not own and control the perimeter in the same way as the main settlement. On PvP servers kingdom guards will hunt enemies within the perimeter.\"}"
      );
      buf.append("text{text=\"\"}");
      ...
      if (Servers.localServer.isFreeDeeds() && !Servers.localServer.isUpkeep()) {
         ...
         buf.append(".\"}");
         buf.append(
            "text{type=\"italic\";text=\"There are no refunds for downsizing your deed. You will receive what is left in the upkeep fund if you later choose to disband.\"}"
         );
      }
   ...
   public void sendQuestion3() {
      StringBuilder buf = new StringBuilder(this.getBmlHeader());
      buf.append(
         "text{type=\"italic\";text=\"Your deed size will be "
            + this.diameterX
            + " by "
            + this.diameterY
            + " and the perimeter will extend beyond the border by "
            + (5 + this.initialPerimeter)
      ...
      if (this.expanding) {
         try {
            ...
            if (village.mayChangeName()) {
               ...
               buf.append(
                  "text{type=\"bold\";color=\"255,50,0\";text=\"NOTE: Changing name will"
      ...
      if (k != null && !k.isCustomKingdom() && this.getResponder().getPower() >= 3) {
         ...
         buf.append(
            "harray{label{text=\"Select a kingdom if this is the start town: \"};dropdown{id=\"kingdomid\";default=\""
               + this.spawnKingdom
               + "\";options=\"None,"
               + "Jenn-Kellon"
               + ","
      ...
      buf.append(
         "harray {button{text=\"Save this name\";id=\"submit\"};label{text=\" \";id=\"spacedlxg\"};button{text=\"Go Back\";id=\"back\"};label{text=\" \";id=\"spacedlxg\"};button{text=\"Cancel\";id=\"cancel\"};};"
      );
      buf.append("text{text=\"\"}");
      buf.append("text{type=\"bold\";text=\"Help\"}");
      buf.append(
         "text{type=\"italic\";text=\"You can enter the name and motto for your deed as well as mark this as a democracy. The citizens of a settlement are allowed to vote for a new mayor up to once every week. The challenger requires 51% of the vote to succeed if it is a democracy, otherwise it is not possible to change mayor.  In a democracy you cannot revoke the citizenship of a citizen. By default, Wurm settlements are Autocracies.\"}"
      );
      buf.append("text{text=\"\"}");
      buf.append("}};null;null;}");
      this.getResponder().getCommunicator().sendBml(600, 600, true, false, buf.toString(), 200, 200, 200, this.title);
   ...
   public void sendQuestion4() {
      ...
      if (Servers.localServer.isChallengeOrEpicServer() && !Servers.localServer.isFreeDeeds()) {
         buf.append(
            "text{text=\"The only guard type is heavy guards. The running upkeep cost increases the more guards you have in a sort of ladder system. The first guards are cheaper than the last.\"};"
         );
         ...
         buf.append(
            "text{text=\"The only guard type is heavy guards. There is no cost for hiring guards here"
               + (Servers.localServer.isUpkeep() ? ", but they have an upkeep of " + Villages.GUARD_UPKEEP_STRING + " per month" : ".")
               + "\"};"
         );
         buf.append("text{text=\"\"};");
      } else {
         buf.append(
            "text{text=\"The only guard type is heavy guards. The cost for hiring them is "
      ...
      buf.append("text{text=\"\"}");
      buf.append(
         "harray {button{text=\"Save the number of guards\";id=\"submit\"};label{text=\" \";id=\"spacedlxg\"};button{text=\"Go Back\";id=\"back\"};label{text=\" \";id=\"spacedlxg\"};button{text=\"Cancel\";id=\"cancel\"};};"
      );
      buf.append("text{text=\"\"}");
      buf.append("text{type=\"bold\";text=\"Help\"}");
      buf.append("text{text=\"\"}");
      buf.append(
         "text{type=\"italic\";text=\"Guards enforce the rules on deed and protect you against enemies. Guards can be hired at any time, but it is good to start your settlement with at least a few! "
      );
      if (!Servers.localServer.isFreeDeeds() && Servers.localServer.isUpkeep()) {
         buf.append(
   ...
   public void sendQuestion5() {
      ...
      buf.append(
         "text{type=\"italic\";text=\"Here are all your settings and the costs. If you are happy, click on "
            + fs
            + " Settlement. If you wish to change anything, use the Go Back button.\"}"
      );
      buf.append("text{text=\"\"}");
      buf.append("text{type=\"bold\";text=\"Settlement Size\"}");
      this.addTileCost(buf);
            ...
            if (!village.isKosAllowed() && village.getReputations().length <= 0) {
               if (this.willHaveHighay()) {
                  if (!village.isHighwayAllowed()) {
                     hasError = true;
                     buf.append(
                        "text{type=\"bold\";color=\"255,0,0\";text=\"Error: The new size covers a highway but highways have not been allowed, see settlment settings to change this option.\"}"
                     );
                  ...
                  buf.append(
                     "text{text=\"Note: You will not be able to use KOS as highways have been enabled for this settlment, see settlment settings to change this option.\"}"
                  );
               } else {
               ...
               hasError = true;
               buf.append(
                  "text{type=\"bold\";color=\"255,0,0\";text=\"Error: Cannot expand over a highway if KOS is enabled, see settlment settings to change this option.\"}"
               );
            } else {
      ...
      if (!hasError) {
         ...
         if (this.expanding) {
            ...
            if (resCost > 0L) {
               ...
               long rest = getExpandMoneyNeededFromBank(resCost, village);
               if (rest > 0L) {
      ...
      buf.append(
         "harray{"
            + (!hasError ? "button{text=\"" + fs + " Settlement\";id=\"submit\"};label{text=\" \"};" : "")
            + "button{text=\"Go Back\";id=\"back\"};label{text=\" \"};button{text=\"Cancel\";id=\"cancel\"};}}};null;null;}"
      );
   ...
   public void sendQuestion6() {
      ...
      buf.append("text{text=\"\"}");
      buf.append(
         "text{type=\"italic\";text=\"The deed for this settlement has been placed in your inventory, and if you have hired any guards, they will no doubt be here as soon as they can be.\"}"
      );
      buf.append("text{text=\"\"}");
      buf.append(
         "text{type=\"italic\";text=\"You can change the size of your deed and its perimeter if you wish, as well as hire and fire guards, using the tools attached to the deed or token.\"}"
      );
      buf.append("text{text=\"\"}");
