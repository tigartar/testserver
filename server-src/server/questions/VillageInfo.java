      ...
      if (this.playerRole != null) {
         ...
         try {
            ...
            if (village.getMayor() != null && village.getMayor().getId() == this.getResponder().getWurmId()) {
               try {
                  ...
                  if (deed.getOwnerId() < 0L) {
                     logger.log(
                        Level.INFO,
                        this.getResponder().getName() + " retrieving and inserting deed " + village.getDeedId() + " for " + village.getName() + "."
                     );
                  ...
                  logger.log(
                     Level.WARNING, "No deed available for " + village.getName() + ". Creating new. Exception was " + var12.getMessage(), (Throwable)var12
                  );
            ...
            if (village.isCitizen(this.getResponder()) || this.getResponder().getPower() >= 2) {
               ...
               buf.append(
                  "text{text=\"The perimeter is " + (5 + village.getPerimeterSize()) + " and it has " + village.plan.getNumHiredGuards() + " guards hired.\"}"
               );
               ...
               buf.append(
                  "text{text=\"War ("
                     + nf.format((double)village.getFaithWarValue())
                     + ") damage: "
                     + nf.format((double)village.getFaithWarBonus())
                     + "% CR: "
                     + nf.format((double)village.getFaithWarBonus())
                     + "%, Healing ("
                     + nf.format((double)village.getFaithHealValue())
                     + "): "
                     + nf.format((double)village.getFaithHealBonus())
                     + "%, Enchanting ("
                     + nf.format((double)village.getFaithCreateValue())
                     + "): "
                     + nf.format((double)village.getFaithCreateBonus())
                     + "%, Rarity window: "
                     + (int)Math.min(10.0F, village.getFaithCreateValue())
                     + " bonus seconds\"}"
               );
               ...
               int brandedCreatures = Creatures.getInstance().getBranded((long)village.getId()).length;
            ...
            if (village.getSkillModifier() == 0.0) {
               ...
               buf.append(
                  "text{text=\"This settlement has acquired knowledge that increases the productivity bonus of its citizens by "
                     + village.getSkillModifier()
                     + "%.\"}"
               );
            }
            ...
            if (mayor != null) {
               buf.append(
                  "text{type=\"italic\";text=\"" + mayor.getName() + ", " + mayor.getRole().getName() + ", " + village.getName() + "\"};text{text=\"\"}"
               );
            } else {
            ...
            logger.log(Level.WARNING, this.getResponder().getName() + " tried to get info for null token with id " + this.target, (Throwable)var13);
