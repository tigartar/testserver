    ...
    public void sendQuestion() {
        try {
            ...
            if (!village.isDisbanding() && village.mayChangeName() && village.getRoleFor(this.getResponder()).mayResizeSettlement()) {
                ...
                buf.append("text{type=\"bold\";color=\"255,50,0\";text=\"NOTE: Changing the name will" + (Servers.localServer.isFreeDeeds() ? "" : " cost 5 silver,") + " remove all the faith bonuses, and lock the name for 6 months.\"}");
            } else {
            ...
            if (village.isDemocracy() && village.getFounderName().equals(this.getResponder().getName()) && village.getMayor().getId() == this.getResponder().getWurmId() || this.getResponder().getPower() >= 4) {
                buf.append("checkbox{id=\"nondemocracy\";text=\"Mark this if you want to make this settlement a non-democracy so you cannot be removed from office: \"}");
            } else {
            ...
            buf.append("checkbox{id=\"unlimitC\"" + ch + ";text=\"Mark this if you want to be able to recruit more than " + village.getMaxCitizens() + " citizens.\"}label{text=\"Your upkeep costs are doubled as long as you have more than that amount of citizens.\"}");
            if (Features.Feature.HIGHWAYS.isEnabled()) {
                ...
                buf.append("checkbox{id=\"highways\"" + ch + en + hover + ";text=\"Mark this if you want citizens to be able to make a highway to your village.\"}");
                hover = "";
                en = "";
                ch = "";
                ...
                buf.append("checkbox{id=\"routing\"" + ch + en + hover + ";text=\"Mark this if you want your village to show in the find route village list.\";hover=\"It will only show in the list if there is a route to your village.\"}");
