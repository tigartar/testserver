   ...
   public void sendQuestion() {
      if (this.reportItem != null) {
         ...
         if (dv != null) {
               ...
               if (this.reportItem.getAuxBit(1)) {
                  if (this.reportItem.getAuxBit(2)) {
                     if (this.reportItem.getAuxBit(3)) {
                        buf.append(
                           "text{text='There is enough location information written in this report that you believe that you could use it to track down the exact location of this village.';}"
                        );
                        buf.append("text{text='Maybe following the report to the location will have some effect.';}");
                     } else {
                        buf.append(
                           "text{text='There is almost enough location information written in this report to be able to use it to track down this village.';}"
                        );
                        buf.append("text{text='Perhaps investigating around the village area some more will provide more location clues.';}");
                     }
                  } else {
                     buf.append(
                        "text{text='Written in this report is a decent number of location markers and reference to nearby landmarks, but you doubt it is enough information to be able to find the village.';}"
                     );
                     buf.append("text{text='Perhaps investigating around the village area some more will provide more location clues.';}");
                  }
               } else {
            ...
            buf.append(
               "text{type='bold';text='The report seems to be written in some foreign language. Perhaps it is meant for a village from some distant lands.'}"
            );
         }
