...
extends Question {
    ...
    public ManageObjectList(Creature aResponder, Type aType, long parent, boolean wasFromList, int aSortBy, String searchFor, boolean aIncludeAll) {
        super(aResponder, ManageObjectList.getTitle(aResponder, aType, parent, wasFromList, searchFor), ManageObjectList.getQuestion(aResponder, aType, parent, wasFromList, searchFor), 121, parent);
        ...
        block97: {
            block98: {
                block96: {
                    block95: {
                        ...
                        if (remall) {
                            for (PermissionsPlayerList.ISettings is : this.objects) {
                                if (is.isActualOwner(this.target)) continue;
                        ...
                        if (managePermissions || manageDoors || findAnimal || queue || viewDelivery) {
                                ...
                                block94: {
                                    ...
                                    if (ct == 1) {
                                        try {
                                            ...
                                            ManagePermissions mp = new ManagePermissions(this.player, Type.ANIMAL2, creature, true, this.target, false, this.objectType, "");
                                            mp.sendQuestion();
                                        }
                                        ...
                                        try {
                                            Structure theItem = Structures.getStructure(selId);
                                            ManagePermissions mp = new ManagePermissions(this.player, Type.BUILDING, theItem, true, this.target, false, this.objectType, "");
                                            mp.sendQuestion();
                                        }
                                        ...
                                        try {
                                            ...
                                            for (Door door : structure.getAllDoors()) {
                                                if (door.getWurmId() != selId) continue;
                            ...
                            if (manageDoors) {
                                mol = new ManageObjectList(this.player, Type.DOOR, selId, true, 1, "", this.includeAll);
                                mol.sendQuestion();
                                ...
                                if (queue && this.objectType == Type.WAGONER) {
                                    ...
                                    mol = new ManageObjectList(this.player, Type.WAGONER, selId, true, 1, "", this.includeAll, this.inQueue, this.waitAccept, this.inProgress, this.delivered, this.rejected, this.cancelled);
                                    mol.sendQuestion2();
                                    return;
                                }
                                this.inQueue = this.getBooleanProp("inqueue");
                                ...
                                this.rejected = this.getBooleanProp("rejected");
                                this.cancelled = this.getBooleanProp("cancelled");
                                mol = new ManageObjectList(this.player, Type.DELIVERY, selId, true, 1, "", this.includeAll, this.inQueue, this.waitAccept, this.inProgress, this.delivered, this.rejected, this.cancelled);
                                mol.sendQuestion3();
                                return;
                            }
                        }
                    ...
                    if (this.objectType != Type.BUILDING) break block96;
                    for (String key : this.getAnswer().stringPropertyNames()) {
                        if (!key.startsWith("demolish")) continue;
                ...
                if (this.objectType != Type.WAGONER) break block98;
                for (String key : this.getAnswer().stringPropertyNames()) {
                    if (!key.startsWith("dismiss")) continue;
            ...
            if (this.objectType != Type.ANIMAL0 && this.objectType != Type.ANIMAL1 && this.objectType != Type.ANIMAL2) break block97;
            for (String key : this.getAnswer().stringPropertyNames()) {
                ...
                if (!key.startsWith("untame")) continue;
    ...
    public void sendQuestion() {
        ...
        if (this.objectType == Type.REPLY) {
            ...
            HashSet<PermissionsPlayerList.ISettings> result = new HashSet<PermissionsPlayerList.ISettings>();
            for (PermissionsPlayerList.ISettings is : this.objects = Creatures.getManagedAnimalsFor(this.player, vid, true)) {
                if (!is.isGuest(this.target)) continue;
                result.add(is);
            }
            for (PermissionsPlayerList.ISettings is : this.objects = Structures.getManagedBuildingsFor(this.player, vid, true)) {
                if (!is.isGuest(this.target)) continue;
                result.add(is);
            }
            for (PermissionsPlayerList.ISettings is : this.objects = FenceGate.getManagedGatesFor(this.player, vid, true)) {
                if (!is.isGuest(this.target)) continue;
                result.add(is);
            }
            for (PermissionsPlayerList.ISettings is : this.objects = Items.getManagedCartsFor(this.player, true)) {
                if (!is.isGuest(this.target)) continue;
                result.add(is);
            }
            for (PermissionsPlayerList.ISettings is : this.objects = MineDoorPermission.getManagedMineDoorsFor(this.player, vid, true)) {
                if (!is.isGuest(this.target)) continue;
                result.add(is);
            }
            for (PermissionsPlayerList.ISettings is : this.objects = Items.getManagedShipsFor(this.player, true)) {
                if (!is.isGuest(this.target)) continue;
                result.add(is);
            }
            ...
            case 1: {
                Arrays.sort(deliveries, new Comparator<Delivery>(){
                    ...
                    public int compare(Delivery param1, Delivery param2) {
                        ...
                        if (value1 == (value2 = param2.getDeliveryId())) {
                            return 0;
                        }
                        if (value1 < value2) {
                            return -1 * upDown;
            ...
            case 3: {
                Arrays.sort(structures, new Comparator<Structure>(){
                    ...
                    public int compare(Structure param1, Structure param2) {
                        int value2;
                        int value1;
                        int n = param1.getAllDoors().length == 0 ? 0 : (value1 = param1.isLockable() ? 1 : 2);
                        int n2 = param2.getAllDoors().length == 0 ? 0 : (value2 = param2.isLockable() ? 1 : 2);
                        if (value1 == value2) {
                            return param1.getObjectName().compareTo(param2.getObjectName()) * upDown;
                        }
                        if (value1 < value2) {
            ...
            case 4: {
                Arrays.sort(structures, new Comparator<Structure>(){
                    ...
                    public int compare(Structure param1, Structure param2) {
                        ...
                        int n = value2 = param2.getVillage() != null ? param2.getVillage().getId() : 0;
                        if (value1 == value2) {
                            return param1.getObjectName().compareTo(param2.getObjectName()) * upDown;
                        }
                        if (value1 < value2) {
    ...
    private String makeLandVehicleList() {
        ...
        for (Item item : items) {
            buf.append((item.canHavePermissions() ? "radio{group=\"sel\";id=\"" + item.getWurmId() + "\";text=\"\"}" : "label{text=\"\"}") + "label{text=\"" + item.getObjectName() + "\"};" + ManageObjectList.addRariryColour(item, item.getTypeName()) + "label{" + this.showBoolean(item.isActualOwner(this.player.getWurmId())) + "};label{" + this.showBoolean(item.isLocked()) + "};label{text=\"\"};");
        }
        buf.append("}");
        return buf.toString();
    }
            ...
            case 3: {
                Arrays.sort(animals, new Comparator<Creature>(){
                    ...
                    public int compare(Creature param1, Creature param2) {
                        ...
                        if (value1 == (value2 = param2.getWagoner().getQueueLength())) {
                            return 0;
                        }
                        if (value1 < value2) {
                            return -1 * upDown;
