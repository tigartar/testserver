...
public final class TerrainQuestion extends Question {
         ...
         if (d1 != null) {
            ...
            if (this.getResponder().getLogger() != null) {
               this.getResponder()
                  ...
                  .log(
                     Level.INFO,
                     this.getResponder() + " changing tile " + this.tilex + ", " + this.tiley + " to : " + theTile.getDesc() + " (" + theTile.getId() + ")"
                  );
            }
            ...
            this.getResponder()
               ...
               .sendNormalServerMessage(
                  "Trying to change tile " + this.tilex + ", " + this.tiley + " to : " + theTile.getDesc() + " (" + theTile.getId() + ")."
               );
            ...
            if (auto) {
               ...
               if (sizex > 0 && sizey > 0) {
                  ...
                  this.getResponder()
                     .getCommunicator()
                     .sendNormalServerMessage("wand setup for " + theTile.getDesc() + " (" + theTile.getId() + ") and x,y of " + sizex + "," + sizey + ".");
               } else {
               ...
               if (!theTile.isSolidCave() && newType != Tiles.Tile.TILE_CAVE.id && !theTile.isReinforcedCave()) {
                  if (this.getResponder().getPower() >= 5
                     || newType != Tiles.Tile.TILE_ROCK.id
                        && Tiles.decodeType(Server.surfaceMesh.getTile(this.tilex, this.tiley)) != Tiles.Tile.TILE_ROCK.id
   ...
   public void sendQuestion() {
      ...
      buf.append(
         "harray{label{text=\"Category\"}dropdown{id=\"cat\";default=\""
            + this.category
            + "\"options=\"All,Bushes,Cave,Minedoors,Normal,Paving,Surface,Trees\"}label{text=\" Filter:\"};input{maxchars=\"30\";id=\"filtertext\";text=\""
            + this.filter
            + "\";onenter=\"filterme\"}label{text=\" \"};button{text=\"Update list\";id=\"filterme\"}}"
      );
      ...
      if (tiles.length == 0) {
         ...
         buf.append(tiles[0].tiledesc + " (" + (tiles[0].id & 255) + ")");
         ...
         for(int x = 0; x < tiles.length; ++x) {
            if (tiles[x] != null) {
               buf.append("," + tiles[x].tiledesc + " (" + (tiles[x].id & 255) + ")");
      ...
      buf.append(
         "harray{label{text=\"You can also set the area to paint, max is 9 in either direction \"};input{id=\"sizex\";text=\"\";maxchars=\"1\"};label{text=\" \"};input{id=\"sizey\";text=\"\";maxchars=\"1\"};}"
      );
      buf.append("label{text=\"\"}");
