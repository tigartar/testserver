         ...
         if (val != null && val.length() > 0 || putOnSurface) {
            try {
               ...
               try {
                  ...
                  short rock = Tiles.decodeHeight(
                     Server.rockMesh.getTile(this.getResponder().getCurrentTile().tilex, this.getResponder().getCurrentTile().tiley)
                  );
                  short height = Tiles.decodeHeight(
                     Server.surfaceMesh.getTile(this.getResponder().getCurrentTile().tilex, this.getResponder().getCurrentTile().tiley)
                  );
                     ...
                     if (x >= diff && !putOnSurface) {
                        this.getResponder()
                           ...
                           .sendNormalServerMessage(
                              "You can not hide the " + i.getName() + " at " + (rock + x) + ". Rock is at " + rock + ", and surface is at " + height + "."
                           );
                     } else {
                        ...
                        if (putOnSurface) {
                           ...
                           this.getResponder()
                              .getCommunicator()
                              .sendNormalServerMessage("You carefully hide the " + i.getName() + " at " + (float)(rock + x) / 10.0F + " meters.");
                        }
                     }
                  }
