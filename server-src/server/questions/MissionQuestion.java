...
extends Question {
    ...
    public void answer(Properties answers) {
        ...
        if (cm.hasCheckBox()) {
            ...
            if (done) {
                ...
                if (this.missionNumber != 9999) {
                    ...
                    popup = new SimplePopup(this.getResponder(), "Tutorial done!", "That concludes the tutorial! The " + guide.getName() + " is most pleased. Congratulations and good luck!");
                    popup.sendQuestion();
                }
            } else {
                popup = new SimplePopup(this.getResponder(), "Wait for now.", "You decide to take a pause and maybe come back later.");
                popup.sendQuestion();
            }
        } else if (answers.getProperty("mission") != null && (ok = answers.getProperty("mission").equals("do"))) {
            OldMission m = OldMission.getMission(this.missionNumber, this.getResponder().getKingdomId());
            if (m != null) {
                popup = new SimplePopup(this.getResponder(), this.title, "You accept the mission.");
                popup.sendQuestion();
            } else {
                ((Player)this.getResponder()).setTutorialLevel(9999);
                popup = new SimplePopup(this.getResponder(), "Tutorial done!", "That concludes the tutorial! The " + guide.getName() + " is most pleased. Congratulations and good luck!");
        ...
        if (answers.getProperty("mission") != null) {
            ...
            if (!skip && ok) {
                SimplePopup popup2 = new SimplePopup(this.getResponder(), "Wait for now.", "You decide to take a pause and maybe come back later.");
                popup2.sendQuestion();
            }
            if (ok = answers.getProperty("mission").equals("skip")) {
            ...
            if (ok = answers.getProperty("mission").equals("skipall")) {
                ((Player)this.getResponder()).setTutorialLevel(9999);
                SimplePopup popup3 = new SimplePopup(this.getResponder(), "Tutorial done!", "You decide to skip all the missions for now. You may return later. Good luck!");
                popup3.sendQuestion();
            }
        }
    }
