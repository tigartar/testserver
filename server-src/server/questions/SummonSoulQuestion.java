...
public class SummonSoulQuestion extends Question {
   ...
   public void answer(Properties aAnswers) {
      if (this.properlySent) {
         ...
         if (soul != null && soul.getPower() <= this.getResponder().getPower()) {
            SummonSoulAcceptQuestion ssaq = new SummonSoulAcceptQuestion(
               soul,
               "Accept Summon?",
               "Would you like to accept a summon from " + this.getResponder().getName() + "?",
   ...
   private static Creature acquireSoul(String name) {
      ...
      if (pinf != null && pinf.loaded) {
         ...
         } catch (NoSuchCreatureException | NoSuchPlayerException var3) {
            logger.log(Level.WARNING, var3.getMessage());
         }
      }
   ...
   public void sendQuestion() {
      ...
      String sb = this.getBmlHeader()
         + "text{text='Which soul do you wish to summon?'};label{text='Name:'};input{id='name';maxchars='40';text=\"\"};"
