...
import com.wurmonline.server.questions.MissionStats;
import com.wurmonline.server.questions.Question;
import com.wurmonline.server.questions.SoundList;
...
CreatureTypes {
    ...
    public static byte CAN_SEE_EPIC_MISSIONS = (byte)2;
    ...
    private boolean parseTriggerList() {
        ...
        for (String key : this.getAnswer().stringPropertyNames()) {
            ...
            if (!key.startsWith("delT")) continue;
            String sid = key.substring(4);
    ...
    private boolean editEffect(int eid, @Nullable Properties ans) {
        ...
        MissionManager mm = new MissionManager(this.getResponder(), "Trigger effect", "Edit trigger effect " + name, this.target, this.targName, this.missionRulerId);
        this.cloneValues(mm);
        mm.triggerId = 0;
        if (ans == null) {
            mm.effectId = eid;
    ...
    private boolean showStats(int mid) {
        ...
        MissionStats ms = new MissionStats(this.getResponder(), "Mission statistics", "Statistics for " + msn.getName(), msn.getId());
        ms.setRoot(this);
        ms.sendQuestion();
        return true;
    }
    ...
    private boolean createNewTrigger(@Nullable Properties ans) {
        ...
        MissionManager mm = new MissionManager(this.getResponder(), "New mission trigger", "Create a new mission trigger", this.target, this.targName, this.missionRulerId);
        this.cloneValues(mm);
        mm.level = 4;
        mm.missionId = 0;
        mm.triggerId = 0;
    ...
    private boolean createNewEffect(@Nullable Properties ans) {
        ...
        MissionManager mm = new MissionManager(this.getResponder(), "New mission effect", "Create a new mission trigger effect", this.target, this.targName, this.missionRulerId);
        this.cloneValues(mm);
        mm.level = 8;
        mm.missionId = 0;
        mm.triggerId = 0;
    ...
    private void sendManageTrigger() {
        ...
        StringBuilder buf = new StringBuilder();
        buf.append("border{border{size=\"20,20\";null;null;varray{rescale=\"true\";harray{label{type='bold';text=\"" + this.question + "    \"};label{type=\"bolditalic\";" + red + ";text=\"" + this.errorText + "\"}}}harray{button{id=\"back\";text=\"Back\"};label{text=\"  \"}}null;}null;scroll{vertical=\"true\";horizontal=\"false\";varray{rescale=\"true\";passthrough{id=\"id\";text=\"" + this.getId() + "\"}");
        ...
        if (this.getResponder().getPower() > 0) {
            ...
            buf.append("harray{checkbox{text=\"\";id=\"useCurrentTarget\";selected=\"" + useCurrentTarget + "\"}label{text=\"Use current: \"};label{" + blue + "text=\"" + currentString + "\"};label{text=\" Current Id: \"};input{id=\"targetid\";text=\"" + this.currentTargetId + "\";}}");
        } else {
            buf.append("harray{label{text=\"Action performed\"};" + this.dropdownActions("actionId", (short)actionId) + "label{text=\" on target \"};label{" + curColour + "text=\"" + tgtString + "\"};}");
            buf.append("harray{checkbox{text=\"\";id=\"useCurrentTarget\";selected=\"" + useCurrentTarget + "\"}label{text=\"Use current: \"};label{" + blue + "text=\"" + currentString + "\"};passthrough{id=\"targetid\";text=\"" + trgTarget + "\"}}");
        }
        buf.append("text{text=\"\"}");
        ...
        buf.append("harray{label{text=\"Triggered state from \"};input{id=\"stateFrom\";maxchars=\"5\";text=\"" + stateFromString + "\"};label{text=\" to \"};input{id=\"stateTo\";maxchars=\"5\";text=\"" + stateToString + "\"};label{" + blue + "text=\"  See Note 4.\";hover=\"The state 'to' is only valid if greater than the 'from'. Leave 'to' as '0.0' if not required.\"}label{text=\"  % Use '.' for decimals.\"}};");
        buf.append("text{text=\"\"}");
        if (this.level == 4) {
        ...
        if (trg != null) {
            ...
            Object[] teffs = TriggerEffects.getFilteredEffects(trigs, this.getResponder(), this.showE, this.incEInactive, this.dontListMine, this.listMineOnly, this.listForUser, false);
            if (teffs.length > 0) {
                Arrays.sort(teffs);
                ...
                for (Object lEff : teffs) {
                    String colour = green;
                    String hover = hoverActive;
                    if (((TriggerEffect)lEff).isInactive()) {
                        colour = red;
                        hover = hoverInactive;
                    } else if (((TriggerEffect)lEff).hasTargetOf(this.currentTargetId, this.getResponder())) {
                        colour = blue;
                        hover = hoverCurrentTarget;
                    }
                    String clrtxt = "label{text=\"         \";hover=\"no text to clear\"}";
                    if (!((TriggerEffect)lEff).getTextDisplayed().isEmpty()) {
                        clrtxt = "harray{label{text=\" \"};button{id=\"clrE" + ((TriggerEffect)lEff).getId() + "\";text=\"Clear Text\"};}";
                    }
                    buf.append("harray{button{id=\"edtE" + ((TriggerEffect)lEff).getId() + "\";text=\"Edit\"};label{text=\" \"};};label{" + colour + "text=\"" + ((TriggerEffect)lEff).getId() + " \"" + hover + "};label{" + colour + "text=\"" + ((TriggerEffect)lEff).getName() + " \"" + hover + "};label{" + colour + "text=\"" + ((TriggerEffect)lEff).getMissionStateChange() + " \"" + hover + "};label{" + colour + "text=\"" + ((TriggerEffect)lEff).getType() + " \"" + hover + "};" + clrtxt + "harray{label{text=\" \"};button{id=\"unlE" + ((TriggerEffect)lEff).getId() + "\";text=\"Unlink\"};};");
                }
                buf.append("}");
            } else {
            ...
            if (sIndex != null && !sIndex.isEmpty()) {
                if (this.getResponder().getPower() > 0) {
                    try {
                        ...
                        if (newexistingItem == existingItem) break block12;
                        try {
                ...
                try {
                    ...
                    if (!this.errorText.isEmpty()) break block12;
                    this.errorText = "Failed to parse value for existingItem.";
                }
            }
        }
        ...
        block7: {
            ...
            if (sIndex != null && !sIndex.isEmpty()) {
                try {
                    ...
                    if (!this.errorText.isEmpty()) break block7;
        ...
        block3: {
            ...
            if (sIndex != null && !sIndex.isEmpty()) {
                try {
                    ...
                    if (!this.errorText.isEmpty()) break block3;
                    this.errorText = "Failed to parse value for " + type + ".";
                }
            }
        }
        ...
        if (sIndex != null && !sIndex.isEmpty()) {
            block11: {
                try {
                    ...
                    if (!this.errorText.isEmpty()) break block11;
                    this.errorText = "Failed to parse value for " + type + ".";
                }
            }
            if (doChecks && ans != 0) {
                SpecialEffects sp;
                if (ans == 1 && !MissionManager.maySetToOpen(this.getResponder(), this.currentTargetId)) {
                    ans = 0;
                    if (this.errorText.isEmpty()) {
                        this.errorText = "You may not set that to open. Try a fence gate you can open.";
                    }
                }
                if (ans != 0 && (sp = SpecialEffects.getEffect(ans)).getPowerRequired() > this.getResponder().getPower()) {
                    ans = 0;
                    if (this.errorText.isEmpty()) {
        ...
        block15: {
            ...
            if (sIndex != null && !sIndex.isEmpty()) {
                try {
                    ...
                    if (!this.errorText.isEmpty()) break block15;
                    this.errorText = "Failed to parse value for " + type + ".";
                }
            }
        }
    ...
    private byte indexCreatureType(String key, String type) {
        ...
        block6: {
            ...
            if (sIndex != null && !sIndex.isEmpty()) {
                try {
                    byte index = Byte.parseByte(sIndex);
                    try {
                    ...
                    if (!this.errorText.isEmpty()) break block6;
                    this.errorText = "Failed to parse value for " + type + ".";
                }
            }
        }
    ...
    private void sendManageEffect() {
        ...
        if (this.getResponder().getPower() > 0) {
            ...
            catch (NumberFormatException nsi) {
                // empty catch block
            }
            buf.append("text{text=\"\"}");
            buf.append("header{text=\"Tiles\"};");
        ...
        if (this.level == 7) {
            ...
            Object[] trigs = Triggers2Effects.getTriggersForEffect(this.effectId, this.incTInactive);
            if (trigs.length > 0) {
                Arrays.sort(trigs);
                buf.append("table{rows=\"1\";cols=\"7\";");
                buf.append("label{text=\"\"};label{text=\"Id\"};label{text=\"Name\"};label{text=\"State\"};label{text=\"Action\"};label{text=\"Target\"};label{text=\"\"};");
                for (Object trigger : trigs) {
                    String colour = green;
                    String hover = hoverActive;
                    if (((MissionTrigger)trigger).isInactive()) {
                        colour = red;
                        hover = hoverInactive;
                    } else if (((MissionTrigger)trigger).hasTargetOf(this.currentTargetId, this.getResponder())) {
                        colour = blue;
                        hover = hoverCurrentTarget;
                    }
                    buf.append("harray{button{id=\"edtT" + ((MissionTrigger)trigger).getId() + "\";text=\"Edit\"};label{text=\"  \"}};label{" + colour + "text=\"" + ((MissionTrigger)trigger).getId() + " \"" + hover + "};label{" + colour + "text=\"" + ((MissionTrigger)trigger).getName() + " \"" + hover + "};label{" + colour + "text=\"" + ((MissionTrigger)trigger).getStateRange() + " \"" + hover + "};label{" + colour + "text=\"" + ((MissionTrigger)trigger).getActionString() + " \"" + hover + "};label{" + colour + "text=\"" + ((MissionTrigger)trigger).getTargetAsString(this.getResponder()) + "\"" + hover + "};harray{label{text=\"  \"}button{id=\"unlT" + ((MissionTrigger)trigger).getId() + "\";text=\"Unlink\"hover=\"This will unlink the trigger " + ((MissionTrigger)trigger).getName() + " from this effect\";};};");
                }
                buf.append("}");
            } else {
    ...
    private String dropdownTriggers(String id, int current, boolean reload) {
        ...
        buf.append("None");
        if (reload) {
            Object[] triggers = MissionTriggers.getFilteredTriggers(this.getResponder(), current, this.creatorType, this.listForUser, this.dontListMine, this.listMineOnly);
            Arrays.sort(triggers);
            this.mtriggers.clear();
            for (Object trig : triggers) {
                this.mtriggers.add((MissionTrigger)trig);
                ++counter;
                buf.append(",");
                buf.append(((MissionTrigger)trig).getName());
                if (current <= 0 || current != ((MissionTrigger)trig).getId()) continue;
                def = counter;
            }
        } else {
            for (MissionTrigger trig : this.mtriggers) {
                ...
                if (current <= 0 || current != trig.getId()) continue;
                def = counter;
            }
        }
    ...
    private String dropdownUnlinkedTriggers(String id, int show) {
        ...
        buf.append("dropdown{id=\"" + id + "\";options=\"");
        buf.append("None");
        Object[] utrigs = MissionTriggers.getFilteredTriggers(this.getResponder(), show, this.incTInactive, 0, 0);
        Arrays.sort(utrigs);
        this.utriggers.clear();
        for (Object trig : utrigs) {
            this.utriggers.add((MissionTrigger)trig);
            buf.append(",");
            buf.append(((MissionTrigger)trig).getName());
        }
    ...
    private String dropdownEffects(String id, int current, boolean reload) {
        ...
        buf.append("None");
        if (reload) {
            Object[] effects = TriggerEffects.getFilteredEffects(this.getResponder(), 0, this.incEInactive, this.dontListMine, this.listMineOnly, this.listForUser);
            Arrays.sort(effects);
            this.teffects.clear();
            for (Object eff : effects) {
                this.teffects.add((TriggerEffect)eff);
                ++counter;
                buf.append(",");
                buf.append(((TriggerEffect)eff).getName());
                if (current <= 0 || current != ((TriggerEffect)eff).getId()) continue;
                def = counter;
            }
        } else {
            for (TriggerEffect eff : this.teffects) {
                ...
                if (current <= 0 || current != eff.getId()) continue;
                def = counter;
            }
        }
        buf.append("\";default=\"" + def + "\"}");
    ...
    private String dropdownUnlinkedEffects(String id, int show) {
        ...
        buf.append("dropdown{id=\"" + id + "\";options=\"");
        buf.append("None");
        Object[] effects = TriggerEffects.getFilteredEffects(this.getResponder(), show, this.incEInactive, this.dontListMine, this.listMineOnly, this.listForUser);
        Arrays.sort(effects);
        this.ueffects.clear();
        for (Object eff : effects) {
            this.ueffects.add((TriggerEffect)eff);
            buf.append(",");
            buf.append(((TriggerEffect)eff).getName());
        }
        buf.append("\";default=\"0\"}");
    ...
    private String dropdownMissions(String id, int current, boolean reload) {
        ...
        buf.append("None");
        if (reload) {
            Mission[] missions = Missions.getFilteredMissions(this.getResponder(), this.includeM, this.incMInactive, this.dontListMine, this.listMineOnly, this.listForUser, this.groupName, this.onlyCurrent, this.currentTargetId);
            ...
            for (Mission lMission : missions) {
                ...
                if (current <= 0 || current != lMission.getId()) continue;
                def = counter;
            }
        } else {
    ...
    private String dropdownSkillTemplates(String id, int current) {
        StringBuilder buf = new StringBuilder();
        Object[] stemps = SkillSystem.getAllSkillTemplates();
        Arrays.sort(stemps);
        int def = 0;
        int counter = 0;
        ...
        buf.append("dropdown{id=\"" + id + "\";options=\"");
        buf.append("None");
        for (Object lStemp : stemps) {
            if (!((SkillTemplate)lStemp).isMission()) continue;
            ++counter;
            buf.append(",");
            this.stemplates.add((SkillTemplate)lStemp);
            buf.append(((SkillTemplate)lStemp).getName());
            if (current <= 0 || current != ((SkillTemplate)lStemp).getNumber()) continue;
            def = counter;
        }
        buf.append("\";default=\"" + def + "\"}");
        return buf.toString();
    ...
    private String dropdownSpecialEffects(String id, int current) {
        ...
        for (SpecialEffects lEffect : effects = SpecialEffects.getEffects()) {
            if (this.getResponder().getPower() < lEffect.getPowerRequired()) continue;
    ...
    private String dropdownItemMaterials(String id, byte current) {
        ...
        for (byte x = 0; x < 96; ++x) {
            if (x == 0) {
                ...
                buf.append(",");
                buf.append(Item.getMaterialString(x));
            }
    ...
    private String dropdownCreatureTemplates(String id, int current) {
        ...
        for (int x = 0; x < crets.length; ++x) {
            ...
            if (current <= 0 || current != crets[x].getTemplateId()) continue;
            def = counter;
        }
        buf.append("\";default=\"" + def + "\"}");
        return buf.toString();
    ...
    private String dropdownItemTemplates(String id, int current, boolean incSpecial) {
        ...
        buf.append("dropdown{id=\"" + id + "\";options=\"");
        buf.append("None");
        for (Object lTemplate : templates) {
            if (!((ItemTemplate)lTemplate).isMissionItem() && (!incSpecial || ((ItemTemplate)lTemplate).getTemplateId() != 791)) continue;
            buf.append(",");
            this.itemplates.add((ItemTemplate)lTemplate);
            if (((ItemTemplate)lTemplate).isMetal()) {
                buf.append(((ItemTemplate)lTemplate).sizeString + Item.getMaterialString(((ItemTemplate)lTemplate).getMaterial()) + " " + ((ItemTemplate)lTemplate).getName());
            } else if (((ItemTemplate)lTemplate).bowUnstringed) {
                buf.append(((ItemTemplate)lTemplate).sizeString + ((ItemTemplate)lTemplate).getName() + " [unstringed]");
            } else {
                buf.append(((ItemTemplate)lTemplate).sizeString + ((ItemTemplate)lTemplate).getName());
            }
            ++counter;
            if (current <= 0 || current != ((ItemTemplate)lTemplate).getTemplateId()) continue;
            def = counter;
        }
        buf.append("\";default=\"" + def + "\"}");
        return buf.toString();
    ...
    private String dropdownInventory(String id, long itemReward) {
        ...
        for (Item lElement : itemarr) {
            if (lElement.isNoDrop() || lElement.isArtifact()) continue;
            buf.append(",");
            this.ritems.add(lElement);
            buf.append(lElement.getName() + " - " + (int)lElement.getQualityLevel());
            ++counter;
            if (itemReward <= 0L || itemReward != lElement.getWurmId()) continue;
            def = counter;
        }
        buf.append("\";default=\"" + def + "\"}");
        return buf.toString();
    ...
    private String dropdownAchievements(String id, int current) {
        ...
        for (AchievementTemplate template : this.myAchievements) {
            ...
            if (current <= 0 || template.getNumber() != current) continue;
            def = counter;
        }
        buf.append("\";default=\"" + def + "\"}");
        return buf.toString();
    ...
    private String dropdownActions(String id, short current) {
        StringBuilder buf = new StringBuilder();
        Object[] acts = (ActionEntry[])Actions.actionEntrys.clone();
        Arrays.sort(acts);
        int def = 0;
        int counter = 0;
        ...
        buf.append("dropdown{id=\"" + id + "\";options=\"");
        buf.append("None");
        for (Object lAct : acts) {
            if (!((ActionEntry)lAct).isMission()) continue;
            ++counter;
            buf.append(",");
            buf.append(((ActionEntry)lAct).getActionString());
            this.actionEntries.add((ActionEntry)lAct);
            if (current <= 0 || current != ((ActionEntry)lAct).getNumber()) continue;
            def = counter;
        }
        buf.append("\";default=\"" + def + "\"}");
        return buf.toString();
    ...
    private void sendManageMission() {
        ...
        if (this.getAnswer() != null) {
            name = this.getStringProp("name");
            group2 = this.getStringProp("groupName");
        ...
        } else if (this.missionId > 0 && (m = Missions.getMissionWithId(this.missionId)) != null) {
            ...
            if (m.getMaxTimeSeconds() > 0) {
                ...
                int seconds = left = (int)((long)left - 60L * (long)minutes);
                sdays = "" + days;
                shours = "" + hours;
                sminutes = "" + minutes;
                sseconds = "" + seconds;
        ...
        StringBuilder buf = new StringBuilder();
        buf.append("border{border{size=\"20,20\";null;null;varray{rescale=\"true\";harray{label{type='bold';text=\"" + this.question + "    \"};label{type=\"bolditalic\";" + red + ";text=\"" + this.errorText + "\"}}}harray{button{id=\"back\";text=\"Back\"};label{text=\"  \"}}null;}null;scroll{vertical=\"true\";horizontal=\"false\";varray{rescale=\"true\";passthrough{id=\"id\";text=\"" + this.getId() + "\"}");
        buf.append("harray{label{text=\"Name (max 100 chars)\"};input{id=\"name\";maxchars=\"100\";text=\"" + name + "\"};label{text=\"  Group (20 chars)\"};input{id=\"groupName\";maxchars=\"20\";text=\"" + group2 + "\"};}");
        ...
        if (this.missionId == 0) {
            ...
            Object[] trigs = MissionTriggers.getFilteredTriggers(this.getResponder(), 0, this.incTInactive, this.missionId, 0);
            if (trigs.length > 0) {
                Arrays.sort(trigs);
                buf.append("table{rows=\"1\";cols=\"7\";");
                buf.append("label{text=\"\"};label{text=\"Id\"};label{text=\"Name\"};label{text=\"State\"};label{text=\"Action\"};label{text=\"Target\"};label{text=\"\"};");
                for (Object trigger : trigs) {
                    ...
                    buf.append("button{id=\"edtT" + ((MissionTrigger)trigger).getId() + "\";text=\"Edit\"};label{" + colour + "text=\"" + ((MissionTrigger)trigger).getId() + " \"" + hover + "};label{" + colour + "text=\"" + ((MissionTrigger)trigger).getName() + " \"" + hover + "};label{" + colour + "text=\"" + ((MissionTrigger)trigger).getStateRange() + " \"" + hover + "};label{" + colour + "text=\"" + ((MissionTrigger)trigger).getActionString() + " \"" + hover + "};label{" + colour + "text=\"" + ((MissionTrigger)trigger).getTargetAsString(this.getResponder()) + "\"" + hover + "};harray{label{text=\"  \"}button{id=\"unlT" + ((MissionTrigger)trigger).getId() + "\";text=\"Unlink\"hover=\"This will unlink the trigger " + ((MissionTrigger)trigger).getName() + " from this mission\";};};");
                }
                buf.append("}");
            } else {
                buf.append("label{text=\"No triggers found\"}");
            ...
            Object[] teffs = TriggerEffects.getFilteredEffects((MissionTrigger[])trigs, this.getResponder(), this.showE, this.incEInactive, this.dontListMine, this.listMineOnly, this.listForUser, false);
            if (teffs.length > 0) {
                Arrays.sort(teffs);
                buf.append("table{rows=\"1\";cols=\"7\";");
                buf.append("label{text=\"\"};label{text=\"Id\"};label{text=\"Name\"};label{text=\"+State\"};label{text=\"Type\"};label{text=\"\"};label{text=\"\"};");
                for (Object lEff : teffs) {
                    ...
                    buf.append("button{id=\"edtE" + ((TriggerEffect)lEff).getId() + "\";text=\"Edit\"};label{" + colour + "text=\"" + ((TriggerEffect)lEff).getId() + " \"" + hover + "};label{" + colour + "text=\"" + ((TriggerEffect)lEff).getName() + " \"" + hover + "};label{" + colour + "text=\"" + ((TriggerEffect)lEff).getMissionStateChange() + " \"" + hover + "};label{" + colour + "text=\"" + ((TriggerEffect)lEff).getType() + " \"" + hover + "};" + clrtxt + "harray{label{text=\" \"}};");
                }
                buf.append("}");
            } else {
                buf.append("label{text=\"No trigger effects found\"}");
            ...
            Object[] meffs = TriggerEffects.getFilteredEffects((MissionTrigger[])trigs, this.getResponder(), this.showE, this.incEInactive, this.dontListMine, this.listMineOnly, this.listForUser, this.missionId);
            if (meffs.length > 0) {
                Arrays.sort(meffs);
                buf.append("table{rows=\"1\";cols=\"7\";");
                buf.append("label{text=\"\"};label{text=\"Id\"};label{text=\"Name\"};label{text=\"+State\"};label{text=\"Type\"};label{text=\"\"};label{text=\"\"};");
                for (Object lEff : meffs) {
                    String colour = green;
                    String hover = hoverActive;
                    if (((TriggerEffect)lEff).isInactive()) {
                        colour = red;
                    ...
                    buf.append("button{id=\"edtE" + ((TriggerEffect)lEff).getId() + "\";text=\"Edit\"};label{" + colour + "text=\"" + ((TriggerEffect)lEff).getId() + " \"" + hover + "};label{" + colour + "text=\"" + ((TriggerEffect)lEff).getName() + " \"" + hover + "};label{" + colour + "text=\"" + ((TriggerEffect)lEff).getMissionStateChange() + " \"" + hover + "};label{" + colour + "text=\"" + ((TriggerEffect)lEff).getType() + " \"" + hover + "};" + clrtxt + "harray{label{text=\" \"}button{id=\"unlE" + ((TriggerEffect)lEff).getId() + "\";text=\"Unlink\"hover=\"This will unlink effect " + ((TriggerEffect)lEff).getName() + " from this mission\";};};");
                }
                buf.append("}");
            } else {
    ...
    public static final boolean maySetToOpen(Creature resp, long targetId) {
        if (resp.getPower() == 0) {
                ...
                if (w != null) {
                    try {
                        ...
                        for (Item key : resp.getKeys()) {
                            if (key.getWurmId() != s.getWritId()) continue;
                            return true;
                        }
                    }
            ...
            if (WurmId.getType(targetId) == 7 && (f = Fence.getFence(targetId)).isDoor()) {
    ...
    private boolean parseEffect(Item ruler) {
        ...
        if (linkTrigger) {
            MissionTrigger mtrig;
            int trig = this.indexUnlinkedTrigger("trigs", "Unlinked trigger");
            if (trig > 0 && (mtrig = MissionTriggers.getTriggerWithId(trig)) != null) {
        ...
        block8: {
            ...
            if (id > 0L) {
                try {
                    ...
                    id = 0L;
                    if (!this.errorText.isEmpty()) break block8;
    ...
    private boolean parseMission(Item ruler) {
        ...
        if (updateMission || createMission) {
            ...
            if (!group2.equals(m.getGroupName())) {
                m.setGroupName(group2);
                changed = true;
            }
        ...
        if (linkTrigger) {
            ...
            if (utrig > 0 && (mtrig = MissionTriggers.getTriggerWithId(utrig)) != null) {
        ...
        if (linkEffect) {
            ...
            if (uEff > 0 && (mEff = TriggerEffects.getTriggerEffect(uEff)) != null) {
        ...
        for (String key : this.getAnswer().stringPropertyNames()) {
            ...
            if (!edtE && !delE && !unlE && !clrE) continue;
            String sid = key.substring(4);
    ...
    private boolean parseTrigger(Item ruler) {
        ...
        if (updateTrigger || createTrigger) {
            ...
            missionRequired = this.indexMission("missionRequired", "available missions");
            if (onActionPerformed == 475 && WurmId.getType(onActionTargetId) == 3 && this.getResponder().getPower() <= 0 && ((v = Villages.getVillage(tilex = Tiles.decodeTileX(onActionTargetId), tiley = Tiles.decodeTileY(onActionTargetId), true)) == null || v != this.getResponder().getCitizenVillage())) {
                if (this.errorText.isEmpty()) {
        ...
        if (linkEffect) {
            ...
            if (effe > 0 && (meff = TriggerEffects.getTriggerEffect(effe)) != null) {
    ...
    private void sendEffectList() {
        ...
        TriggerEffect[] effs = TriggerEffects.getFilteredEffects(trigs, this.getResponder(), this.showE, this.incEInactive, this.dontListMine, this.listMineOnly, this.listForUser, showAll);
        if (effs.length > 0) {
            switch (absSortBy) {
                case 1: {
                ...
                case 3: {
                    Arrays.sort(effs, new Comparator<TriggerEffect>(){
                        ...
                        public int compare(TriggerEffect param1, TriggerEffect param2) {
                            ...
                            if (value1 == (value2 = param2.getMissionStateChange())) {
                                return 0;
                            }
                            if (value1 < value2) {
                                return -1 * upDown;
        ...
        block10: {
            ...
            if (svalue != null && !svalue.isEmpty()) {
                try {
                    ...
                    if (!this.errorText.isEmpty()) break block10;
                    this.errorText = "Failed to parse value for " + key + ".";
                }
            }
        }
