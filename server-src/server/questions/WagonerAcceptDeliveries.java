...
public class WagonerAcceptDeliveries extends Question implements MonetaryConstants {
                        ...
                        if (passed) {
                           ...
                           this.getResponder()
                              ...
                              .sendNormalServerMessage(
                                 "If this amount is incorrect, please wait a while since the information may not immediately be updated."
                              );
                        }
   ...
   public void sendQuestion() {
      ...
      buf.append(
         "text{text=\"If an delivery has a Cash On Delivery (C.O.D) cost associated with it, you have to pay upfront before the delivery will start, the monies will be held by the wagoner and will only be paid to the supplier when the delivery is complete.\"}"
      );
      ...
      switch(absSortBy) {
         ...
         case 7:
            Arrays.sort(deliveries, new Comparator<Delivery>() {
               public int compare(Delivery param1, Delivery param2) {
                  return param1.getReceiverCost() < param2.getReceiverCost() ? 1 * upDown : -1 * upDown;
               }
            });
      }
      ...
      buf.append(
         "table{rows=\"1\";cols=\"9\";label{text=\"\"};"
            + this.colHeader("Sender", 1, this.sortBy)
            ...
            + this.colHeader("Cost", 7, this.sortBy)
      ...
      for(Delivery delivery : deliveries) {
         ...
         buf.append(
            (connected ? "radio{group=\"sel\";id=\"" + delivery.getDeliveryId() + "\";" + selected + "text=\"\"};" : "label{text=\"  \"};")
               + "label{text=\""
               ...
               + delivery.getCrates()
               + "\"};label{text=\""
               + delivery.getWagonerName()
               + "\"};label{text=\""
               + delivery.getWagonerState()
               + "\"};label{text=\""
               + new Change(delivery.getReceiverCost()).getChangeShortString()
               + "\"};"
         );
         if (sameWaystone) {
      ...
      if (this.waystone.getData() != -1L) {
         ...
         if (wagoner == null) {
            logger.log(
               Level.WARNING,
               "wagoner ("
                  + this.waystone.getData()
                  + ") not found that was associated with waystone "
                  + this.waystone.getWurmId()
                  + " @"
                  + this.waystone.getTileX()
                  + ","
                  + this.waystone.getTileY()
                  + ","
                  + this.waystone.isOnSurface()
            );
