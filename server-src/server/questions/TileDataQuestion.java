...
public final class TileDataQuestion extends Question {
   ...
   public void sendQuestion() {
      ...
      if (surfTile.isBush() || surfTile.isTree()) {
         ...
         txUnused = "";
         if (surfTile.isTree() && (surfData >>> 4 & 15) == 15) {
            noGUnused = "";
            canHaveGrubs = true;
         }

         if (surfTile.isBush() && (surfData >>> 4 & 14) == 14) {
            noGUnused = "";
            canHaveGrubs = true;
         }

         if ((
               surfType == Tiles.Tile.TILE_TREE_BIRCH.id
                  || surfType == Tiles.Tile.TILE_ENCHANTED_TREE_BIRCH.id
                  || surfType == Tiles.Tile.TILE_MYCELIUM_TREE_BIRCH.id
            )
            && (surfData >>> 4 & 14) == 14) {
            noGUnused = "";
            canHaveGrubs = true;
         ...
         buf.append("input{id=\"count\";maxchars=\"2\";text=\"" + (surfResource >>> 11 & 31) + "\"};");
         buf.append("label{text=\"Yield chance:\"};");
         buf.append("input{id=\"surf\";maxchars=\"4\";text=\"" + (surfResource & 1023) + "\"};");
         buf.append("}");
         txUnused = "";
      } else if (surfType == Tiles.Tile.TILE_SAND.id
         || surfType == Tiles.Tile.TILE_GRASS.id
         || surfType == Tiles.Tile.TILE_MYCELIUM.id
         || surfType == Tiles.Tile.TILE_STEPPE.id
         || surfType == Tiles.Tile.TILE_CLAY.id
         || surfType == Tiles.Tile.TILE_PEAT.id
         || surfType == Tiles.Tile.TILE_TAR.id
         || surfType == Tiles.Tile.TILE_TUNDRA.id
         || surfType == Tiles.Tile.TILE_MOSS.id) {
      ...
      if (surfTile.isGrass()) {
         ...
         buf.append("label{text=\" Flower:\"};input{id=\"flower\";maxchars=\"2\";text=\"" + (surfData & 15) + "\"};");
         buf.append("}");
      } else if (surfType == Tiles.Tile.TILE_MYCELIUM.id) {
         buf.append("harray{label{text=\"Mycelium tile data \"};");
         buf.append("label{text=\" Growth:\"};input{id=\"growth\";maxchars=\"1\";text=\"" + (surfData >>> 6 & 3) + "\"};");
         buf.append("label{text=\" GrassType(not used):\"};input{id=\"grasstype\";maxchars=\"1\";text=\"" + (surfData >>> 4 & 3) + "\"};");
         buf.append("label{text=\" Flower (not used):\"};input{id=\"flower\";maxchars=\"2\";text=\"" + (surfData & 15) + "\"};");
         buf.append("}");
      } else if (surfType == Tiles.Tile.TILE_BUSH.id
         || surfType == Tiles.Tile.TILE_ENCHANTED_BUSH.id
         || surfType == Tiles.Tile.TILE_TREE.id
         || surfType == Tiles.Tile.TILE_ENCHANTED_TREE.id
         || surfType == Tiles.Tile.TILE_MYCELIUM_BUSH.id
         || surfType == Tiles.Tile.TILE_MYCELIUM_TREE.id) {
         buf.append("harray{label{text=\"Tree/Bush tile data \"};");
         buf.append("label{text=\" Age:\"};input{id=\"age\";maxchars=\"3\";text=\"" + (surfData >>> 4 & 15) + "\"};");
         buf.append("label{text=\" Type:\"};input{id=\"type\";maxchars=\"3\";text=\"" + (surfData & 15) + "\"};");
         buf.append("}");
         ...
         buf.append("label{text=\" Crop:\"};input{id=\"crop\";maxchars=\"3\";text=\"" + (surfData & 15) + "\"};");
         buf.append("}");
      } else if (Tiles.isRoadType(surfType)) {
         buf.append("harray{label{text=\"Surface tile data \"};");
         buf.append("label{text=\" (not used):\"};input{id=\"unused\";maxchars=\"2\";text=\"" + (surfData >>> 3 & 31) + "\"};");
         buf.append("label{text=\" Dir:\"};dropdown{id='dir';options=\"none,NW,NE,SE,SW\";default=\"" + (surfData & 7) + "\"}");
         buf.append("}");
         if (surfType == Tiles.Tile.TILE_STONE_SLABS.id
            || surfType == Tiles.Tile.TILE_POTTERY_BRICKS.id
            || surfType == Tiles.Tile.TILE_SANDSTONE_BRICKS.id
            || surfType == Tiles.Tile.TILE_SANDSTONE_SLABS.id
            || surfType == Tiles.Tile.TILE_SLATE_BRICKS.id
            || surfType == Tiles.Tile.TILE_SLATE_SLABS.id
            || surfType == Tiles.Tile.TILE_MARBLE_BRICKS.id
            || surfType == Tiles.Tile.TILE_MARBLE_SLABS.id) {
            colUnused = "";
         }
      } else {
         buf.append("harray{label{text=\"Surface tile data \"};");
      ...
      buf.append(
         "harray{label{text=\"Cave Height    \"};input{id=\"caveheight\";maxchars=\"6\";text=\""
            + caveHeight
            + "\"}label{text=\"  Cave ceiling:\"};input{id=\"caveceiling\";maxchars=\"3\";text=\""
            + caveData
            + "\"}label{text=\"  (Default is -100, 0)\"}};"
      );
      ...
      buf.append(
         "checkbox{id=\"transforming\";selected=\"" + Server.isBeingTransformed(this.tilex, this.tiley) + "\";text=\"Being Transformed" + txUnused + "\"};"
      );
      buf.append("checkbox{id=\"transformed\";selected=\"" + Server.wasTransformed(this.tilex, this.tiley) + "\";text=\"Been Transformed" + txUnused + "\"};");
      buf.append(
         "checkbox{id=\"hive\";selected=\""
            + Server.isCheckHive(this.tilex, this.tiley)
            + "\";text=\"Check Hive\";hover=\"Used exclusivly by the hive generation code.\"};"
      );
      buf.append(
         "checkbox{id=\"hasGrubs\";enabled=\""
            + canHaveGrubs
            + "\";selected=\""
            + (canHaveGrubs && Server.hasGrubs(this.tilex, this.tiley))
            + "\";text=\"Has Grubs (or Wurms)"
            + noGUnused
            + "\"};"
      );
      buf.append(
         "harray{label{text=\"Cave Flags:\"};label{text=\" Server:\"}input{id=\"caveserverflag\";maxchars=\"3\";text=\""
            + (Server.getServerCaveFlags(this.tilex, this.tiley) & 255)
            + "\"}label{text=\" Client:\"}input{id=\"caveclientflag\";maxchars=\"3\";text=\""
            + (Server.getClientCaveFlags(this.tilex, this.tiley) & 255)
            + "\"}};"
      );
