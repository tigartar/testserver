...
extends Question {
    ...
    public void sendQuestion() {
        ...
        for (int x = 0; x < crets.length; ++x) {
            Shop shop;
            if (!crets[x].isTrader() || crets[x].getKingdomId() != this.getResponder().getKingdomId() || !crets[x].isNpcTrader() || (shop = Economy.getEconomy().getShop(crets[x])).isPersonal()) continue;
            if (shop.getMoney() >= 0L) {
