...
implements TimeConstants {
    ...
    public void answer(Properties answers) {
            ...
            if (zoneType != 0) {
                ...
                if (zname != null && zname.length() > 2) {
                    if (zoneType < 7 || zoneType == 11 || zoneType == 12 || zoneType == 14 || zoneType == 16 || zoneType == 17 || zoneType == 18) {
                        FocusZone fz = new FocusZone(Zones.safeTileX(this.getResponder().getTileX() - sx), Zones.safeTileX(this.getResponder().getTileX() + sx), Zones.safeTileY(this.getResponder().getTileY() - sy), Zones.safeTileY(this.getResponder().getTileY() + sy), (byte)zoneType, zname, zdesc, true);
                        ...
                        if (zoneType == 6) {
                            Servers.localServer.setNextHota(System.currentTimeMillis() + (Servers.isThisATestServer() ? 300000L : 0x6DDD000L));
                        }
                    } else {
                        ...
                        if (zoneType == 8) {
                            if (this.getResponder().getPower() < 4) {
                                logger.severe(this.getResponder().getName() + " has attempted to use Focus Zone Flatten DIRT with power level " + this.getResponder().getPower() + "! Needs " + 4 + "at least!");
                                return;
                            }
                            ...
                            if (this.getResponder().getPower() < 4) {
                                logger.severe(this.getResponder().getName() + " has attempted to use Focus Zone Flatten ROCK with power level " + this.getResponder().getPower() + "! Needs " + 4 + "at least!");
                                return;
                            }
                            if (Constants.devmode) {
                                this.getResponder().getCommunicator().sendAlertServerMessage("Made flatten zone. Player standing on [" + this.getResponder().getTileX() + ", " + this.getResponder().getTileY() + "]. Creating zones with StartX " + stx + ", EndX " + endtx + ", StartY " + sty + ", EndY " + endty);
                                ...
                                for (int x = xa; x <= xe; ++x) {
                                    for (int y = ya; y <= ye; ++y) {
                                        ...
                                        if (x == xa || x == xe) {
                                            ...
                                            try {
                                                zone = Zones.getZone(x, y, true);
                                                fence = new DbFence(StructureConstantsEnum.FENCE_STONEWALL_HIGH, x, y, 0, 1.0f, Tiles.TileBorderDirection.DIR_DOWN, zone.getId(), 0);
                                                fence.setState(fence.getFinishState());
                                                ((Fence)fence).setQualityLevel(80.0f);
                                                ((Fence)fence).improveOrigQualityLevel(80.0f);
                                                ((Fence)fence).save();
                                                zone.addFence(fence);
                                            }
                                        ...
                                        if (y != ya && y != ye || Tiles.decodeHeight(Server.surfaceMesh.getTile(x, y)) <= 0) continue;
                                        try {
                                            zone = Zones.getZone(x, y, true);
                                            fence = new DbFence(StructureConstantsEnum.FENCE_STONEWALL_HIGH, x, y, 0, 1.0f, Tiles.TileBorderDirection.DIR_HORIZ, zone.getId(), 0);
                                            fence.setState(fence.getFinishState());
                                            ((Fence)fence).setQualityLevel(80.0f);
                                            ((Fence)fence).improveOrigQualityLevel(80.0f);
                                            ((Fence)fence).save();
        ...
        try {
            int index;
            if (delzone != null && delzone.length() > 0 && (index = Integer.parseInt(delzone)) > 0) {
                try {
