...
public final class PortalQuestion extends Question implements MonetaryConstants, TimeConstants {
         ...
         if (this.portal != null) {
            ...
            if (entry != null) {
               ...
               if (this.getResponder().hasBeenAttackedWithin(300)) {
                  this.getResponder()
                     .getCommunicator()
                     .sendNormalServerMessage("Nothing happens. You sense a disturbance - maybe your are not calm enough yet.");
                  return;
               }
                  ...
                  if (this.getResponder().getEpicServerKingdom() == 0) {
                     ...
                     if (kval != null) {
                        try {
                           ...
                           this.getResponder()
                              .getCommunicator()
                              .sendAlertServerMessage("An error occured with the target kingdom. You can't select that kingdom.");
                           return;
                        }
                     }
                  } else {
               ...
               if (entry.isAvailable(this.getResponder().getPower(), this.getResponder().isReallyPaying())) {
                  if (entry.ISPAYMENT && !this.getResponder().isReallyPaying()) {
                     ...
                     for(int x = 0; x < bodyItems.length; ++x) {
                        if (!bodyItems[x].willLeaveServer(true, changingCluster, this.getResponder().getPower() > 0)) {
                           ++stayBehind;
                     ...
                     if (numitems < 200) {
                        ...
                        if (this.getResponder().getPower() == 0 && changingCluster) {
                           try {
                              ...
                              this.getResponder()
                                 .getCommunicator()
                                 .sendNormalServerMessage("A sudden strong wind blows through the portal, throwing you back!");
                           ...
                           if (this.getResponder().getPower() <= 0) {
                              try {
                                 ...
                                 if (fs.getKnowledge() > 50.0) {
                                    double x = 100.0 - fs.getKnowledge();
                                    x -= x * 0.95;
                                    double newskill = fs.getKnowledge() - x;
                                    fs.setKnowledge(newskill, false);
                                    this.getResponder()
                                       .getCommunicator()
                                       .sendAlertServerMessage("Your group fighting skill has been set to " + fs.getKnowledge(0.0) + "!");
                                 }
                              ...
                              try {
                                 ...
                                 if (as.getKnowledge() > 50.0) {
                                    double x = 100.0 - as.getKnowledge();
                                    x -= x * 0.95;
                                    double newskill = as.getKnowledge() - x;
                                    as.setKnowledge(newskill, false);
                                    this.getResponder()
                                       .getCommunicator()
                                       .sendAlertServerMessage("Your archery skill has been set to " + as.getKnowledge(0.0) + "!");
                                 }
                        ...
                        this.getResponder()
                           .sendToLoggers(" at 4: " + entry.getName() + " target kingdom " + targetKingdom + "tx=" + targetTileX + ", ty=" + targetTileY);
                        if (Servers.localServer.entryServer) {
                           this.getResponder().setRotation(270.0F);
                           if (this.getResponder().isPlayer()) {
                              ((Player)this.getResponder()).addTitle(Titles.Title.Educated);
                        ...
                        this.getResponder()
                           .sendTransfer(
                              ...
                              targetTileY,
                              true,
                              entry.isChallengeServer(),
                              targetKingdom
                           );
                     } else {
                        this.getResponder()
                           .getCommunicator()
                           .sendNormalServerMessage("The portal does not work. You are probably carrying too much. Try 200 items on body and in inventory.");
                     }
                  }
               } else if (entry.maintaining) {
                  this.getResponder()
                     .getCommunicator()
                     .sendNormalServerMessage("The portal is shut but a flicker indicates that it may open soon. You may try later.");
            ...
            try {
               ...
               if (kval != null) {
                  try {
                     ...
                     this.getResponder()
                        .sendToLoggers("At 3: " + spnum + ", server=" + this.selectedServer + ", val2=" + val2 + " selected kingdom=" + this.selectedKingdom);
               ...
               logger.log(Level.WARNING, var29.getMessage() + ": " + val2);
            }
         }
      }
   }
      ...
      if (this.portal == null) {
         ...
         if (this.step == 1) {
            ...
            if (epicServerId > 0 && epicServerId != Servers.localServer.id) {
               data1 = epicServerId;
               ServerEntry entry = Servers.getServerWithId(epicServerId);
         ...
         if (entry != null) {
            ...
            if (this.getResponder().getPower() != 0
               ...
               || !entry.entryServer && !Servers.localServer.isChallengeServer() && (!entry.isChallengeServer() || Servers.localServer.entryServer)) {
               if (this.portal.isEpicPortal()) {
                  ...
                  if (entry.ISPAYMENT && !this.getResponder().isReallyPaying()) {
                     buf.append(
                        "text{text=\"Alas! A trifle stops you from entering the portal. You need to purchase some nice premium time in order to enter the portal.\"}"
                     );
                  } else {
                  ...
                  if (entry.isAvailable(this.getResponder().getPower(), this.getResponder().isReallyPaying())) {
                     ...
                     if (Servers.localServer.PVPSERVER != entry.PVPSERVER) {
                        ...
                        buf.append(
                           "text{text=\"You will not be able to use this portal. You must use an Epic Portal which you can build yourself using stones and logs.\"};"
                        );
                     } else if (targetKingdom == 3) {
                        ...
                        if (this.getResponder().getPower() <= 0) {
                           try {
                              ...
                              if (fs.getKnowledge(0.0) > 50.0) {
                                 buf.append("text{text=\"Your new group fighting skill will become " + fs.getKnowledge(0.0) * 0.95F + "!\"};");
                              }
                           ...
                           try {
                              ...
                              if (as.getKnowledge(0.0) > 50.0) {
                                 buf.append("text{text=\"Your new group archery skill will become " + as.getKnowledge(0.0) * 0.95F + "!\"};");
                              }
                     ...
                     if (!changingEpicCluster) {
                        ...
                        for(int x = 0; x < bodyItems.length; ++x) {
                           if (!bodyItems[x].willLeaveServer(false, changingCluster, this.getResponder().getPower() > 0)) {
                              ++stayBehind;
                              buf.append("text{text=\"The " + bodyItems[x].getName() + " will stay behind.\"};");
                              if (Servers.localServer.entryServer && bodyItems[x].getTemplateId() == 166) {
                                 buf.append("text{text=\"The structure will be destroyed.\"};");
                              }
                           }
                        }
                     ...
                     if (numitems > 200) {
                        buf.append(
                           "text{text=\"The portal seems to become unresponsive as you approach. You are carrying too much. Try removing "
                              + (numitems - 200)
                              + " items from body and inventory.\"};"
                        );
   ...
   public final void sendQuestion2(int portalNumber) {
      ...
      if (entries.size() == 0) {
         ...
         ServerEntry[] entryArr = entries.toArray(new ServerEntry[entries.size()]);
         Arrays.sort((Object[])entryArr);
      ...
      if (spawns != null && spawns.length > 0) {
         ...
         for(Spawnpoint spawn : spawns) {
            if (selectedKingdom != 0 && spawn.kingdom == selectedKingdom) {
               buf.append(
                  "radio{group=\"spawnpoint\";id=\"spawn"
                     + spawn.number
                     + "\"; text=\""
                     + spawn.name
                     + " ("
