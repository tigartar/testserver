               ...
               if (vills[x].getName().equalsIgnoreCase(cap)) {
                  if (vills[x].kingdom == this.getResponder().getKingdomId()) {
                     ...
                     if (!cap.equalsIgnoreCase(k.capital) && !k.setCapital(cap, false)) {
                        this.getResponder()
                           .getCommunicator()
                           .sendNormalServerMessage("You must wait 6 hours between capital changes. Also the server must have been up that long.");
                     }
                  } else {
   ...
   public void checkAlliances(Properties answers, Kingdom kingd) {
      ...
      if (val != null && val.length() > 0) {
         try {
            ...
            if (index > 0) {
               Kingdom kingdz = this.askedAllies.get(index - 1);
               if (!kingd.hasSentRequestingAlliance(kingdz.getId()) && !kingdz.hasSentRequestingAlliance(kingd.getId())) {
            ...
            logger.log(Level.WARNING, val + ": " + var12.getMessage(), (Throwable)var12);
         }
      }
   }
   ...
   public void append(byte kingdom, StringBuilder buf) {
      ...
      buf.append(
         "text{text=\""
            ...
            + mottwo
            + "' ("
            + twoDecimals.format((double)Zones.getPercentLandForKingdom(kingdom))
            + "% land)\"}"
      );
      if (k != null) {
         ...
         if (kingd != null && k.kingid == this.getResponder().getWurmId()) {
                     ...
                     if (v.pmkKickDate > 0L) {
                        ...
                        buf.append(
                           "checkbox{id=\"revRej"
                              ...
                              + Server.getTimeFor(v.pmkKickDate - System.currentTimeMillis())
                              + ". Remove kicking? \";selected=\"false\"};"
                        );
                     }
                  }
               }
            ...
            if (sperc == 100.0F) {
               ...
               buf.append(
                  "text{text=\"You have increased your kingdom share from "
                     + String.format("%.2f%%", k.startLand)
                     + " to "
                     + String.format("%.2f%%", k.currentLand)
                     + " of the available land, which is an increase by "
                     + String.format("%.2f%%", sperc - 100.0F)
                     + ".\"}"
               );
            } else {
               buf.append(
                  "text{text=\"Your share of the available lands has decreased from "
                     + String.format("%.2f%%", k.startLand)
                     + " to "
                     + String.format("%.2f%%", k.currentLand)
                     + " of the available land, so you have lost "
                     + String.format("%.2f%%", 100.0F - sperc)
                     + ".\"}"
               );
            }
            ...
            if (this.getResponder().getPower() >= 3) {
               buf.append(
                  "text{text=\" Challenges: " + k.getChallengeSize() + " Declined: " + k.getDeclinedChallengesNumber() + " Votes: " + k.getVotes() + ".\"}"
               );
            }
                     ...
                     if (kingdz.isAllied(kingdom)) {
                        ...
                        buf.append(
                           "checkbox{id=\"acc"
                              ...
                              + "\";text=\""
                              + kingdz.getName()
                              + " is asking to become an ally. Accept? \";selected=\"false\"};"
                        );
                        ...
                        buf.append(
                           "checkbox{id=\"rem"
                              ...
                              + "\";text=\" You are asking "
                              + kingdz.getName()
                              + " to become an ally. Revoke? \";selected=\"false\"};"
                        );
                     } else {
                        allAllies = false;
                     }
            ...
            if (sperc == 100.0F) {
               ...
               buf.append(
                  "text{text=\"During the reign of "
                     + k.kingName
                     + ", the share of the available lands has decreased from "
                     + String.format("%.2f%%", k.startLand)
                     + " to "
                     + String.format("%.2f%%", k.currentLand)
                     + " of the available land, so the kingdom has lost "
                     + String.format("%.2f%%", 100.0F - sperc)
                     + ".\"}"
               );
            }
