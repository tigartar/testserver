...
public class MeshTile implements MiscConstants {
   ...
   public int getLowerLip() {
      if (this.getNorthSlope() == 0 && this.getWestSlope() > 0 && this.getEastSlope() > 0) {
         return 0;
      } else if (this.getSouthSlope() == 0 && this.getWestSlope() < 0 && this.getEastSlope() < 0) {
         return 4;
      } else if (this.getWestSlope() == 0 && this.getNorthSlope() > 0 && this.getSouthSlope() > 0) {
         return 6;
      } else {
         return this.getEastSlope() == 0 && this.getNorthSlope() < 0 && this.getSouthSlope() < 0 ? 2 : -1;
      }
   }
   ...
   public boolean checkSlopes(int maxStraight, int maxDiagonal) {
      if (Math.abs(this.getNorthSlope()) <= maxStraight
         && Math.abs(this.getSouthSlope()) <= maxStraight
         && Math.abs(this.getEastSlope()) <= maxStraight
         && Math.abs(this.getWestSlope()) <= maxStraight) {
         int southEastTile = this.mesh.getTile(this.tilex + 1, this.tiley + 1);
         short heightSE = Tiles.decodeHeight(southEastTile);
   ...
   public boolean isUnder(int height) {
      if (this.currentHeight <= height) {
         return true;
      } else if (this.getSouthMeshTile().getTileHeight() <= height) {
         return true;
      } else if (this.getEastMeshTile().getTileHeight() <= height) {
         return true;
      } else {
