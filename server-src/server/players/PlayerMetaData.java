...
implements MiscConstants {
    ...
    private static final String INSERT_ENEMY = DbConnector.isUseSqlite() ? "INSERT OR IGNORE INTO ENEMIES(WURMID,ENEMY) VALUES(?,?)" : "INSERT IGNORE INTO ENEMIES(WURMID,ENEMY) VALUES(?,?)";
    ...
    public Map<Integer, Long> cooldowns = new HashMap<Integer, Long>();
    ...
    public void save() throws IOException {
        ...
        try {
            ...
            if (this.exists(dbcon)) {
                ps = dbcon.prepareStatement(DELETE_PLAYER);
            ...
            if (this.titleArr.length > 0) {
                for (x = 0; x < this.titleArr.length; ++x) {
                    Titles.Title t = Titles.Title.getTitle(this.titleArr[x]);
                    if (t == null) continue;
                    ps = dbcon.prepareStatement(INSERT_TITLE);
                    ps.setLong(1, this.wurmid);
            ...
            if (this.cooldowns.size() > 0) {
                ...
                for (Map.Entry<Integer, Long> ent : this.cooldowns.entrySet()) {
