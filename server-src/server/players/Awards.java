...
public class Awards {
   ...
   public Awards(
      ...
      int daysPremium,
      int _monthsPaidEver,
      int monthsPaidInSuccession,
      int _monthsPaidSinceReset,
      int silversPurchased,
      long _lastTickedDay,
      int _currentLoyalty,
      int _totalLoyalty,
      boolean createInDb
   ...
   public final String toString() {
      return "Awards for "
         ...
         + this.monthsPaidEver
         + ", mo's row="
         + this.monthsPaidInARow
         + ", mo's reset="
         + this.monthsPaidSinceReset
         + ", silvers="
         + this.silversPaidEver
         + ", loyalty="
         + this.currentLoyaltyPoints
         + ", totalLoyalty="
         + this.totalLoyaltyPoints
         + ", tick="
         + Server.getTimeFor(System.currentTimeMillis() - this.lastTickedDay)
         + " ago";
   }
   ...
   public final void save() {
      ...
      try {
         ...
         logger.log(Level.WARNING, this.wurmId + " " + var7.getMessage(), (Throwable)var7);
      } finally {
   ...
   public final void update() {
      ...
      try {
         dbcon = DbConnector.getPlayerDbCon();
         ps = dbcon.prepareStatement(
            "UPDATE AWARDS SET DAYSPREM=?, MONTHSPREM=?, MONTHSEVER=?, CONSECMONTHS=?, SILVERSPURCHASED=?, LASTTICKEDPREM=?, TOTALLOYALTY=?, CURRENTLOYALTY=? WHERE WURMID=?"
         );
