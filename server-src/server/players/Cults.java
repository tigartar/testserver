...
public final class Cults implements TimeConstants, MiscConstants {
   ...
   public static final byte getPathFor(int tilex, int tiley, int layer, int performerHeight) {
      byte path = 0;
      if (layer >= 0) {
         short[] steepness = Creature.getTileSteepness(tilex, tiley, layer >= 0);
         if (steepness[1] > 10) {
            return 0;
         }
      }

      int[][] closeTiles = getCloseTiles(tilex, tiley, layer);
      Map<Byte, Integer> dominatingType = new HashMap<>();
      ...
      for(int[] lCloseTile : closeTiles) {
         for(int y = 0; y < closeTiles.length; ++y) {
            boolean detectedWater = false;
            byte type = Tiles.decodeType(lCloseTile[y]);
            Tiles.Tile theTile = Tiles.getTile(type);
            Integer nums = dominatingType.get(type);
            if (nums == null) {
               nums = 0;
            }
            ...
            short height = Tiles.decodeHeight(lCloseTile[y]);
            ...
            if (type == Tiles.Tile.TILE_GRASS.id || type == Tiles.Tile.TILE_REED.id) {
               GrassData.FlowerType f = GrassData.FlowerType.decodeTileData(Tiles.decodeData(lCloseTile[y]));
               if (f != GrassData.FlowerType.NONE) {
                  ++flowerTiles;
               }
            } else if (theTile.isMyceliumBush() && theTile.isThorn(Tiles.decodeData(lCloseTile[y]))) {
               ++thornTiles;
            }
         }
      }
      ...
      if (layer >= 0) {
         ...
         for(Entry<Byte, Integer> e : dominatingType.entrySet()) {
         ...
         if (domType == Tiles.Tile.TILE_MOSS.id || domType == Tiles.Tile.TILE_CLAY.id || domType == Tiles.Tile.TILE_TUNDRA.id) {
            return 0;
         } else if (secondType == Tiles.Tile.TILE_MOSS.id || secondType == Tiles.Tile.TILE_CLAY.id || secondType == Tiles.Tile.TILE_TUNDRA.id) {
            return 0;
         } else if (performerHeight > 2000) {
            return (byte)(belowRockfall || domType != Tiles.Tile.TILE_ROCK.id && domType != Tiles.Tile.TILE_CLIFF.id && !quickDropoff ? 0 : 5);
         } else if (!Tiles.getTile(domType).isMycelium() || nearWater || !hateEnabler && thornTiles <= 3) {
            if (farWater && nearWater) {
            ...
            if (!nearWater && farWater && secondType == Tiles.Tile.TILE_GRASS.id && flowerTiles > 4) {
               path = 1;
            }
         ...
         return (byte)(!insanityBlocker ? 4 : 0);
      }
   }
   ...
   public static final boolean meditate(Creature performer, int layer, Action aAct, float counter, Item rug) {
      ...
      if (performer.getPositionZ() + performer.getAltOffZ() < 0.1F) {
         ...
         return true;
      } else if (!rug.isMeditation()) {
         performer.getCommunicator().sendNormalServerMessage("You can't meditate with that.");
         return true;
      } else {
         byte path = getPathFor(performer.getTileX(), performer.getTileY(), layer, (int)(performer.getPositionZ() * 10.0F));
         ...
         if (performer.getPower() == 0
            && Math.abs(performer.getMeditateX() - performer.getTileX()) < 10
            && Math.abs(performer.getMeditateY() - performer.getTileY()) < 10) {
            if (counter == 1.0F) {
         ...
         Cultist cultist = Cultist.getCultist(performer.getWurmId());
         ...
         if (counter == 1.0F) {
            ...
            if (performer.getPower() >= 5) {
               tosend = tosend + " Path is " + getPathNameFor(path);
            }
            ...
            if (meditation.skillCheck((double)difficulty, rug, 0.0, true, 1.0F + (float)rug.getRarity() * 0.1F) < 0.0) {
               performer.getCommunicator().sendNormalServerMessage("You fail to relax.");
               if (performer.getPower() == 5) {
                  performer.getCommunicator()
                     .sendNormalServerMessage("Difficulty=" + difficulty + " increaseSkill=" + increaseSkill + " cultist is null?" + (cultist == null));
               }
            ...
            performer.getCommunicator().sendNormalServerMessage("You fall into a trance. " + getMeditationString(path, level, tilex, tiley));
         }
         ...
         if (counter * 10.0F > (float)time) {
            ...
            if (meditation.skillCheck(95.0, rug, 0.0, !increaseSkill, 1.0F + (float)rug.getRarity() * 0.1F) > 0.0) {
               ...
               if (cultist != null) {
                  ...
                  if (timeToNextLevel <= 0L) {
                     if (!((double)(cultist.getLevel() * 10) - meditation.getKnowledge(0.0) < 30.0) && !(meditation.getKnowledge(0.0) > 90.0)) {
                        difficulty = (float)(1 + cultist.getLevel() * 3);
                        performer.getCommunicator()
                           .sendNormalServerMessage("You have a hard time to focus and no question comes to you. You probably need higher meditation skill.");
                     } else {
                        mayIncreaseLevel = true;
                        difficulty = (float)(1 + cultist.getLevel() * 10);
                     }
                  } else {
                     performer.getCommunicator().sendNormalServerMessage(getWaitForProgressMessage(timeToNextLevel));
                     if (performer.getPower() == 5) {
                        mayIncreaseLevel = true;
                        difficulty = (float)(1 + cultist.getLevel() * 10);
                        timeToNextLevel = 0L;
            ...
            power = meditation.skillCheck((double)difficulty, rug, 0.0, !increaseSkill, counter / 3.0F * (1.0F + (float)rug.getRarity() * 0.1F));
                  ...
                  if (power > 0.0) {
                     if (cultist == null) {
                        CultQuestion cq = new CultQuestion(
                           performer, "Following a path", "Do you wish to embark on a journey?", timeToNextLevel, null, path, false, false
                        );
                        cq.sendQuestion();
                     } else if (path == cultist.getPath()) {
                        if (cultist.getLevel() < 6 || performer.isPaying()) {
                           CultQuestion cq = new CultQuestion(
                              performer, "Following a path", "Are you ready to proceed?", timeToNextLevel, cultist, path, false, false
                           );
                           cq.sendQuestion();
                        }
                     } else {
   ...
   static String getWaitForProgressMessage(long timeToNextLevel) {
      String message;
      if (timeToNextLevel > 1814400000L) {
         message = "You still need to let your recent progress sink in for weeks to come.";
      } else if (timeToNextLevel > 1209600000L) {
         message = "You still need to let your recent progress sink in for a few weeks.";
      } else if (timeToNextLevel > 604800000L) {
         message = "You still need to let your recent progress sink in for more than a week.";
      } else if (timeToNextLevel > 259200000L) {
         message = "You still need to let your recent progress sink in for about a week.";
      } else if (timeToNextLevel > 86400000L) {
         message = "You still need to let your recent progress sink in for a few days.";
      } else if (timeToNextLevel > 43200000L) {
         message = "You still need to let your recent progress sink in for many hours.";
      } else if (timeToNextLevel > 10800000L) {
         message = "You still need to let your recent progress sink in for several hours.";
      } else if (timeToNextLevel > 0L) {
         message = "You still need to let your recent progress sink in for a few hours.";
      } else {
         message = "";
      }
   ...
   public static final String getMeditationString(byte path, byte level, int tilex, int tiley) {
      ...
      random.setSeed((long)((tilex << 16) + tiley));
      int max;
      int baseRand;
      int pathForMessage;
      ...
      switch(pathForMessage) {
         case 0:
            medString = getMeditationStringForNoPath(rand);
            break;
         case 1:
            medString = getMeditationStringForLovePath(rand);
            break;
         case 2:
            medString = getMeditationStringForHatePath(rand);
            break;
         case 3:
            medString = getMeditationStringForKnowledgePath(rand);
            break;
         case 4:
            medString = getMeditationStringForInsanityPath(rand);
            break;
         case 5:
            medString = getMeditationStringForPowerPath(rand);
            break;
         default:
   ...
   public static final String getNameForLevel(byte path, byte level) {
      ...
      switch(path) {
         case 1:
            name = getNameForLevelForLovePath(level);
            break;
         case 2:
            name = getNameForLevelForHatePath(level);
            break;
         case 3:
            name = getNameForLevelForKnowledgePath(level);
            break;
         case 4:
            name = getNameForLevelForInsanityPath(level);
            break;
         case 5:
            name = getNameForLevelForPowerPath(level);
            break;
         default:
            name = "uninitiated";
      }
   ...
   public static final String getQuestionForLevel(byte path, byte level) {
      ...
      switch(path) {
         case 1:
            question = getQuestionForLevelForLovePath(level);
            break;
         case 2:
            question = getQuestionForLevelForHatePath(level);
            break;
         case 3:
            question = getQuestionForLevelForKnowledgePath(level);
            break;
         case 4:
            question = getQuestionForLevelForInsanityPath(level);
            break;
         case 5:
            question = getQuestionForLevelForPowerPath(level);
            break;
         default:
   ...
   public static final String[] getAnswerAlternativesForLevel(byte path, byte level) {
      ...
      switch(path) {
         case 1:
            alts = getAnswerAlternativesForLevelForLovePath(level);
            break;
         case 2:
            alts = getAnswerAlternativesForLevelForHatePath(level);
            break;
         case 3:
            alts = getAnswerAlternativesForLevelForKnowledgePath(level);
            break;
         case 4:
            alts = getAnswerAlternativesForLevelForInsanityPath(level);
            break;
         case 5:
            alts = getAnswerAlternativesForLevelForPowerPath(level);
            break;
         default:
            alts = YES_NO_STRINGS;
      }
   ...
   static String[] getAnswerAlternativesForLevelForInsanityPath(byte level) {
      ...
      switch(level) {
         ...
         case 2:
            alts = new String[]{
               "The number on the ticket", "The person who pulled the ticket", "Simple luck", "The events leading up to the draw", "World history"
            };
            break;
         case 3:
         ...
         case 13:
            alts = new String[]{
               "are more envious and dangerous", "are more common and popular", "don't have to be protected", "are more trustworthy", "have more to lose"
            };
            break;
         case 14:
   ...
   static String[] getAnswerAlternativesForLevelForPowerPath(byte level) {
      ...
      switch(level) {
         ...
         case 1:
            alts = new String[]{
               "Money is mainly control", "You can buy everything with money", "Money is mainly oppression", "Money is mainly energy", "Money is only numbers"
            };
            break;
         case 2:
         ...
         case 3:
            alts = new String[]{
               "To push down their competitors", "To gain control over their lives", "To control others", "To become rich", "They act like animals"
            };
            break;
         case 4:
         ...
         case 7:
            alts = new String[]{
               "Remove their feeling of self sufficiency", "Terrorize them", "Give them what they want", "Give them limited power", "Ignore them"
            };
            break;
         case 8:
         ...
         case 12:
            alts = new String[]{
               "Being able to kill", "Controlling people", "Being very rich", "Being free of riches and dependancies", "Being impervious to personal attacks"
            };
            break;
         case 13:
            alts = new String[]{
               "Build a system of coherent ideology that can't be refuted",
   ...
   static String[] getAnswerAlternativesForLevelForHatePath(byte level) {
      ...
      switch(level) {
         ...
         case 10:
            alts = new String[]{
               "When my enemies are gone", "When I have power and knowledge to use it", "When I am calm inside", "When I can loose it at will", "Nobody can"
            };
            break;
         case 11:
            alts = new String[]{
               ...
               "Others will give you power and coins",
         ...
         case 12:
            alts = new String[]{
               "The enemy which doesn't pay me",
               "The enemy which tries to like me",
               "The enemy in my head",
               "The enemy on the way to the real enemy",
   ...
   static String[] getAnswerAlternativesForLevelForKnowledgePath(byte level) {
      ...
      switch(level) {
         case 0:
            alts = new String[]{
               "Everything I have seen", "Lies are the truth", "Commonly accepted facts", "Information that I have accepted as a fact", "Applied experience"
            };
            break;
         case 1:
            alts = new String[]{
               "Experience", "Anything I accept as the truth", "Logically verified information", "I trust nothing", "What a lot of people say"
            };
            break;
         case 2:
         ...
         case 3:
            alts = new String[]{
               "I accept nothing as fact",
               "I test facts the first time I encounter them",
               "I only accept facts from my own experiences",
               "I do not trust my senses but other sources of information",
         ...
         case 5:
            alts = new String[]{
               ...
               "Someone who is paid",
               "Someone who is extremely skillful",
               "Someone who knows a bit more than average",
               "Someone worthy of respect"
            };
            break;
         case 6:
            alts = new String[]{
               "To really learn the details", "To buy new clothes", "To relax and take it easy", "To know a little about everything", "To talk to friends"
            };
            break;
         case 7:
         ...
         case 8:
            alts = new String[]{
               "It can help", "It is very important", "You should not be allowed to make decisions without it", "The history is useless", "I don't know"
            };
            break;
         case 9:
   ...
   public static final String getWrongAnswerStringForLevel(byte path, byte level) {
      ...
      switch(path) {
         case 1:
            wrongAnswer = getWrongAnswerStringForLevelForLovePath(level);
            break;
         case 2:
            wrongAnswer = getWrongAnswerStringForLevelForHatePath(level);
            break;
         case 3:
            wrongAnswer = getWrongAnswerStringForLevelForKnowledgePath(level);
            break;
         case 4:
            wrongAnswer = getWrongAnswerStringForLevelForInsanityPath(level);
            break;
         case 5:
            wrongAnswer = getWrongAnswerStringForLevelForPowerPath(level);
            break;
         default:
   ...
   public static final int getCorrectAnswerForNextLevel(byte path, byte level) {
      ...
      switch(path) {
         case 1:
            answerForNextLevel = getCorrectAnswerForNextLevelForLovePath(level);
            break;
         case 2:
            answerForNextLevel = getCorrectAnswerForNextLevelForHatePath(level);
            break;
         case 3:
            answerForNextLevel = getCorrectAnswerForNextLevelForKnowledgePath(level);
            break;
         case 4:
            answerForNextLevel = getCorrectAnswerForNextLevelForInsanityPath(level);
            break;
         case 5:
            answerForNextLevel = getCorrectAnswerForNextLevelForPowerPath(level);
            break;
         default:
   ...
   public static final String getCorrectAnswerStringForNextLevel(byte path, byte level) {
      ...
      switch(path) {
         case 1:
            answerForNextLevel = getCorrectAnswerStringForNextLevelForLovePath(level);
            break;
         case 2:
            answerForNextLevel = getCorrectAnswerStringForNextLevelForHatePath(level);
            break;
         case 3:
            answerForNextLevel = getCorrectAnswerStringForNextLevelForKnowledgePath(level);
            break;
         case 4:
            answerForNextLevel = getCorrectAnswerStringForNextLevelForInsanityPath(level);
            break;
         case 5:
            answerForNextLevel = getCorrectAnswerStringForNextLevelForPowerPath(level);
            break;
         default:
