...
extends PlayerCommunicator {
    ...
    private static final BlockingQueue<PlayerMessage> MESSAGES_TO_PLAYERS = new LinkedBlockingQueue<PlayerMessage>();
    ...
    public void sendMessage(Message message) {
        if (this.player.hasLink()) {
            try {
                ...
                ByteBuffer bb = this.getBuffer(Short.MAX_VALUE);
                bb.put((byte)99);
                ...
                logger.log(Level.INFO, this.player.getName() + " could not send a message '" + message + "' due to : " + ex.getMessage(), ex);
        ...
        try {
            if (Players.getInstance().getPlayer(this.player.getWurmId()) != null) {
                ...
                if (logger.isLoggable(Level.FINEST)) {
                    logger.finest("Added " + lPlayerMessage + " message with " + aByteBuffer.position() + " bytes to MESSAGES_TO_PLAYERS, queue size: " + MESSAGES_TO_PLAYERS.size());
                }
            }
        }
        catch (NoSuchPlayerException e) {
            logger.log(Level.WARNING, "Player is not in Players map so could not add " + lPlayerMessage + " message with " + aByteBuffer.position() + " bytes to MESSAGES_TO_PLAYERS - " + e.getMessage(), e);
        }
    }
    ...
    public void sendGmMessage(long time, String sender, String message) {
        if (this.player.hasLink()) {
            try {
                ...
                logger.log(Level.INFO, this.player.getName() + " could not send a GM message '" + message + "' due to : " + ex.getMessage(), ex);
                this.player.setLink(false);
            }
        }
    }
            ...
            try {
                ...
                if (item.isLockable() && item.getLockId() != -10L) {
                    try {
                        ...
                        logger.log(Level.WARNING, item.getWurmId() + " has lock " + item.getLockId() + " but that doesn't exist." + nsi.getMessage(), nsi);
                    }
                }
    ...
    public void sendNewCreature(long id, String name, String model, float x, float y, float z, long onBridge, float rot, byte layer, boolean onGround, boolean floating, boolean isSolid, byte kingdomId, long face, byte blood, boolean isUndead, boolean isCopy, byte modtype) {
        if (this.player.hasLink()) {
            try {
                ...
                bb.putLong(face);
                if ((WurmId.getType(id) == 0 || isCopy) && !isUndead) {
                    bb.putInt(Math.abs(PlayerCommunicatorQueued.generateSoundSourceId(id)));
                }
                bb.put(blood);
                bb.put(modtype);
                bb.put((byte)0);
    ...
    public void sendAvailableActions(byte requestId, List<ActionEntry> availableActions, String helpstring) {
        if (this.player.hasLink()) {
            try {
                if (logger.isLoggable(Level.FINEST)) {
                    logger.finest(this.player.getName() + ", sending # of available Actions: " + availableActions.size() + ", requestId: " + String.valueOf(requestId) + ", availableActions: " + availableActions + ", helpstring: " + helpstring);
                } else if (logger.isLoggable(Level.FINER)) {
                    logger.finer(this.player.getName() + ", sending # of available Actions: " + availableActions.size() + " , requestId: " + String.valueOf(requestId) + ", helpstring: " + helpstring);
                }
                ByteBuffer bb = this.getBuffer(Short.MAX_VALUE);
                bb.put((byte)20);
                bb.put(requestId);
    ...
    public void sendItem(Item item, long creatureId, boolean onGroundLevel) {
        if (this.player.hasLink()) {
            try {
                ...
                logger.log(Level.WARNING, "Failed to send item: " + this.player.getName() + ":" + item.getWurmId() + ", " + ex.getMessage(), ex);
                this.player.setLink(false);
            }
        }
    }
    ...
    public void sendRename(Item item, String newName, String newModelName) {
        if (this.player.hasLink()) {
            try {
                ...
                logger.log(Level.WARNING, "Failed to rename item: " + this.player.getName() + ":" + item.getWurmId() + ", " + ex.getMessage(), ex);
                this.player.setLink(false);
            }
        }
    }
    ...
    public void sendActionControl(long creatureId, String actionString, boolean start, int timeLeft) {
        if (this.player.hasLink()) {
            try {
                byte[] byteArray = EMPTY_STRING.getBytes("UTF-8");
                if (start) {
    ...
    protected void sendStamina(int stamina, int damage) {
        if (this.player.hasLink() && !this.player.isTransferring()) {
            try {
                ...
                bb.put((byte)90);
                short lStamina = (short)((long)(stamina & 0xFFFE) | this.newSeed >> this.newSeedPointer++ & 1L);
                bb.putShort(lStamina);
                bb.putShort((short)damage);
                this.addMessageToQueue(bb);
                if (this.newSeedPointer == 32) {
                    this.getConnection().encryptRandom.setSeed(this.newSeed & 0xFFFFFFFFFFFFFFFFL);
    ...
    public void sendPassable(boolean passable, Door door) {
        if (this.player.hasLink()) {
            try {
                ...
                logger.log(Level.WARNING, this.player.getName() + ": Trying to make door passable for wall with no id! Structure=" + door.getStructureId(), nsw);
                return;
            }
            catch (Exception ex) {
                logger.log(Level.INFO, this.player.getName() + ":" + ex.getMessage(), ex);
    ...
    public void sendOpenDoor(Door door) {
        if (this.player.hasLink()) {
            try {
                ...
                logger.log(Level.WARNING, this.player.getName() + ": trying to open door for wall with no id! Structure=" + door.getStructureId(), nsw);
                return;
            }
            catch (Exception ex) {
                logger.log(Level.INFO, this.player.getName() + ":" + ex.getMessage(), ex);
    ...
    public void sendCloseDoor(Door door) {
        if (this.player.hasLink()) {
            try {
                ...
                logger.log(Level.WARNING, this.player.getName() + ": trying to close door for wall with no id! Structure=" + door.getStructureId(), nsw);
                return;
            }
            catch (Exception ex) {
                logger.log(Level.INFO, this.player.getName() + ":" + ex.getMessage(), ex);
    ...
    public void sendToggle(int toggle, boolean set) {
        if (this.player.hasLink()) {
            try {
                ...
                logger.log(Level.INFO, "Problem sending toggle (" + toggle + ',' + set + ") to " + this.player.getName() + " due to :" + ex.getMessage(), ex);
                this.player.setLink(false);
            }
        }
    }
    ...
    public void sendTeleport(boolean aLocal, boolean disembark, byte commandType) {
        if (this.player.hasLink()) {
            try {
                ...
                logger.log(Level.INFO, "Problem sending teleport (local: " + aLocal + ") to " + this.player.getName() + " due to :" + ex.getMessage(), ex);
                this.player.setLink(false);
            }
        }
    }
    ...
    public void sendAddWound(Wound wound, Item bodyPart) {
        if (this.player.hasLink()) {
            try {
                ...
                bb.putFloat(100.0f);
                if (logger.isLoggable(Level.FINER)) {
                    logger.finer("Sending wound ID: " + wound.getWurmId() + ", severity: " + wound.getSeverity() + "*" + 0.0015259022f + "=" + wound.getSeverity() * 0.0015259022f);
                }
