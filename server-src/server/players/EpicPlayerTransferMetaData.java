...
public class EpicPlayerTransferMetaData {
   ...
   public EpicPlayerTransferMetaData(
      ...
      boolean islogged,
      boolean _isCA,
      boolean mayAppointCA,
      long _referrer,
      String _pwQuestion,
      String _pwAnswer,
      boolean _overRideShop,
      short _muteTimes,
      long muteExpiry,
      String muteReason,
      boolean mayMute,
      ...
      int aMonthsPaidEver,
      int aMonthsPaidInARow,
      int aMonthsPaidSinceReset,
      int aSilverPaidEver,
      boolean awards
   ) {
      this.wurmid = aWurmid;
   ...
   public void save() throws IOException {
      ...
      try {
         ...
         ps = dbcon.prepareStatement(
            "UPDATE PLAYERS SET NAME=?, PASSWORD=?, MAYUSESHOP=?, PLAYINGTIME=?, SEX=?, IPADDRESS=?, REIMBURSED=?, BANNED=?, PAYMENTEXPIRE=?, POWER=?,    DEVTALK=?, WARNINGS=?, LASTWARNED=?, KINGDOM=?, SESSIONKEY=?, SESSIONEXPIRE=?, VERSION=?, MUTED=?, MONEY=?, BANEXPIRY=?,        BANREASON=?, FACE=?, LOGGING=?, MAYMUTE=?, MUTEEXPIRY=?, MUTEREASON=?, REFERRER=?, EMAIL=?, PWQUESTION=?, PWANSWER=?,        CREATIONDATE=?, PA=?, APPOINTPA=?, PAWINDOW=?, MUTETIMES=?, EPICKINGDOM=?, EPICSERVER=?, CURRENTSERVER=?, CHAOSKINGDOM=?, BLOOD=?,       FLAGS=?, FLAGS2=?, MONEYSALES=?     WHERE WURMID=?"
         );
         ...
         logger.log(Level.WARNING, this.name + " " + var29.getMessage(), (Throwable)var29);
      ...
      if (this.hasAwards) {
         try {
            ...
            logger.log(Level.WARNING, this.name + " " + var27.getMessage(), (Throwable)var27);
         ...
         try {
            ...
            logger.log(Level.WARNING, this.name + " " + var25.getMessage(), (Throwable)var25);
