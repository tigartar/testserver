...
public final class FaithZone {
   ...
   public int getStrengthForTile(int tileX, int tileY, boolean surfaced) {
      return (int)(
         this.altar.getCurrentQualityLevel() / (float)(surfaced == this.altar.isOnSurface() ? 1 : 2)
            - (float)Math.max(Math.abs(this.altar.getTileX() - tileX), Math.abs(this.altar.getTileY() - tileY))
      );
   }
   ...
   public void pollMycelium() {
      if (this.getCurrentRuler() != null) {
         if (this.getCurrentRuler().number == 4 && Server.rand.nextInt(50) == 0) {
            int x = this.startX + Server.rand.nextInt(this.endX - this.startX);
            int y = this.startY + Server.rand.nextInt(this.endY - this.startY);
