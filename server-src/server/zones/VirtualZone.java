...
public final class VirtualZone implements MiscConstants, CounterTypes, AttitudeConstants, ProtoConstants, TimeConstants {
      ...
      if (!this.watcher.isDead()) {
         ...
         for(int i = 0; i < this.watchedZones.length; ++i) {
            try {
               ...
               logger.log(Level.INFO, var3.getMessage(), (Throwable)var3);
            }
         }
      }
   }
   ...
   public void stopWatching() {
      Zone[] checkedZones = Zones.getZonesCoveredBy(
         Math.max(0, this.startX - 100),
         Math.max(0, this.startY - 100),
         Math.min(Zones.worldTileSizeX - 1, this.endX + 100),
         Math.min(Zones.worldTileSizeY - 1, this.endY + 100),
         this.isOnSurface
      );
   ...
   public Long[] getCreatures() {
      return this.creatures != null ? this.creatures.keySet().toArray(new Long[this.creatures.size()]) : emptyLongArray;
   }

   public boolean containsCreature(Creature creature) {
      return this.creatures != null ? this.creatures.keySet().contains(creature.getWurmId()) : false;
   }
   ...
   private byte getLayer() {
      return (byte)(this.isOnSurface() ? 0 : -1);
   }
   ...
   public final boolean addCreature(long creatureId, boolean overRideRange, long copyId, float offx, float offy, float offz) throws NoSuchCreatureException, NoSuchPlayerException {
      ...
      if (!this.coversCreature(creature) && (this.watcher == null || !this.watcher.isPlayer() || !overRideRange)) {
         ...
         && this.watcher.getWurmId() != creatureId
         && creature.isVisibleTo(this.watcher)
         && (!this.watcher.isPlayer() || this.watcher.hasLink())) {
         ...
         if (this.watcher.hasLink()) {
            ...
            if (creature.getPower() > 0 && !Servers.localServer.testServer) {
               ...
               if (this.watcher.getKingdomId() != 0 && creature.getKingdomId() != 0 && !creature.isFriendlyKingdom(this.watcher.getKingdomId())) {
                  ...
                  && (creature.getTitle() != null || Features.Feature.COMPOUND_TITLES.isEnabled() && creature.getSecondTitle() != null)
                  && !creature.getTitleString().isEmpty()) {
                  suff = suff + " [";
            ...
            if (enemy
               && creature.getPower() < 2
               && this.watcher.getPower() < 2
               && creature.isPlayer()
               && (creature.getFightingSkill().getRealKnowledge() > 20.0 || creature.getFaith() > 25.0F)
            ...
            this.watcher
               .getCommunicator()
               .sendNewCreature(
                  copyId != -10L ? copyId : creatureId,
                  pre + StringUtilities.raiseFirstLetterOnly(creature.getName()) + suff,
                  hoverText,
                  creature.isUndead() ? creature.getUndeadModelName() : creature.getModelName(),
                  creature.getStatus().getPositionX() + offx,
                  creature.getStatus().getPositionY() + offy,
                  creature.getStatus().getPositionZ() + offz,
                  creature.getStatus().getBridgeId(),
                  creature.getStatus().getRotation(),
                  layer,
                  creature.getBridgeId() <= 0L
                     && !creature.isSubmerged()
                     && (creature.getPower() == 0 || creature.getMovementScheme().onGround)
                     && creature.getFloorLevel() <= 0
                     && creature.getMovementScheme().getGroundOffset() <= 0.0F,
                  false,
                  creature.getTemplate().getTemplateId() != 119,
                  creature.getKingdomId(),
                  creature.getFace(),
                  creature.getBlood(),
                  creature.isUndead(),
                  copyId != -10L || creature.isNpc(),
                  creature.getStatus().getModType()
               );
               ...
               if (item != null) {
                  try {
                     ...
                     if (creature.isAnimal() && creature.isVehicle()) {
                        this.watcher
                           .getCommunicator()
                           .sendHorseWear(creature.getWurmId(), item.getTemplateId(), item.getMaterial(), armorSlot, item.getAuxData());
                     } else {
                        this.watcher
                           .getCommunicator()
                           .sendWearItem(
                              copyId != -10L ? copyId : creature.getWurmId(),
                              item.getTemplateId(),
                              armorSlot,
            ...
            if (creature.hasCustomColor()) {
               this.sendRepaint(
                  copyId != -10L ? copyId : creatureId,
                  creature.getColorRed(),
                  creature.getColorGreen(),
                  creature.getColorBlue(),
                  (byte)-1,
                  (byte)creature.getPaintMode()
               );
            }
            ...
            if (creature.isGhost()) {
               this.watcher
                  .getCommunicator()
                  .sendAttachEffect(
                     creature.getWurmId(), (byte)2, (byte)(creature.isSpiritGuard() ? -56 : 100), (byte)(creature.isSpiritGuard() ? 1 : 1), (byte)0, (byte)1
                  );
               this.watcher
                  .getCommunicator()
                  .sendAttachEffect(creature.getWurmId(), (byte)3, (byte)50, (byte)(creature.isSpiritGuard() ? 50 : 50), (byte)50, (byte)1);
               ...
               if (creature.hasCustomColor()) {
                  ...
                  this.watcher
                     .getCommunicator()
                     .sendAttachEffect(creature.getWurmId(), (byte)3, creature.getColorRed(), creature.getColorGreen(), creature.getColorGreen(), (byte)1);
               }
            }
         ...
         if (this.watcher.isTypeFleeing()
            && (creature.isPlayer() || creature.isAggHuman() || creature.isHuman() || creature.isCarnivore() || creature.isMonster())) {
                  ...
                  if (seats[x].isOccupied()) {
                     if (!this.creatures.containsKey(seats[x].occupant)) {
                        try {
                           ...
                           logger.log(Level.INFO, var20.getMessage(), (Throwable)var20);
                        } catch (NoSuchPlayerException var21) {
                           logger.log(Level.INFO, var21.getMessage(), (Throwable)var21);
                        }
                     }
      ...
      if (creature.isPlayer()) {
         try {
            ...
            if (lTempItem != null && !lTempItem.isBodyPartAttached()) {
               this.sendWieldItem(
                  creature.getWurmId() == this.watcher.getWurmId() ? -1L : creature.getWurmId(),
                  (byte)0,
                  lTempItem.getModelName(),
                  lTempItem.getRarity(),
                  WurmColor.getColorRed(lTempItem.getColor()),
                  WurmColor.getColorGreen(lTempItem.getColor()),
                  WurmColor.getColorBlue(lTempItem.getColor()),
                  WurmColor.getColorRed(lTempItem.getColor2()),
                  WurmColor.getColorGreen(lTempItem.getColor2()),
                  WurmColor.getColorBlue(lTempItem.getColor2())
               );
            }
         } catch (NoSpaceException var4) {
            logger.log(Level.WARNING, creature.getName() + " could not get equipped weapon for left hand due to " + var4.getMessage(), (Throwable)var4);
         }
         ...
         try {
            ...
            logger.log(Level.WARNING, creature.getName() + " could not get equipped weapon for right hand due to " + var3.getMessage(), (Throwable)var3);
         }
      }
   }
   ...
   void newLayer(Creature creature, boolean tileIsSurfaced) {
      if (creature != null && this.watcher.getWurmId() != creature.getWurmId()) {
         ...
         && this.watcher.getWurmId() == creature.getWurmId()
         && this.watcher.getVehicle() != -10L
         && !this.watcher.isVehicleCommander()) {
   ...
   public void addToVisionArea(Creature creature, CreatureMove cm, VirtualZone newzone) {
      ...
      if (creature.isRidden()) {
         for(Long rider : creature.getRiders()) {
            cm = this.creatures.remove(rider);
      ...
      if (vehicle != null && this.items != null && this.items.contains(vehicle)) {
         ...
         if (newlayer < 0) {
            if (this.watcher.getVisionArea().getUnderGround() != null
               && this.watcher.getVisionArea().getUnderGround().covers(vehicle.getTileX(), vehicle.getTileY())) {
         ...
         if (!creature.isTransferring()) {
            if (this.watcher.isPlayer()) {
               if (creature.addingAfterTeleport
                  && this.watcher.lastOpponent == creature
                  && creature.isWithinDistanceTo(
                     this.watcher.getPosX(), this.watcher.getPosY(), this.watcher.getPositionZ() + this.watcher.getAltOffZ(), 12.0F
                  )) {
                  this.watcher.setTarget(creatureId, false);
               }
            } else if (!creature.isNpc() || !this.watcher.isNpc() || creature.getAttitude(this.watcher) == 2) {
               if (creature.getLayer() == this.watcher.getLayer() || this.watcher.isKingdomGuard() || this.watcher.isUnique() || this.watcher.isWarGuard()) {
                  ...
                  if (!creature.isWithinDistanceTo(
                        this.watcher.getPosX(),
                        this.watcher.getPosY(),
                        this.watcher.getPositionZ() + this.watcher.getAltOffZ(),
                        !this.watcher.isSpiritGuard() && !this.watcher.isKingdomGuard() && !this.watcher.isWarGuard() && !this.watcher.isUnique()
                           ? 12.0F
                           : 30.0F
                     )
                     && !isCreatureTurnedTowardsTarget(creature, this.watcher)
                     && !this.watcher.isKingdomGuard()
                  ...
                  if (this.watcher.getAttitude(creature) == 2) {
                     if (this.watcher.target == -10L) {
                        ...
                        if (this.watcher.isWarGuard()) {
                           ...
                           if (target != null
                              && this.watcher.isWithinTileDistanceTo(target.getTileX(), target.getTileY(), 0, 15)
                              && creature.isWithinTileDistanceTo(target.getTileX(), target.getTileY(), 0, 5)) {
                              this.watcher.setTarget(creatureId, false);
                              return;
                           }
                        ...
                        if (creature instanceof Player && this.watcher.isAggHuman()) {
                           ...
                           if (creature.getSpellEffects() != null
                              ...
                              && !creature.isWithinDistanceTo(this.watcher, 7.0F)) {
                              return;
                           }

                           if (creature.addingAfterTeleport
                              || (float)Server.rand.nextInt(100) <= (float)this.watcher.getAggressivity() * this.watcher.getStatus().getAggTypeModifier()) {
                              this.watcher.setTarget(creatureId, false);
                           }
                        } else if (this.watcher.isAggHuman() && creature.isKingdomGuard()) {
                           if (creature.addingAfterTeleport
                              || creature.getAlertSeconds() > 0
                                 && Server.rand
                                       .nextInt((int)Math.max(1.0F, (float)this.watcher.getAggressivity() * this.watcher.getStatus().getAggTypeModifier()))
                                    == 0) {
                              this.watcher.setTarget(creatureId, false);
                           }
                           ...
                           if (this.watcher.getCitizenVillage() == null) {
                              ...
                              && this.watcher.getCitizenVillage().isWithinAttackPerimeter(creature.getTileX(), creature.getTileY())) {
                              if (!this.watcher.getCitizenVillage().isEnemy(creature)) {
                                 return;
                              }
                     ...
                     && this.watcher.leader == null
                     && !this.watcher.isDominated()
                     && creature.getTemplate().getTemplateId() == this.watcher.getTemplate().getLeaderTemplateId()
                     && (!this.watcher.isHerbivore() || !this.watcher.isHungry())
                     && ((double)creature.getPositionZ() >= -0.71 || creature.isSwimming() && this.watcher.isSwimming())
                     && creature.mayLeadMoreCreatures()) {
      ...
      if (effect != null) {
         ...
         if (!this.areaEffects.contains(effect)) {
            ...
            if (this.watcher.hasLink()) {
               this.watcher
                  .getCommunicator()
                  .sendAddAreaSpellEffect(
                     effect.getTilex(), effect.getTiley(), effect.getLayer(), effect.getType(), effect.getFloorLevel(), effect.getHeightOffset(), loop
                  );
            }
         }
      }
   ...
   public boolean checkIfEnemyIsPresent(boolean checkedFromOtherVirtualZone) {
      if (this.watcher.isPlayer()
         && !this.watcher.isTeleporting()
         && this.watcher.hasLink()
         && this.watcher.getVisionArea() != null
         && this.watcher.getVisionArea().isInitialized()) {
            ...
            for(int x = 0; x < crets.length; ++x) {
               if (WurmId.getType(crets[x]) == 0) {
                  try {
                     ...
                     } else if (this.watcher.getCitizenVillage() != null
                        && this.watcher.getCitizenVillage().isEnemy(creature)
                        && creature.getPower() < 2
                        && this.watcher.getPower() < 2
                        && creature.getFightingSkill().getRealKnowledge() > 20.0) {
                        foundEnemy = true;
                     }
   ...
   private boolean coversCreature(Creature creature) {
      if (creature.isDead()) {
         return false;
      } else if (creature == this.watcher) {
         return true;
      } else if (creature.isPlayer() && Servers.localServer.PVPSERVER) {
         if (this.watcher.isOnSurface() && creature.isOnSurface() && this.watcher.isWithinDistanceTo(creature.getTileX(), creature.getTileY(), 80)) {
            return true;
         } else if (!this.watcher.isOnSurface()
            && creature.isOnSurface()
            && this.watcher.isWithinDistanceTo(creature.getTileX(), creature.getTileY(), caveToSurfaceLocalDistance)) {
            return true;
         } else if (this.watcher.isOnSurface() && !creature.isOnSurface() && this.watcher.isWithinDistanceTo(creature.getTileX(), creature.getTileY(), 20)) {
            return true;
         } else {
   ...
   public void moveAllCreatures() {
      Entry<Long, CreatureMove>[] arr = this.creatures.entrySet().toArray(new Entry[this.creatures.size()]);

      for(int x = 0; x < arr.length; ++x) {
         if (arr[x].getValue().timestamp != 0L) {
            try {
               Creature creature = Server.getInstance().getCreature(arr[x].getKey());
               if (isMovingSameLevel(creature) && !Structure.isGroundFloorAtPosition(creature.getPosX(), creature.getPosY(), creature.isOnSurface())) {
                  this.watcher
                     .getCommunicator()
                     .sendMoveCreature(arr[x].getKey(), creature.getPosX(), creature.getPosY(), arr[x].getValue().rotation, creature.isMoving());
               } else {
                  this.watcher
                     .getCommunicator()
                     .sendMoveCreatureAndSetZ(arr[x].getKey(), creature.getPosX(), creature.getPosY(), creature.getPositionZ(), arr[x].getValue().rotation);
               }
   ...
   private static final boolean isMovingSameLevel(Creature creature) {
      return creature.getBridgeId() <= 0L
         && (creature.isPlayer() && creature.getMovementScheme().onGround || !creature.isSubmerged())
         && creature.getFloorLevel() <= 0
         && creature.getMovementScheme().getGroundOffset() == 0.0F;
   }
   ...
   boolean creatureMoved(long creatureId, float diffX, float diffY, float diffZ, int diffTileX, int diffTileY) throws NoSuchCreatureException, NoSuchPlayerException {
      if (this.watcher != null && (!this.watcher.isPlayer() || this.watcher.hasLink())) {
         ...
         if (this.watcher.equals(creature)) {
            ...
            return false;
         } else if (!this.coversCreature(creature)) {
            this.removeCreature(creature);
            return false;
         } else if (!creature.isVisibleTo(this.watcher)) {
            return false;
         } else {
            if (!this.addCreature(creatureId, false)) {
               if (!this.watcher.isPlayer()) {
                     ...
                     if (illusions != null) {
                        for(MovementEntity e : illusions) {
                           this.watcher
                              ...
                              .sendMoveCreature(
                                 e.getWurmid(),
                                 creature.getPosX() + e.getMovePosition().diffX,
                                 creature.getPosY() + e.getMovePosition().diffY,
                  ...
                  if (diffX == 0.0F && diffY == 0.0F && diffZ == 0.0F) {
                     ...
                     if (!(Math.abs(cmove.diffX + diffX) > this.MOVELIMIT)
                        && !(Math.abs(cmove.diffY + diffY) > this.MOVELIMIT)
                        && !(Math.abs(cmove.diffZ + diffZ) > this.MOVELIMIT)) {
                        if (creature.getAttitude(this.watcher) != 2 && !(Math.abs(diffZ) > 0.3F)) {
                           cmove.timestamp = System.currentTimeMillis();
                           cmove.diffX += diffX;
                           cmove.diffY += diffY;
                           cmove.diffZ += diffZ;
                        ...
                        if (moveSameLevel && !Structure.isGroundFloorAtPosition(creature.getPosX(), creature.getPosY(), creature.isOnSurface())) {
                           this.watcher
                              .getCommunicator()
                              .sendMoveCreature(creatureId, creature.getPosX(), creature.getPosY(), cmove.rotation, creature.isMoving());
                        } else {
                           this.watcher
                              .getCommunicator()
                              .sendMoveCreatureAndSetZ(creatureId, creature.getPosX(), creature.getPosY(), creature.getPositionZ(), cmove.rotation);
                        }
   ...
   private void checkNewZone() {
      if (this.size > 0) {
         ...
         ListIterator<Zone> it = newZones.listIterator();
         ...
         while(it.hasNext()) {
            ...
            ListIterator<Zone> it2 = oldZones.listIterator();
         ...
         for(Zone toRemove : oldZones) {
            try {
               ...
               logger.log(Level.WARNING, "Zone with id does not exist!", (Throwable)var8);
            }
         }
      ...
      if (this.watcher == null) {
         ...
         if (removeAsTarget) {
            ...
            if (creature.isTeleporting()) {
               removeTarget = Math.abs(this.watcher.getPosX() - creature.getTeleportX()) > 20.0F
                  || Math.abs(this.watcher.getPosY() - creature.getTeleportY()) > 20.0F;
            } else if (this.watcher.isTeleporting()) {
               removeTarget = Math.abs(creature.getPosX() - this.watcher.getTeleportX()) > 20.0F
                  || Math.abs(creature.getPosY() - this.watcher.getTeleportY()) > 20.0F;
            }
            ...
            if (removeTarget) {
               if (creature.getWurmId() == this.watcher.target
                  && (
                     this.watcher.getVisionArea() == null
                        || this.watcher.getVisionArea().getSurface() == null
               ...
               if (creature.target == this.watcher.getWurmId()
                  && (
                     creature.getVisionArea() == null
                        || creature.getVisionArea().getSurface() == null
   ...
   private final Structure getStructureAtWatcherPosition() {
      try {
         ...
         logger.log(Level.WARNING, "Unable to find the zone at the watchers tile position.", (Throwable)var3);
         return null;
      }
   }
         ...
         if (this.watcher.hasLink()) {
            if (item.isMovingItem()) {
               ...
               this.watcher
                  ...
                  .sendNewMovingItem(
                     ...
                     item.isFloating() && item.getCurrentQualityLevel() >= 10.0F,
                     true,
                     item.getMaterial(),
                     item.getRarity()
                  );
               ...
               if (item.isWarTarget()) {
                  this.watcher
                     .getCommunicator()
                     .sendAddEffect(item.getWurmId(), (short)24, item.getPosX(), item.getPosY(), (float)item.getData1(), (byte)(item.isOnSurface() ? 0 : -1));
                  this.watcher.getCommunicator().sendTargetStatus(item.getWurmId(), (byte)item.getData2(), (float)item.getData1());
               }
                  ...
                  for(Item i : item.getItems()) {
                     if (!normalContainer || i.isPlacedOnParent()) {
                        ...
                        if (i.getColor() != -1) {
                           this.sendRepaint(
                              i.getWurmId(),
                              (byte)WurmColor.getColorRed(i.getColor()),
                              (byte)WurmColor.getColorGreen(i.getColor()),
                              (byte)WurmColor.getColorBlue(i.getColor()),
                              (byte)-1,
                              (byte)0
                           );
                        }
                        ...
                        if (i.getColor2() != -1) {
                           this.sendRepaint(
                              i.getWurmId(),
                              (byte)WurmColor.getColorRed(i.getColor2()),
                              (byte)WurmColor.getColorGreen(i.getColor2()),
                              (byte)WurmColor.getColorBlue(i.getColor2()),
                              (byte)-1,
                              (byte)1
                           );
                        }
            ...
            if (item.getExtra() != -1L && (item.getTemplateId() == 491 || item.getTemplateId() == 490)) {
               ...
               if (extraItem.isPresent()) {
                  this.sendBoatAttachment(
                     item.getWurmId(), extraItem.get().getTemplateId(), extraItem.get().getMaterial(), (byte)1, extraItem.get().getAuxData()
                  );
               }
            }
         }
   ...
   void addStructure(Structure structure) {
      ...
      if (!this.structures.contains(structure)) {
         ...
         this.watcher
            ...
            .sendAddStructure(
               structure.getName(),
               (short)structure.getCenterX(),
               (short)structure.getCenterY(),
               structure.getWurmId(),
            ...
            for(int x = 0; x < wallArr.length; ++x) {
               if (wallArr[x].getType() != StructureTypeEnum.PLAN) {
                  ...
                  if (wallArr[x].getDamage() >= 60.0F) {
                     this.watcher.getCommunicator().sendWallDamageState(structure.getWurmId(), wallArr[x].getId(), (byte)((int)wallArr[x].getDamage()));
                  }
               }
            }
         } catch (NoSuchStructureException var8) {
            logger.log(Level.WARNING, "The new building doesn't exist.", (Throwable)var8);
         }
   ...
   public static boolean isCreatureTurnedTowardsItem(Item target, Creature performer, float angle) {
      double newrot = Math.atan2(
         (double)(target.getPosY() - (float)((int)performer.getStatus().getPositionY())),
         (double)(target.getPosX() - (float)((int)performer.getStatus().getPositionX()))
      );
   ...
   public static boolean isItemTurnedTowardsCreature(Creature target, Item performer, float angle) {
      double newrot = Math.atan2(
         (double)(target.getPosY() - (float)((int)performer.getPosY())), (double)(target.getPosX() - (float)((int)performer.getPosX()))
      );
      float attAngle = (float)(newrot * (180.0 / Math.PI)) - 90.0F;
      attAngle = Creature.normalizeAngle(attAngle);
      ...
      if (creatureId == -1L || this.creatures.containsKey(creatureId)) {
         this.watcher
            ...
            .sendWearItem(
               ...
               (float)colorGreen,
               (float)colorBlue,
               (float)secondaryColorRed,
               (float)secondaryColorGreen,
               (float)secondaryColorBlue,
               material,
               rarity
            );
      }
   ...
   void sendWieldItem(
      ...
      if (creatureId == -1L || this.creatures.containsKey(creatureId)) {
         this.watcher
            .getCommunicator()
            .sendWieldItem(creatureId, slot, modelname, rarity, colorRed, colorGreen, colorBlue, secondaryColorRed, secondaryColorGreen, secondaryColorBlue);
      }
   }
   ...
   void sendUseItem(
      ...
      if (creature == null) {
         this.watcher
            .getCommunicator()
            .sendUseItem(-1L, modelname, rarity, colorRed, colorGreen, colorBlue, secondaryColorRed, secondaryColorGreen, secondaryColorBlue);
      } else if (!creature.isTeleporting() && this.creatures.containsKey(creature.getWurmId())) {
         this.watcher
            .getCommunicator()
            .sendUseItem(creature.getWurmId(), modelname, rarity, colorRed, colorGreen, colorBlue, secondaryColorRed, secondaryColorGreen, secondaryColorBlue);
      }
   }
   ...
   void sendAttachCreature(long creatureId, long targetId, float offx, float offy, float offz, int seatId) {
      ...
      if (targetId != -1L) {
         if (WurmId.getType(targetId) != 1 && WurmId.getType(targetId) != 0) {
            if (WurmId.getType(targetId) == 2 || WurmId.getType(targetId) == 19 || WurmId.getType(targetId) == 20) {
               try {
                  ...
                  if (this.items == null || !this.items.contains(item)) {
                     if (this.watcher.getVisionArea() != null) {
                        if (this.isOnSurface()) {
                           if (!item.isOnSurface()
                              && (
                                 this.watcher.getVisionArea().getUnderGround() == null
         ...
         } else if (!this.creatures.containsKey(targetId)) {
            try {
      ...
      if (creatureId != -1L) {
         if (WurmId.getType(creatureId) != 1 && WurmId.getType(creatureId) != 0) {
            if (WurmId.getType(creatureId) == 2 || WurmId.getType(creatureId) == 19 || WurmId.getType(creatureId) == 20) {
               try {
         ...
         } else if (this.watcher.getWurmId() != creatureId && !this.creatures.containsKey(creatureId) && targetId != -1L) {
            try {
   ...
   public void sendNewProjectile(
      ...
      this.watcher
         .getCommunicator()
         .sendNewProjectile(itemid, type, modelName, name, material, startingPosition, startingVelocity, endingPosition, rotation, surface);
   }
