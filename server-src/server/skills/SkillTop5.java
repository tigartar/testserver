   ...
   public static SkillTop5[][] getAllSkillsTop5() throws Exception {
      ...
      try {
         ...
         ps = dbcon.prepareStatement(
            "SELECT S.NUMBER, S.OWNER, P.NAME, S.VALUE FROM WURMPLAYERS.SKILLS S JOIN WURMPLAYERS.PLAYERS P ON S.OWNER=P.WURMID WHERE S.NUMBER=? AND P.POWER=0 ORDER BY S.VALUE DESC LIMIT 5;"
         );
         ...
         for(int i = 0; i < skills.length; ++i) {
            ...
            for(int count = 0; rs.next() && count < 5; ++count) {
               SkillTop5 skill = new SkillTop5(rs.getInt("NUMBER"), count, rs.getLong("OWNER"), rs.getString("NAME"), rs.getDouble("VALUE"));
               skillsTop5[i][count] = skill;
            }
