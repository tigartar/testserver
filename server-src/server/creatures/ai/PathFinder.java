...
public class PathFinder implements MiscConstants {
      ...
      if (!this.mesh.contains(x, y)) {
         ...
         try {
            ...
            logger.log(
               Level.WARNING,
               "Mesh info: "
                  + this.mesh.getBorderStartX()
                  + ", "
                  + this.mesh.getBorderStartY()
                  + ", to "
                  + this.mesh.getBorderEndX()
                  + ", "
                  + this.mesh.getBorderEndY()
            );
   ...
   int step2() {
      ...
      now = best;
      this.pathList.remove(best);
      best.setUsed();
      PathTile[] next = this.mesh.getAdjacent(best);
               ...
               if (result != null) {
                  ...
                  if (this.creature.getBridgeId() > 0L) {
                     try {
                        ...
                        logger.log(
                           Level.WARNING, this.creature.getWurmId() + " at " + this.creature.getTileX() + "," + this.creature.getTileY() + " no structure"
                        );
                     }
                  }
