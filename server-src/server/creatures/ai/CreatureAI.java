...
public abstract class CreatureAI {
      ...
      try {
         ...
         if (p != null && !p.isEmpty()) {
            ...
            if (nextTile.hasSpecificPos()) {
               ...
               float lMod = c.getMoveModifier(
                  (c.isOnSurface() ? Server.surfaceMesh : Server.caveMesh)
   ...
   protected void creatureMovementTick(Creature c, boolean rotateFromBlocker) {
      ...
      float lXPosMod = (float)Math.sin((double)(lRotation * (float) (Math.PI / 180.0))) * c.getSpeed() * lMoveModifier * aiDataMoveModifier;
      float lYPosMod = -((float)Math.cos((double)(lRotation * (float) (Math.PI / 180.0)))) * c.getSpeed() * lMoveModifier * aiDataMoveModifier;
      ...
      if (lDiffTileX != 0 || lDiffTileY != 0) {
         if (!c.isOnSurface()) {
            ...
            if (Tiles.isSolidCave(Tiles.decodeType(Server.caveMesh.getTile(lNewTileX, lNewTileY)))) {
               ...
               if (!Tiles.isMineDoor(Tiles.decodeType(Server.caveMesh.getTile(c.getTileX(), c.getTileY())))
      ...
      if (!(lPosX >= (float)(Zones.worldTileSizeX - 1 << 2)) && !(lPosX < 0.0F) && !(lPosY < 0.0F) && !(lPosY >= (float)(Zones.worldTileSizeY - 1 << 2))) {
         ...
         if ((double)lPosZ < 0.5) {
            if ((lPosZ > -2.0F || lPosZ <= -2.0F) && (lPosZ < 0.0F || c.getTarget() != null) && c.isSwimming()) {
         ...
         c.moved(lPosX - lPosX, lPosY - lPosY, lPosZ - lPosZ, lDiffTileX, lDiffTileY);
   ...
   public boolean pollCreature(Creature c, long delta) {
      ...
      if (this.pollSpecialPreAttack(c, delta)) {
            ...
            if (c.isFighting()) {
               ...
               if (System.currentTimeMillis() - c.lastSavedPos > 3600000L) {
                  ...
                  try {
                     ...
                     logger.warning("Unable to save creature position, creature id: " + c.getWurmId() + " reason: " + var8.getMessage());
                  }
               }
            }
         }
         ...
         if (isDead) {
            return true;
         } else if (this.pollSpecialPreItems(c, delta)) {
            return true;
         } else {
            ...
            if (this.pollSpecialPreMisc(c, delta)) {
               ...
               return isDead ? true : this.pollSpecialFinal(c, delta);
            }
         }
      }
   }
