    ...
    StaticPathMeshNPC(int startx, int starty, int endx, int endy, boolean surf, int borderSize, int[] aMesh) {
        ...
        for (int x = 0; x < this.sizex; ++x) {
            for (int y = 0; y < this.sizey; ++y) {
                StaticPathMeshNPC.pathables[x][y] = new PathTile(this.borderStartX + x, this.borderStartY + y, lMesh[this.borderStartX + x | this.borderStartY + y << Constants.meshSize], this.surfaced, (int)((byte)(this.surfaced ? 0 : -1)));
                if (logger.isLoggable(Level.FINEST)) {
                    logger.finest("pathables: " + pathables[x][y].toString());
                }
                if (pathables[x][y].getTileX() == startx && pathables[x][y].getTileY() == starty) {
                    lStart = pathables[x][y];
                }
                if (pathables[x][y].getTileX() != endx || pathables[x][y].getTileY() != endy) continue;
