...
extends CreatureAI {
    ...
    protected boolean pollMovement(Creature c, long delta) {
        BartenderAIData aiData = (BartenderAIData)c.getCreatureAIData();
        ...
        if (c.getStatus().getPath() == null) {
            ...
            if (this.isTimerReady(c, 1, 30000L)) {
                if (Server.rand.nextInt(100) < 10) {
                    ...
                    for (Creature otherC : nearbyCreatures = c.getCurrentTile().getZone().getAllCreatures()) {
                        if (otherC == c || !otherC.isPlayer() || !otherC.getStatus().isHungry() || otherC.getCurrentTile().getVillage() != aiData.getHomeVillage() || otherC.getTileX() == c.getTileX() && otherC.getTileY() == c.getTileY()) continue;
