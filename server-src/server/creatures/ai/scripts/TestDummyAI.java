...
public class TestDummyAI extends CreatureAI {
   ...
   public CreatureAIData createCreatureAIData() {
      return new TestDummyAI.TestDummyAIData();
   }
   ...
   public double receivedWound(Creature c, @Nullable Creature performer, byte dmgType, int dmgPosition, float armourMod, double damage) {
      if (performer != null) {
         try {
            String message = "You dealt "
               + String.format("%.2f", damage / 65535.0 * 100.0)
               + " to "
               + c.getBody().getBodyPart(dmgPosition).getName()
               + " of type "
               + Wound.getName(dmgType)
               + ".";
