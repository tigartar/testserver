...
public final class Offspring {
    ...
    private static final Map<Long, Offspring> offsprings = new ConcurrentHashMap<Long, Offspring>();
    ...
    private static final String[] MALE_NAMES = Offspring.createMaleNames();
    private static final String[] FEMALE_NAMES = Offspring.createFemaleNames();
    private static final String[] GENERIC_NAMES = Offspring.createGenericNames();
    private static final String[] MALE_UNI_NAMES = Offspring.createMaleUnicornNames();
    private static final String[] FEMALE_UNI_NAMES = Offspring.createFemaleUnicornNames();
    ...
    private void create() {
        ...
        try {
            ...
            ps = dbcon.prepareStatement(CREATE_OFFSPRING);
            ...
            try {
                logger.log(Level.WARNING, "Failed to create offspring for " + this.mother, ex);
            }
    ...
    boolean decreaseDaysLeft() {
        block4: {
            block5: {
                this.checked = true;
                byte by = this.deliveryDays;
                this.deliveryDays = (byte)(by - 1);
                if (by <= 0) break block5;
                Connection dbcon = null;
                PreparedStatement ps = null;
                try {
                    dbcon = DbConnector.getCreatureDbCon();
                    ps = dbcon.prepareStatement(UPDATE_OFFSPRING_DAYS);
                    ...
                    try {
                        logger.log(Level.WARNING, "Failed to update offspring for " + this.mother + ", days=" + this.deliveryDays, ex);
                    }
                    catch (Throwable throwable) {
                        DbUtilities.closeDatabaseObjects(ps, null);
                        DbConnector.returnConnection(dbcon);
            ...
            Offspring.deleteSettings(this.mother);
            return true;
        }
        return false;
    }
    ...
    static void deleteSettings(long motherid) {
        ...
        try {
            dbcon = DbConnector.getCreatureDbCon();
            ps = dbcon.prepareStatement(DELETE_OFFSPRING);
            ps.setLong(1, motherid);
            ps.executeUpdate();
        }
        catch (SQLException ex) {
            try {
                logger.log(Level.WARNING, "Failed to delete offspring for " + motherid, ex);
            }
            catch (Throwable throwable) {
                DbUtilities.closeDatabaseObjects(ps, null);
                DbConnector.returnConnection(dbcon);
    ...
    static void loadAllOffspring() {
        ...
        try {
            dbcon = DbConnector.getCreatureDbCon();
            ps = dbcon.prepareStatement(LOAD_ALL_OFFSPRING);
            rs = ps.executeQuery();
            while (rs.next()) {
            ...
            try {
                logger.log(Level.WARNING, "Failed loading all offspring " + ex.getMessage(), ex);
            }
            catch (Throwable throwable) {
    ...
    public static void resetOffspringCounters() {
        ArrayList<Offspring> invalid = new ArrayList<Offspring>();
        ...
        for (Offspring offspring : invalid) {
            logger.warning("Deleting offspring data, mother not found. mother=" + offspring.mother + ", father=" + offspring.father + ", traits=" + offspring.traits + ", deliveryDays=" + offspring.deliveryDays);
            Offspring.deleteSettings(offspring.mother);
        }
    }
    ...
    static String generateMaleName() {
        if (MALE_NAMES.length > 0) {
            ...
            if (num < 50) {
                return Offspring.generateGenericPrefix() + MALE_NAMES[Server.rand.nextInt(MALE_NAMES.length)];
            }
            return MALE_NAMES[Server.rand.nextInt(MALE_NAMES.length)] + Offspring.generateGenericPrefix();
        }
        return "";
    }
    ...
    static String generateGenericName() {
        if (GENERIC_NAMES.length > 0) {
            return GENERIC_NAMES[Server.rand.nextInt(GENERIC_NAMES.length)] + GENERIC_NAMES[Server.rand.nextInt(GENERIC_NAMES.length)];
        }
        return "";
    }
    ...
    static String generateFemaleName() {
        if (FEMALE_NAMES.length > 0) {
            ...
            if (num < 50) {
                return Offspring.generateGenericPrefix() + FEMALE_NAMES[Server.rand.nextInt(FEMALE_NAMES.length)];
            }
            return FEMALE_NAMES[Server.rand.nextInt(FEMALE_NAMES.length)] + Offspring.generateGenericPrefix();
        }
        return "";
    }
    ...
    static String generateFemaleUnicornName() {
        if (FEMALE_UNI_NAMES.length > 0) {
            ...
            if (num < 50) {
                return Offspring.generateGenericPrefix() + FEMALE_UNI_NAMES[Server.rand.nextInt(FEMALE_UNI_NAMES.length)];
            }
            return FEMALE_UNI_NAMES[Server.rand.nextInt(FEMALE_UNI_NAMES.length)] + Offspring.generateGenericPrefix();
        }
        return "";
    }
    ...
    static String generateMaleUnicornName() {
        if (MALE_UNI_NAMES.length > 0) {
            ...
            if (num < 50) {
                return Offspring.generateGenericPrefix() + MALE_UNI_NAMES[Server.rand.nextInt(MALE_UNI_NAMES.length)];
            }
            return MALE_UNI_NAMES[Server.rand.nextInt(MALE_UNI_NAMES.length)] + Offspring.generateGenericPrefix();
        }
        return "";
    }
    ...
    static String[] createMaleNames() {
        ArrayList<String> malelist = new ArrayList<String>(80);
        malelist.add("lars");
        malelist.add("bom");
        malelist.add("ally");
        malelist.add("dom");
    ...
    static String[] createFemaleNames() {
        ArrayList<String> femalelist = new ArrayList<String>(80);
    ...
    static String[] createGenericNames() {
        ArrayList<String> genericlist = new ArrayList<String>(59);
