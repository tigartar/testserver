...
PermissionsPlayerList.ISettings {
    ...
    public boolean covers(float x, float y, float posz, int floorLevel, boolean followGround) {
        return (this.wall != null && this.wall.isWithinZ(posz + 1.0f, posz, followGround) || this.isTransition() && floorLevel <= 0) && x >= (float)this.startx && x <= (float)this.endx && y >= (float)this.starty && y <= (float)this.endy;
    }
                ...
                if (!(this.addCreature(creature) || diffTileX == 0 && diffTileY == 0)) {
                    if (!this.canBeOpenedBy(creature, true)) {
                        try {
                            ...
                            if (tile != null) {
                                if (tile == this.innerTile) {
                                    ...
                                    if (creature instanceof Player) {
                                        ...
                                        logger.log(Level.WARNING, creature.getName() + " a cheater? Passed through door at " + creature.getStatus().getPositionX() + ", " + creature.getStatus().getPositionY() + ", z=" + creature.getPositionZ() + ", minZ=" + this.wall.getMinZ());
                            ...
                            logger.log(Level.WARNING, "A door on no tile at " + creature.getStatus().getPositionX() + ", " + creature.getStatus().getPositionY() + ", structure: " + this.structure);
                        }
                        catch (NoSuchZoneException nsz) {
                            logger.log(Level.WARNING, "A door in no zone at " + creature.getStatus().getPositionX() + ", " + creature.getStatus().getPositionY() + ", structure: " + this.structure + " - " + nsz);
                        }
                    } else if (this.structure != -10L && creature.isPlayer()) {
                        int tiley;
                        int tilex = creature.getTileX();
                        VolaTile tile = Zones.getTileOrNull(tilex, tiley = creature.getTileY(), creature.isOnSurface());
                        if (tile == this.innerTile) {
    ...
    public final Item getLock() throws NoSuchLockException {
        ...
        try {
            Item toReturn = Items.getItem(this.lock);
            return toReturn;
        }
        catch (NoSuchItemException nsi) {
