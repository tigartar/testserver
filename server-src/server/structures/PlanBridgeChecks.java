...
implements MiscConstants {
    ...
    public static boolean passChecks(Creature performer, Point start, Point trueEnd, byte dir, int[] hts, boolean insta) {
        ...
        if (!ans.failed()) {
            ans = PlanBridgeChecks.checkForHighways(start, end, dir, performer.getLayer());
        }
        if (!ans.failed()) {
            ans = PlanBridgeChecks.checkClearance(start, end, dir, insta, performer.getLayer());
        }
        ...
        if (!ans.failed()) {
            ans = PlanBridgeChecks.checkForCaveEntrances(start, end, dir, insta);
        }
        if (!ans.failed()) {
            ans = PlanBridgeChecks.checkForSpecialItems(start, end, dir, insta, performer.isOnSurface());
        }
        if (!ans.failed()) {
            ans = PlanBridgeChecks.checkForSettlements(performer, start, end, dir, insta, performer.isOnSurface());
        }
        if (!ans.failed()) {
            ans = PlanBridgeChecks.checkForBridges(start, end, dir, insta, performer.isOnSurface());
        }
        if (!ans.failed()) {
            ans = PlanBridgeChecks.checkForBuildings(start, end, dir, insta, performer.isOnSurface());
        }
        if (ans.failed()) {
    ...
    public static PlanBridgeCheckResult checkPlanWidth(Point start, Point end, byte dir, int layer) {
        ...
        if (startBuilding != null) {
            if (dir == 0) {
                for (x = start.getX() + 1; x <= end.getX(); ++x) {
                    building = Structures.getStructureForTile(x, start.getY() - 1, layer == 0);
                    if (building == null || startBuilding.getWurmId() == building.getWurmId()) continue;
                ...
                for (y = start.getY() + 1; y <= end.getY(); ++y) {
                    Structure building2 = Structures.getStructureForTile(start.getX() - 1, y, layer == 0);
                    if (building2 == null || startBuilding.getWurmId() == building2.getWurmId()) continue;
            ...
            for (y = start.getY(); y <= end.getY() + 1; ++y) {
        ...
        if (endBuilding != null) {
            if (dir == 0) {
                for (x = start.getX() + 1; x <= end.getX(); ++x) {
                    building = Structures.getStructureForTile(x, end.getY(), layer == 0);
                    if (building == null || endBuilding.getWurmId() == building.getWurmId()) continue;
                ...
                for (y = start.getY() + 1; y <= end.getY(); ++y) {
                    building = Structures.getStructureForTile(end.getX(), y, layer == 0);
                    if (building == null || endBuilding.getWurmId() == building.getWurmId()) continue;
    ...
    public static PlanBridgeCheckResult checkForHighways(Point start, Point end, byte dir, int layer) {
        MeshIO mesh;
        MeshIO meshIO = mesh = layer == 0 ? Server.surfaceMesh : Server.caveMesh;
        if (dir == 0) {
            for (int x = start.getX(); x <= end.getX(); ++x) {
                int encodedStartTile = mesh.getTile(x, start.getY());
                byte startType = Tiles.decodeType(encodedStartTile);
                short startHeight = Tiles.decodeHeight(encodedStartTile);
                int startOffset = start.getH() - startHeight;
                if (startOffset == 0 && PlanBridgeChecks.isRoadTile(startType) && MethodsHighways.onHighway(x, start.getY(), layer == 0)) {
                ...
                short endHeight = Tiles.decodeHeight(encodedEndTile);
                int endOffset = end.getH() - endHeight;
                if (start.getY() == end.getY() || endOffset != 0 || !PlanBridgeChecks.isRoadTile(endType) || !MethodsHighways.onHighway(x, y, layer == 0)) continue;
                String pMsg = "Cannot build a bridge on a highway.";
                return new PlanBridgeCheckResult(true, "Cannot build a bridge on a highway.", "");
            }
        } else {
            for (int y = start.getY(); y <= end.getY(); ++y) {
                ...
                short startHeight = Tiles.decodeHeight(encodedStartTile);
                int startOffset = start.getH() - startHeight;
                if (startOffset == 0 && PlanBridgeChecks.isRoadTile(startType) && MethodsHighways.onHighway(start.getX(), y, layer == 0)) {
                    String pMsg = "Cannot build a bridge on a highway.";
                    return new PlanBridgeCheckResult(true, "Cannot build a bridge on a highway.", "");
                }
                int x = end.getX() - 1;
                ...
                short endHeight = Tiles.decodeHeight(encodedEndTile);
                int endOffset = end.getH() - endHeight;
                if (start.getX() == end.getX() || endOffset != 0 || !PlanBridgeChecks.isRoadTile(endType) || !MethodsHighways.onHighway(x, y, layer == 0)) continue;
                String pMsg = "Cannot build a bridge on a highway.";
                return new PlanBridgeCheckResult(true, "Cannot build a bridge on a highway.", "");
            }
        }
        ...
        if (len == 1) {
            ...
            for (int x = start.getX() + 1; x < end.getX(); ++x) {
                ...
                for (int y = start.getY(); y <= end.getY() + 1; ++y) {
                    ...
                    if (!((float)(ht + 10) > pz)) continue;
                    if (insta) {
                        qMsg = "(W-E) S:" + start.getX() + "," + start.getY() + "," + start.getH() + " U:" + end.getX() + "," + end.getY() + "," + end.getH() + " Failed at x:" + x + ", y:" + y + ", z:" + pz + ", h:" + ht + ", s:" + slope;
                    }
                    String pMsg = "Some terrain is in the way!";
                    return new PlanBridgeCheckResult(true, "Some terrain is in the way!", qMsg);
                }
        ...
        if (dir == 0) {
            ...
            for (int y = start.getY() + 1; y < end.getY(); ++y) {
                ...
                for (int x = start.getX(); x <= end.getX() + 1; ++x) {
                    ...
                    int cht;
                    int ht = (int)(Zones.getHeightForNode(x, y, layer) * 10.0f);
                    int tht = ht + (cht = CaveTile.decodeCeilingHeight(encodedTile = Server.caveMesh.getTile(x, y)));
                    if (!((float)tht < pz + 30.0f)) continue;
                    if (insta) {
            ...
            for (int x = start.getX() + 1; x < end.getX(); ++x) {
                ...
                for (int y = start.getY(); y <= end.getY() + 1; ++y) {
                    ...
                    if (!((float)tht < pz + 30.0f)) continue;
                    if (insta) {
                        qMsg = "(W-E) S:" + start.getX() + "," + start.getY() + "," + start.getH() + " U:" + end.getX() + "," + end.getY() + "," + end.getH() + " Failed at x:" + x + ", y:" + y + ", z:" + pz + ", h:" + ht + ", s:" + slope + " Ceiling:" + cht + " Total:" + tht;
                    }
                    String pMsg = "Ceiling is in the way!";
                    return new PlanBridgeCheckResult(true, "Ceiling is in the way!", qMsg);
                }
        ...
        if (dir == 0) {
            for (int x = start.getX(); x <= end.getX(); ++x) {
                for (int y = start.getY(); y < end.getY(); ++y) {
                    ...
                    if (Terraforming.isBridgeableTile(ttype)) continue;
                    if (insta) {
                        qMsg = " (@ " + x + "," + y + ")";
                    }
                    String pMsg = Tiles.getTile((byte)ttype).tiledesc + " is blocking the planning" + qMsg + ".";
                    return new PlanBridgeCheckResult(true, pMsg, qMsg);
                }
            }
        } else {
    ...
    public static PlanBridgeCheckResult checkForCaveEntrances(Point start, Point end, byte dir, boolean insta) {
        if (dir == 0) {
            for (int x = start.getX() - 1; x <= end.getX() + 1; ++x) {
                for (int y = start.getY() - 1; y <= end.getY(); ++y) {
                    PlanBridgeCheckResult pbcr = PlanBridgeChecks.checkForCaveEntrances(x, y, insta);
                    if (pbcr == null) continue;
                    return pbcr;
                }
            }
        } else {
    ...
    private static PlanBridgeCheckResult checkForCaveEntrances(int x, int y, boolean insta) {
        ...
        if (Terraforming.isCaveEntrance(ttype)) {
            String qMsg = "";
            if (insta) {
                qMsg = " (" + Tiles.getTile((byte)ttype).tiledesc + " @ " + x + "," + y + ")";
            }
    ...
    public static PlanBridgeCheckResult checkForSpecialItems(Point start, Point end, byte dir, boolean insta, boolean onSurface) {
        if (dir == 0) {
            for (int x = start.getX(); x <= end.getX(); ++x) {
                for (int y = start.getY(); y < end.getY(); ++y) {
                    PlanBridgeCheckResult pbcr = PlanBridgeChecks.checkForSpecialItems(x, y, insta, onSurface);
                    if (pbcr == null) continue;
                    return pbcr;
                }
            }
    ...
    public static PlanBridgeCheckResult checkForSettlements(Creature performer, Point start, Point end, byte dir, boolean insta, boolean onSurface) {
        if (dir == 0) {
            for (int x = start.getX() - 1; x <= end.getX() + 1; ++x) {
                for (int y = start.getY() - 1; y <= end.getY(); ++y) {
                    PlanBridgeCheckResult pbcr = PlanBridgeChecks.checkForSettlements(performer, x, y, insta, onSurface);
                    if (pbcr == null) continue;
                    return pbcr;
                }
            }
    ...
    private static PlanBridgeCheckResult checkForSettlements(Creature performer, int x, int y, boolean insta, boolean onSurface) {
        ...
        if (vill != null && !vill.isActionAllowed((short)116, performer, false, 0, 0)) {
            ...
            pMsg = vill.isEnemy(performer) ? vill.getName() + " does not allow that" + qMsg + "." : "That would be illegal here. You can check the settlement token for the local laws" + qMsg + ".";
            return new PlanBridgeCheckResult(true, pMsg, qMsg);
        }
        return null;
    }
        ...
        if (dir == 0) {
            for (int x = start.getX(); x <= end.getX(); ++x) {
                for (int y = start.getY(); y < end.getY(); ++y) {
                    Village perim;
                    Village vill = Villages.getVillage(x, y, onSurface);
                    if (vill != null || (perim = Villages.getVillageWithPerimeterAt(x, y, onSurface)) == null || perim.isCitizen(performer) || Methods.isActionAllowed(performer, (short)116, x, y)) continue;
                    String qMsg = "";
                    if (insta) {
                        qMsg = " (@ " + x + "," + y + ")";
                    }
    ...
    public static PlanBridgeCheckResult checkForBridges(Point start, Point end, byte dir, boolean insta, boolean onSurface) {
        if (dir == 0) {
            for (int x = start.getX(); x <= end.getX(); ++x) {
                PlanBridgeCheckResult pbcrN = PlanBridgeChecks.checkForBridges(x, end.getY() - 1, insta, onSurface);
                if (pbcrN != null) {
                    return pbcrN;
                }
                PlanBridgeCheckResult pbcrS = PlanBridgeChecks.checkForBridges(x, end.getY(), insta, onSurface);
                if (pbcrS == null) continue;
                return pbcrS;
            }
        } else {
            for (int y = start.getY(); y <= end.getY(); ++y) {
                PlanBridgeCheckResult pbcrW = PlanBridgeChecks.checkForBridges(start.getX() - 1, y, insta, onSurface);
                if (pbcrW != null) {
                    return pbcrW;
                }
                PlanBridgeCheckResult pbcrE = PlanBridgeChecks.checkForBridges(end.getX(), y, insta, onSurface);
                if (pbcrE == null) continue;
                return pbcrE;
            }
        }
        return new PlanBridgeCheckResult(false);
    ...
    public static PlanBridgeCheckResult checkForBuildings(Point start, Point end, byte dir, boolean insta, boolean onSurfrace) {
        if (dir == 0) {
            for (int x = start.getX(); x <= end.getX(); ++x) {
                for (int y = start.getY(); y < end.getY(); ++y) {
                    PlanBridgeCheckResult pbcr = PlanBridgeChecks.checkForBuildings(x, y, insta, onSurfrace);
                    if (pbcr == null) continue;
                    return pbcr;
                }
            }
    ...
    public static PlanBridgeCheckResult checkCeilingClearance(Creature performer, int len, Point start, Point end, byte dir, boolean insta) {
        if (!performer.isOnSurface()) {
            ...
            PlanBridgeCheckResult res = PlanBridgeChecks.checkCeilingClearance(start, end, dir, hts, insta);
            if (res.failed()) {
                hts = PlanBridgeMethods.calcArch(performer, 10, len, start, end);
                res = PlanBridgeChecks.checkCeilingClearance(start, end, dir, hts, insta);
            }
            if (res.failed()) {
        ...
        if (dir == 0) {
            for (int x = start.getX(); x <= end.getX(); ++x) {
                for (int y = start.getY(); y < end.getY(); ++y) {
                    ...
                    short ht = Tiles.decodeHeight(encodedTile);
                    int cht = CaveTile.decodeCeilingHeight(encodedTile);
                    int tht = ht + cht;
                    if (hts[y - start.getY()] + 30 <= tht) continue;
            ...
            for (int y = start.getY(); y <= end.getY(); ++y) {
                for (int x = start.getX(); x < end.getX(); ++x) {
                    ...
                    int cht = CaveTile.decodeCeilingHeight(encodedTile);
                    int tht = ht + cht;
                    if (hts[x - start.getX()] + 30 <= tht) continue;
                    String qMsg = insta ? " (@ " + x + "," + y + ")" : "";
                    String pMsg = "Ceiling too close" + qMsg + ".";
                    return new PlanBridgeCheckResult(true, pMsg, qMsg);
                }
