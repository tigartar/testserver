...
public abstract class BridgePart implements MiscConstants, TimeConstants, Blocker, IFloor, SoundNames, Permissions.IAllow {
   ...
   private final Vector3f calculateCenterPoint() {
      return new Vector3f((float)(this.tilex * 4 + 2), (float)(this.tiley * 4 + 2), (float)(this.getRealHeight() + this.slope) / 10.0F);
   }
      ...
      if (this.slope == 0) {
         ...
         if (verticalBlocker == null) {
            if (this.dir == 0 || this.dir == 4) {
               if (this.slope < 0) {
                  verticalBlocker = new Float((float)(this.tilex * 4), (float)(this.tiley * 4), 4.0F, Math.abs((float)this.slope / 10.0F));
               } else {
                  verticalBlocker = new Float((float)(this.tilex * 4), (float)((this.tiley + 1) * 4), 4.0F, Math.abs((float)this.slope / 10.0F));
               }
            }
            ...
            if (this.dir == 6 || this.dir == 2) {
               if (this.slope < 0) {
                  ...
                  verticalBlocker = new Float((float)((this.tilex + 1) * 4), (float)(this.tiley * 4), 4.0F, Math.abs((float)this.slope / 10.0F));
               }
            }
         }
   ...
   public int getNorthExitFloorLevel() {
      return this.northExit == -1 ? -1 : this.northExit / 30;
   }

   public int getEastExitFloorLevel() {
      return this.eastExit == -1 ? -1 : this.eastExit / 30;
   }

   public int getSouthExitFloorLevel() {
      return this.southExit == -1 ? -1 : this.southExit / 30;
   }

   public int getWestExitFloorLevel() {
      return this.westExit == -1 ? -1 : this.westExit / 30;
   }
   ...
   public String getFullName() {
      if (this.bridgePartState == BridgeConstants.BridgeState.PLANNED) {
         ...
         return this.bridgePartState.isBeingBuilt() ? "Unfinished " + this.getName() : this.getName();
      }
   }
   ...
   public final Vector3f isBlocking(Creature creature, Vector3f startPos, Vector3f endPos, Vector3f aNormal, int blockType, long target, boolean followGround) {
      if (target == this.getId()) {
         ...
         return this.isAPlan() ? null : this.getIntersectionPoint(startPos, endPos, aNormal, creature, blockType);
      }
   }
   ...
   public final float getBlockPercent(Creature creature) {
      return (float)Math.min(100, Math.max(0, this.bridgePartState.getCode() * 14));
   }
   ...
   public final Vector3f getFloorIntersection(Vector3f startPos, Vector3f endPos, Vector3f aNormal, Creature creature, int blockType) {
      ...
      if (diffend.length() < interDiff.length()) {
         ...
         if (this.isWithinFloorBounds(intersection, creature, blockType)) {
            return u >= 0.0F && u <= 1.0F ? intersection : null;
         } else {
            return null;
         }
      }
   ...
   private final boolean isWithinFloorBounds(Vector3f pointToCheck, Creature creature, int blockType) {
      if (pointToCheck.getY() >= (float)(this.tiley * 4)
         ...
         if (Servers.isThisATestServer()) {
            logger.info(
               "WithinBounds?:"
                  ...
                  + " height checked:"
                  + pointToCheck.getZ()
                  + " against bridge real height:"
                  + (float)this.getRealHeight() / 10.0F
                  + " ("
                  + this.tilex
                  + ","
                  + this.tiley
   ...
   private final boolean isWithinVerticalBounds(Vector3f pointToCheck, Creature creature) {
      ...
      if (rect == null) {
         ...
         if (this.dir != 0 && this.dir != 4) {
            if ((double)pointToCheck.getX() >= rect.getX() - 0.1F
               && (double)pointToCheck.getX() <= rect.getX() + 0.1F
               && (double)pointToCheck.getY() >= rect.getY()
               && (double)pointToCheck.getY() <= rect.getY() + rect.getWidth()
               && pointToCheck.getZ() >= this.getMinZ()
               && (double)pointToCheck.getZ() <= (double)this.getMinZ() + rect.getHeight()) {
               return true;
            }
         } else if ((double)pointToCheck.getY() >= rect.getY() - 0.1F
            && (double)pointToCheck.getY() <= rect.getY() + 0.1F
            && (double)pointToCheck.getX() >= rect.getX()
            && (double)pointToCheck.getX() <= rect.getX() + rect.getWidth()
            && pointToCheck.getZ() >= this.getMinZ()
            && (double)pointToCheck.getZ() <= (double)this.getMinZ() + rect.getHeight()) {
            return true;
         }
   ...
   public static final void loadAllBridgeParts() throws IOException {
      ...
      try {
         ...
         logger.log(Level.WARNING, "Failed to load bridge parts!" + var33.getMessage(), (Throwable)var33);
   ...
   Structure getStructure() {
      ...
      try {
         ...
         logger.log(
            Level.WARNING,
            " Failed to find Structures.getStructure(" + this.getStructureId() + " for a BridgePart about to be deleted: " + var3.getMessage(),
            (Throwable)var3
         );
      }
   ...
   public void buildProgress(int numSteps) {
      if (this.getBridgePartState().isBeingBuilt()) {
         ...
         logger.log(
            Level.WARNING,
            "buildProgress method called on bridge part when bridge part was not in buildable state: " + this.getId() + " " + this.bridgePartState.toString()
         );
      }
   }
   ...
   public final boolean poll(long currTime, Structure struct) {
      if (struct == null) {
         return true;
      } else if (currTime - struct.getCreationDate() <= (Servers.localServer.testServer ? 3600000L : 86400000L) * 2L) {
         return false;
   ...
   public final boolean supports(StructureSupport support) {
      return !this.supports() ? false : false;
   }
