...
extends TileBehaviour {
    ...
    public List<ActionEntry> getBehavioursFor(Creature performer, boolean onSurface, Floor floor) {
        ...
        if (MethodsStructure.mayModifyStructure(performer, structure, floorTile, (short)177)) {
            toReturn.add(new ActionEntry(-2, "Rotate", "rotating"));
            toReturn.add(new ActionEntry(177, "Turn clockwise", "turning"));
            toReturn.add(new ActionEntry(178, "Turn counterclockwise", "turning"));
        }
        return toReturn;
    }

    public static final List<ActionEntry> getCompletedFloorsBehaviour(boolean andStaircases, boolean onSurface) {
        ArrayList<ActionEntry> plantypes = new ArrayList<ActionEntry>();
        ...
        ArrayList<ActionEntry> toReturn = new ArrayList<ActionEntry>(5);
    ...
    public List<ActionEntry> getBehavioursFor(Creature performer, Item source, boolean onSurface, Floor floor) {
        ...
        if (MethodsStructure.mayModifyStructure(performer, structure, floorTile, (short)169)) {
            switch (floor.getFloorState()) {
                ...
                case COMPLETED: {
                    if (floor.getType() == StructureConstants.FloorType.ROOF) break;
                    toReturn.addAll(FloorBehaviour.getCompletedFloorsBehaviour(true, floor.isOnSurface()));
                    break;
                }
            }
            toReturn.add(new ActionEntry(-2, "Rotate", "rotating"));
    ...
    private boolean buildAction(Action act, Creature performer, Item source, Floor floor, short action, float counter) {
        switch (floor.getFloorState()) {
            case PLANNING: {
                ...
                if (!FloorBehaviour.isOkToBuild(performer, source, floor, floor.getFloorLevel(), floor.isRoof())) {
                    floor.setMaterial(oldMaterial);
                    return true;
                }
                if (!autoAdvance && !FloorBehaviour.advanceNextState(performer, floor, act, true)) {
                    String message = FloorBehaviour.buildRequiredMaterialString(floor, false);
                ...
                float qlevel = MethodsStructure.calculateNewQualityLevel(act.getPower(), craftSkill.getKnowledge(0.0), oldql, FloorBehaviour.getTotalMaterials(floor));
                ...
                if (floor.getMaterial() == StructureConstants.FloorMaterial.STANDALONE) {
                    ...
                    performer.getCommunicator().sendNormalServerMessage("You plan a " + floor.getName() + " made of " + FloorBehaviour.getMaterialDescription(floor) + ".");
                } else {
                    performer.getCommunicator().sendNormalServerMessage("You plan a " + floor.getName() + " made of " + floor.getMaterial().getName().toLowerCase() + ".");
                }
                return FloorBehaviour.floorBuilding(act, performer, source, floor, action, counter);
            }
            case BUILDING: {
                if (FloorBehaviour.floorBuilding(act, performer, source, floor, action, counter)) {
    ...
    static boolean advanceNextState(Creature performer, Floor floor, Action act, boolean justCheckIfItemsArePresent) {
        List<BuildMaterial> mats = FloorBehaviour.getRequiredMaterialsAtState(floor);
        if (FloorBehaviour.takeItemsFromCreature(performer, floor, act, mats, justCheckIfItemsArePresent)) {
            return true;
        }
    ...
    static boolean takeItemsFromCreature(Creature performer, Floor floor, Action act, List<BuildMaterial> mats, boolean justCheckIfItemsArePresent) {
        ...
        block0: for (Item item : inventoryItems) {
        ...
        block2: for (Item item : bodyItems) {
            for (BuildMaterial mat : mats) {
                if (mat.getNeededQuantity() <= 0 || item.getTemplateId() != mat.getTemplateId() || item.getWeightGrams() < mat.getWeightGrams()) continue;
                takeItemsOnSuccess.add(item);
                mat.setNeededQuantity(0);
    ...
    static byte getFinishedState(Floor floor) {
        int numStates = 0;
        List<BuildMaterial> mats = FloorBehaviour.getRequiredMaterialsFor(floor);
        for (BuildMaterial mat : mats) {
            if (numStates >= mat.getTotalQuantityRequired()) continue;
            numStates = (byte)mat.getTotalQuantityRequired();
        }
        if (numStates <= 0) {
            numStates = 1;
        }
        return (byte)numStates;
    }
    ...
    public static final List<BuildMaterial> getRequiredMaterialsForRoof(StructureConstants.FloorMaterial material) {
        ArrayList<BuildMaterial> toReturn = new ArrayList<BuildMaterial>();
        try {
            switch (material) {
                case WOOD: {
        ...
        try {
            if (type == StructureConstants.FloorType.OPENING) {
                switch (material) {
                    ...
                    default: {
                        logger.log(Level.WARNING, "Someone tried to make a floor with an opening but the material choice was not supported (" + material.toString() + ")");
                        break;
                    }
                }
                ...
                switch (material) {
                    ...
                    default: {
                        logger.log(Level.WARNING, "Someone tried to make a staircase with an opening but the material choice was not supported (" + material.toString() + ")");
                        break;
                    }
                }
    ...
    static List<BuildMaterial> getRequiredMaterialsFor(Floor floor) {
        if (floor.getType() == StructureConstants.FloorType.ROOF) {
            return FloorBehaviour.getRequiredMaterialsForRoof(floor.getMaterial());
        }
        return FloorBehaviour.getRequiredMaterialsForFloor(floor.getType(), floor.getMaterial());
    }

    static List<BuildMaterial> getRequiredMaterialsForFloor(Floor floor) {
        List<BuildMaterial> toReturn = FloorBehaviour.getRequiredMaterialsForFloor(floor.getType(), floor.getMaterial());
        return toReturn;
    }

    public static List<BuildMaterial> getRequiredMaterialsAtState(Floor floor) {
        if (floor.getType() == StructureConstants.FloorType.ROOF) {
            return FloorBehaviour.getRequiredMaterialsAtStateForRoof(floor);
        }
        return FloorBehaviour.getRequiredMaterialsAtStateForFloor(floor);
    }

    public static List<BuildMaterial> getRequiredMaterialsAtStateForRoof(Floor floor) {
        List<BuildMaterial> mats = FloorBehaviour.getRequiredMaterialsForRoof(floor.getMaterial());
        for (BuildMaterial mat : mats) {
    ...
    public static List<BuildMaterial> getRequiredMaterialsAtStateForFloor(Floor floor) {
        List<BuildMaterial> mats = FloorBehaviour.getRequiredMaterialsForFloor(floor.getType(), floor.getMaterial());
        for (BuildMaterial mat : mats) {
            int qty = mat.getTotalQuantityRequired();
            if (floor.getState() > 0) {
                qty -= floor.getState();
    ...
    static final boolean isOkToBuild(Creature performer, Item tool, Floor floor, int floorLevel, boolean roof) {
        ...
        if (!FloorBehaviour.hasValidTool(floor.getMaterial(), tool)) {
        ...
        Skill buildSkill = FloorBehaviour.getBuildSkill(floor.getType(), floorMaterial, performer);
        if (!FloorBehaviour.mayPlanAtLevel(performer, floorLevel, buildSkill, roof)) {
            return false;
        }
        if (buildSkill.getKnowledge(0.0) < (double)FloorBehaviour.getRequiredBuildSkillForFloorType(floorMaterial)) {
            if (floor.getMaterial() == StructureConstants.FloorMaterial.STANDALONE) {
    ...
    public static final boolean mayPlanAtLevel(Creature performer, int floorLevel, Skill buildSkill, boolean roof) {
        return FloorBehaviour.mayPlanAtLevel(performer, floorLevel, buildSkill, roof, true);
    }
    ...
    public static final boolean mayPlanAtLevel(Creature performer, int floorLevel, Skill buildSkill, boolean roof, boolean sendMessage) {
        if (buildSkill.getKnowledge(0.0) < (double)FloorBehaviour.getRequiredBuildSkillForFloorLevel(floorLevel, roof)) {
            if (sendMessage) {
    ...
    private static final boolean floorBuilding(Action act, Creature performer, Item source, Floor floor, short action, float counter) {
        ...
        int time = 10;
        boolean bl = insta = !(!Servers.isThisATestServer() && performer.getPower() < 4 || performer.getPower() <= 1 || source.getTemplateId() != 315 && source.getTemplateId() != 176);
        if (floor.isFinished()) {
        ...
        if (counter == 1.0f) {
            ...
            if (!FloorBehaviour.advanceNextState(performer, floor, act, true) && !insta) {
                String message = FloorBehaviour.buildRequiredMaterialString(floor, false);
            ...
            Skill buildSkill = FloorBehaviour.getBuildSkill(floor.getType(), floor.getMaterial(), performer);
            time = Actions.getSlowActionTime(performer, buildSkill, source, 0.0);
            act.setTimeLeft(time);
            performer.getStatus().modifyStamina(-1000.0f);
            FloorBehaviour.damageTool(performer, floor, source);
            ...
            if (act.currentSecond() % 5 == 0) {
                SoundPlayer.playSound(FloorBehaviour.getBuildSound(floor), floor.getTileX(), floor.getTileY(), performer.isOnSurface(), 1.6f);
                performer.getStatus().modifyStamina(-1000.0f);
                FloorBehaviour.damageTool(performer, floor, source);
            }
        }
        if (counter * 10.0f > (float)time || insta) {
            String message = FloorBehaviour.buildRequiredMaterialString(floor, false);
            if (!FloorBehaviour.advanceNextState(performer, floor, act, false) && !insta) {
                if (floor.getType() == StructureConstants.FloorType.WIDE_STAIRCASE) {
                    performer.getCommunicator().sendNormalServerMessage("You need " + message + " to build the " + floor.getName() + " with " + floor.getMaterial().getName().toLowerCase() + ".");
                } else {
                    performer.getCommunicator().sendNormalServerMessage("You need " + message + " to build the " + floor.getName() + " of " + floor.getMaterial().getName().toLowerCase() + ".");
                }
                performer.getCommunicator().sendActionResult(false);
                return true;
            }
            double bonus = 0.0;
            Skill toolSkill = FloorBehaviour.getToolSkill(floor, performer, source);
            if (toolSkill != null) {
            ...
            float oldql = floor.getQualityLevel();
            float qlevel = MethodsStructure.calculateNewQualityLevel(act.getPower(), buildSkill.getKnowledge(0.0), oldql, FloorBehaviour.getTotalMaterials(floor));
            qlevel = Math.max(1.0f, qlevel);
            floor.setQualityLevel(qlevel);
            if (floor.getState() >= FloorBehaviour.getFinishedState(floor)) {
                if (insta) {
    ...
    public static final float getRequiredBuildSkillForFloorLevel(int floorLevel, boolean roof) {
        int fLevel;
        int n = fLevel = roof ? floorLevel - 1 : floorLevel;
        if (fLevel <= 0) {
            return 5.0f;
        }
        switch (fLevel) {
    ...
    public static final Skill getBuildSkill(StructureConstants.FloorType floorType, StructureConstants.FloorMaterial floorMaterial, Creature performer) {
        int primSkillTemplate = floorType == StructureConstants.FloorType.ROOF ? FloorBehaviour.getSkillForRoof(floorMaterial) : FloorBehaviour.getSkillForFloor(floorMaterial);
        ...
        if (action == 523 || action == 522) {
            ...
            if (counter == 1.0f) {
                ...
                if (floor.getMaterial() == StructureConstants.FloorMaterial.STANDALONE) {
                    ...
                    boolean bl = getCloser = qx < 1.0f || qx > 3.0f || qy < 1.0f || qy > 3.0f;
                }
                if (getCloser) {
                    performer.getCommunicator().sendNormalServerMessage("Move a little bit closer to the ladder.", (byte)3);
                    return true;
                ...
                if (action == 523) {
                    ...
                    if (performer.getFloorLevel() - 1 == 0) {
                        ...
                        int tfloor;
                        VolaTile t = performer.getCurrentTile();
                        if (t.getFloors((tfloor = (performer.getFloorLevel() - 1) * 30) - 10, tfloor + 10).length == 0) {
    ...
    public boolean action(Action act, Creature performer, Item source, boolean onSurface, Floor floor, int encodedTile, short action, float counter) {
        ...
        if (!FloorBehaviour.isConstructionAction(action)) {
        ...
        if (action == 524 || action == 525) {
            return FloorBehaviour.actionDestroyFloor(act, performer, source, floor, action, counter);
        }
        if (action == 193) {
            if (!(Servers.localServer.challengeServer && performer.getEnemyPresense() > 0 || floor.isNoRepair())) {
        ...
        if (action == 659 || action == 704 || action == 705 || action == 706 || action == 709 || action == 710 || action == 711 || action == 712 || action == 713 || action == 714 || action == 715) {
            ...
            StructureConstants.FloorType ft = action == 704 ? StructureConstants.FloorType.WIDE_STAIRCASE : (action == 705 ? StructureConstants.FloorType.RIGHT_STAIRCASE : (action == 706 ? StructureConstants.FloorType.LEFT_STAIRCASE : (action == 709 ? StructureConstants.FloorType.CLOCKWISE_STAIRCASE : (action == 710 ? StructureConstants.FloorType.ANTICLOCKWISE_STAIRCASE : (action == 711 ? StructureConstants.FloorType.CLOCKWISE_STAIRCASE_WITH : (action == 712 ? StructureConstants.FloorType.ANTICLOCKWISE_STAIRCASE_WITH : (action == 713 ? StructureConstants.FloorType.WIDE_STAIRCASE_RIGHT : (action == 714 ? StructureConstants.FloorType.WIDE_STAIRCASE_LEFT : (action == 715 ? StructureConstants.FloorType.WIDE_STAIRCASE_BOTH : StructureConstants.FloorType.STAIRCASE)))))))));
        ...
        if (action - 20000 >= 0) {
            return this.buildAction(act, performer, source, floor, action, counter);
        }
        return true;
    }
    ...
    private boolean examine(Creature performer, Floor floor) {
        String materials = "";
        if (floor.getFloorState() == StructureConstants.FloorState.BUILDING) {
            materials = FloorBehaviour.buildRequiredMaterialString(floor, true);
            ...
            performer.getCommunicator().sendNormalServerMessage("It is a normal " + floor.getName() + " made of " + FloorBehaviour.getMaterialDescription(floor).toLowerCase() + ".");
        }
        FloorBehaviour.sendQlString(performer, floor);
        return true;
    }
    ...
    private static final String buildRequiredMaterialString(Floor floor, boolean detailed) {
        ...
        for (BuildMaterial mat : billOfMaterial) {
            ...
            if (numMats > 1) {
                description = numMats < maxMats ? description + ", " : description + " and ";
            }
            if (template != null) {
                if (detailed) {
