...
public final class BodyPartBehaviour extends ItemBehaviour {
   ...
   public List<ActionEntry> getBehavioursFor(Creature performer, Item source, Item object) {
      ...
      if (source.isHolyItem() && object.isBodyPartAttached()) {
         ...
         && (source.isMagicStaff() || source.getTemplateId() == 176 && performer.getPower() >= 4 && Servers.isThisATestServer())) {
      ...
      if (object.isBodyPartAttached() && source.getTemplate().isRune()) {
         ...
         if (RuneUtilities.isSingleUseRune(source)
            && (
      ...
      if (target.isBodyPartRemoved()) {
         ...
         if (action != 945
            ...
            if ((action == 6 || action == 7) && target.isBodyPartAttached()) {
               ...
               return done;
            } else if (action == 744 && source.canHaveInscription()) {
               return PapyrusBehaviour.addToCookbook(act, performer, source, target, action, counter);
            } else {
               return action != 1 && action != 494 && action != 495 && action != 511 && action != 566
                  ? super.action(act, performer, source, target, action, counter)
                  : this.action(act, performer, target, action, counter);
            }
         } else {
      ...
      if (target.isBodyPartRemoved()) {
         ...
         if (action == 1) {
            ...
            if (target.isBodyPartAttached()) {
                  ...
                  if (owner.equals(performer)) {
                     ...
                     if (owner.isPlayer()) {
                        performer.getCommunicator()
                           .sendNormalServerMessage("Rank=" + ((Player)owner).getRank() + ", Max Rank=" + ((Player)owner).getMaxRank());
                     }
               ...
               if (performer.getPower() >= 3) {
                  ...
                  if (performer.getCurrentTile() != null) {
                     performer.getCommunicator()
                        .sendNormalServerMessage("Tile position x=" + owner.getCurrentTile().tilex + ", " + owner.getCurrentTile().tiley);
                  } else {
                  ...
                  performer.getCommunicator()
                     .sendNormalServerMessage(
                        "Layer="
                           ...
                           + owner.getVehicle()
                           + " Origin server "
                           + WurmId.getOrigin(performer.getWurmId())
                     );
                  if (owner.getVisionArea() != null) {
                     performer.getCommunicator()
                        .sendNormalServerMessage(
                           "Visionarea initialized="
                              + owner.getVisionArea().isInitialized()
                              + ". Surface startx="
                              + owner.getVisionArea().getSurface().getStartX()
                              + ", startY="
                              + owner.getVisionArea().getSurface().getStartY()
                              + ", endX="
                              + owner.getVisionArea().getSurface().getEndX()
                              + ", endY="
                              + owner.getVisionArea().getSurface().getEndY()
                        );
                  } else {
            ...
            if (performer.getCultist() != null && performer.getCultist().mayStartDoubleWarDamage()) {
               ...
               performer.getCultist().touchCooldown1();
               performer.getCommunicator()
                  .sendAddSpellEffect(SpellEffectsEnum.HATE_DOUBLE_WAR, performer.getCultist().getDoubleWarDamageTimeLeftSeconds(), 100.0F);
            }
         } else if (action == 391) {
            if (performer.getCultist() != null && performer.getCultist().mayStartDoubleStructDamage()) {
               performer.getCommunicator().sendNormalServerMessage("You feel the rage pulsate within.");
               ...
               performer.getCommunicator()
                  .sendAddSpellEffect(SpellEffectsEnum.HATE_DOUBLE_STRUCT, performer.getCultist().getDoubleStructDamageTimeLeftSeconds(), 100.0F);
            }
         } else if (action == 392) {
            if (performer.getCultist() != null && performer.getCultist().mayStartFearEffect()) {
               ...
               performer.refreshAttitudes();
               performer.getCommunicator()
                  .sendAddSpellEffect(SpellEffectsEnum.HATE_FEAR_EFFECT, performer.getCultist().getFearEffectTimeLeftSeconds(), 100.0F);
            }
         } else if (action == 393) {
            if (performer.getCultist() != null && performer.getCultist().mayStartNoElementalDamage()) {
               ...
               performer.getCultist().touchCooldown1();
               performer.getCommunicator()
                  .sendAddSpellEffect(SpellEffectsEnum.POWER_NO_ELEMENTAL, performer.getCultist().getElementalImmunityTimeLeftSeconds(), 100.0F);
            }
         } else if (action == 394) {
            if (performer.getCultist() != null && performer.getCultist().mayStartIgnoreTraps()) {
               ...
               performer.getCultist().touchCooldown3();
               performer.getCommunicator()
                  .sendAddSpellEffect(SpellEffectsEnum.POWER_IGNORE_TRAPS, performer.getCultist().getIgnoreTrapsTimeLeftSeconds(), 100.0F);
            }
         } else if (action == 396) {
            if (performer.getCultist() != null && performer.getCultist().mayFillup()) {
            ...
            if (action == 95) {
               if (performer.getCultist() != null && performer.getCultist().mayTeleport()) {
                  ...
                  int tiley = Zones.safeTileY(Server.rand.nextInt(Zones.worldTileSizeY));
                  ...
                  Server.getInstance()
                     .broadCastAction(performer.getName() + " shuts " + performer.getHisHerItsString() + " eyes, dreaming away.", performer, 5);
               ...
               if (performer.getCultist() != null && performer.getCultist().mayRecall()) {
                  ...
                  if (performer.getCitizenVillage() != null) {
                     ...
                     if (performer.startTeleporting()) {
                        ...
                        Server.getInstance()
                           .broadCastAction(performer.getName() + " shuts " + performer.getHisHerItsString() + " eyes and suddenly disappears.", performer, 5);
                        performer.getCommunicator().sendTeleport(false);
