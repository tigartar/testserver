...
AttitudeConstants {
    ...
    static final void teleportSet(Creature performer, Item source, int tilex, int tiley) {
        if (source.getTemplateId() == 174) {
            ...
            } else if (Servers.localServer.PVPSERVER && EndGameItems.getGoodAltar() != null && performer.isWithinDistanceTo((egi = EndGameItems.getGoodAltar()).getItem().getPosX(), egi.getItem().getPosY(), egi.getItem().getPosZ(), 50.0f)) {
                performer.getCommunicator().sendNormalServerMessage("The magic of this place interferes. You can not use the " + source.getName() + " here.");
                return;
            }
            if (Tiles.isOreCave(Tiles.decodeType(tilenum)) && !performer.isOnSurface() && (tile = performer.getCurrentTile()) != null && tile.getStructure() == null && (Math.abs(tilex - tile.tilex) <= 1 || Math.abs(tiley - tile.tiley) <= 1)) {
                tilex = tile.tilex;
                tiley = tile.tiley;
                ok = true;
            ...
            if (performer.isOnSurface() && (tile = performer.getCurrentTile()) != null && tile.getStructure() == null && (Math.abs(tilex - tile.tilex) <= 1 || Math.abs(tiley - tile.tiley) <= 1) && Tiles.isTree(Tiles.decodeType(tilenum = Server.surfaceMesh.getTile(tilex, tiley)))) {
                tilex = tile.tilex;
                tiley = tile.tiley;
                ok = true;
            ...
            if (performer.getVisionArea() != null && performer.getVisionArea().isInitialized()) {
                ...
                if (tx >= 0 && ty >= 0 && tx < 1 << Constants.meshSize && ty < 1 << Constants.meshSize) {
                    ...
                    if ((source.getTemplateId() == 176 || source.getTemplateId() == 315) && performer.getPower() >= 2 || (source.getTemplateId() == 174 || source.getTemplateId() == 1027) && performer.getPower() >= 1) {
                        ...
                        barid = 0L;
                        layer = source.getTemplateId() == 525 ? -1 : 0;
                    } else {
                        Item[] inventoryItems;
                        for (Item lInventoryItem : inventoryItems = performer.getInventory().getAllItems(false)) {
                            if (lInventoryItem.getTemplateId() != 45 || lInventoryItem.getWeightGrams() < lInventoryItem.getTemplate().getWeightGrams()) continue;
                            barid = lInventoryItem.getWurmId();
                            break;
                        }
                        if (barid == -10L) {
                            Item[] bodyItems;
                            for (Item lBodyItem : bodyItems = performer.getBody().getAllItems()) {
                                if (lBodyItem.getTemplateId() != 45 || lBodyItem.getWeightGrams() < lBodyItem.getTemplate().getWeightGrams()) continue;
                    ...
                    if (barid == -10L) {
                        ...
                        if (source.getTemplateId() != 176 && source.getTemplateId() != 315 && source.getTemplateId() != 1027) {
                            if (MethodsCreatures.mayTelePortToTile(performer, tx, ty, layer == 0)) {
                                if (barid > 0L) {
    ...
    static String getElapsedTimeString(int hours) {
        String timeString = hours > 48 ? "more than two days ago." : (hours > 24 ? "more than a day ago." : (hours > 12 ? "more than half a day ago." : (hours > 6 ? "more than six hours ago." : (hours > 3 ? "more than three hours ago." : (hours > 2 ? "more than two hours ago." : (hours > 1 ? "more than an hour ago." : "less than an hour ago."))))));
        return timeString;
    }
        ...
        block15: {
            ...
            if (Tiles.decodeHeight(tile) > 0) {
                try {
                    ...
                    if (!(counter * 10.0f > (float)time) && performer.getPower() <= 0) break block15;
                    ...
                    if (tracks.length > 0) {
                        ...
                        for (int x = 0; x < min; ++x) {
                            if (!tracks[x].getCreatureName().equals(performer.getName())) {
                                ...
                                String direction = MethodsCreatures.getDirectionStringFor(rot, dir);
                                long diff = System.currentTimeMillis() - tracks[x].getTime();
                                int gameSeconds = (int)(diff / 125L);
                                int hours = gameSeconds / 3600;
                                String timeString = MethodsCreatures.getElapsedTimeString(hours);
                                if (performer.getPower() > 0) {
    ...
    static final boolean firstAid(Creature performer, Item bandaid, Wound wound, float counter, Action act) {
        ...
        if (bandaid.isHealingSalve() && (wound.isInternal() || wound.isBruise() || wound.isPoison())) {
            ...
            if ((nums = bandaid.getRarity() + act.getRarity() + bandaid.getWeightGrams() / 10) < wound.getNumBandagesNeeded() && performer instanceof Player) {
            ...
            nums = bandaid.getRarity() + act.getRarity() + bandaid.getWeightGrams() / 100;
        ...
        if (wound.getCreature() == null) {
            ...
            if (counter == 1.0f) {
                ...
                if (wound.getSeverity() > 3275.0f) {
                    time = wound.getSeverity() < 9825.0f ? 100 : (wound.getSeverity() < 19650.0f ? 150 : (wound.getSeverity() < 29475.0f ? 300 : 450));
                }
                act.setTimeLeft(time);
                targetName = wound.getCreature().getNameWithGenus();
            ...
            if (counter > 5.0f && counter * 10.0f > (float)time) {
                ...
                if (power > 0.0) {
                    ...
                    wound.modifySeverity((int)Math.min((double)(-wound.getSeverity() / 10.0f), -((double)(3275.0f * (float)rarity) + power * (double)(performer.getCultist() != null && performer.getCultist().healsFaster() ? 100.0f : 50.0f))));
            ...
            if (counter > 5.0f && counter * 10.0f > (float)time) {
                ...
                if (power > 0.0 || Server.rand.nextInt(100) < 20) {
                    ...
                    if (!wound.isAcidWound()) {
                        wound.setHealeff((byte)((float)cover.getAuxData() + act.getPower() + (float)cover.getRarity()));
                    } else if (cover.getTemplateId() == 128) {
                        if ((float)cover.getWeightGrams() > wound.getSeverity() / 10.0f) {
                            wound.setType((byte)4);
                            wound.setHealeff((byte)(18.0f + act.getPower() + (float)cover.getRarity()));
                        } else {
    ...
    static boolean shear(Action act, Item source, Creature performer, Creature target, float counter) {
        ...
        if (counter * 10.0f > (float)time) {
            ...
            int numberOfWoolProduced = MethodsCreatures.getWoolProduceForAge(target.getStatus().age);
            int knowledge = (int)animalHusbandry.getKnowledge(0.0);
            float diff = MethodsCreatures.getShearingDifficulty(target.getStatus().age, knowledge);
            double power = animalHusbandry.skillCheck(diff, source, 0.0, false, counter);
    ...
    static boolean milk(Action act, Item tool, Creature performer, Creature target, float counter) {
        ...
        if (cont.length < 2) {
            if (cont.length == 1) {
                ...
                if (!(cont[0].getTemplateId() == 142 && target.getTemplate().getTemplateId() == 3 || cont[0].getTemplateId() == 1012 && target.getTemplate().getTemplateId() == 96 || cont[0].getTemplateId() == 1013 && target.getTemplate().getTemplateId() == 82)) {
        ...
        if (counter == 1.0f) {
            ...
            int maxSearches = MethodsCreatures.calcMaxPasses(target, skill.getKnowledge(0.0), tool);
        ...
        if (counter * 10.0f >= act.getNextTick()) {
            ...
            int diff = MethodsCreatures.getShearingDifficulty(target.getStatus().age, knowledge);
            ...
            double power = skill.skillCheck(diff, tool, 0.0, false, counter / (float)searchCount);
            try {
                ...
                logger.log(Level.WARNING, "No template for " + milkType, nst);
    ...
    private static int calcMaxPasses(Creature creature, double currentSkill, Item tool) {
        ...
        return Math.min(MethodsCreatures.getCreatureAgeFactor(creature), (int)(currentSkill + 28.0) / 27 + bonus);
    }
    ...
    static final void sendAskKingdomQuestion(Creature performer, Creature target) {
        ...
        question.sendQuestion();
        if (performer.isPlayer()) {
            ((Player)performer).lastSentQuestion = (short)60;
        }
    }
    ...
    static final boolean tame(Action act, Creature performer, Creature target, Item food, float counter) {
        ...
        if (!Servers.isThisAPvpServer() && (bVill = target.getBrandVillage()) != null && !bVill.isActionAllowed((short)46, performer)) {
        ...
        if (counter * 10.0f > (float)time) {
            ...
            if ((power = taming.skillCheck(soul.getKnowledge() + (double)difficultyBonus, bonus, dryrun, counter)) > 0.0) {
                double maxpower = taming.getKnowledge(0.0);
                MethodsCreatures.tameEffect(performer, target, power, dryrun, maxpower);
            } else {
                    ...
                    if (receiver.getPet() == null) {
                        if (dominator.getPet() != null) {
                            try {
                                ...
                                logger.log(Level.WARNING, dominator.getName() + " and " + receiver.getName() + ": " + ex.getMessage(), ex);
                            }
                        } else {
                ...
                if (performer.getCurrentTile().isTransition) {
                    ...
                    if (tilePosX != performer.getCurrentTile().tilex || tilePosY != performer.getCurrentTile().tiley) {
                        ...
                        try {
                            ...
                            Path path = pf.findPath(performer, performer.getTileX(), (int)performer.getStatus().getPositionY() >> 2, tilePosX, tilePosY, performer.isOnSurface(), 10);
                            if (path != null) {
                                while (!path.isEmpty()) {
                                    try {
                                        ...
                                        logger.log(Level.INFO, performer.getName() + " " + fe.getMessage(), fe);
                                        orderer.getCommunicator().sendNormalServerMessage("Failed to create marker.");
                                    }
                                    catch (NoSuchTemplateException nst) {
                                        logger.log(Level.INFO, performer.getName() + " " + nst.getMessage(), nst);
                                        orderer.getCommunicator().sendNormalServerMessage("Failed to create marker.");
                                    }
                                }
                                break block12;
    ...
    static final boolean stealth(Creature performer, float counter) {
        ...
        if (counter == 1.0f) {
            ...
            if (t != null) {
                Set<VolaTile> set = t.getThisAndSurroundingTiles(1);
                for (VolaTile lVolaTile : set) {
                    Creature[] crets;
                    for (Creature lCret : crets = lVolaTile.getCreatures()) {
                        if (lCret == performer || lCret.getPower() > 0) continue;
            ...
            VolaTile t = performer.getCurrentTile();
            if (t != null) {
                double mod = MethodsCreatures.getStealthTerrainModifier(performer, t.tilex, t.tiley, performer.isOnSurface());
                if ((double)(Server.rand.nextFloat() * 100.0f) < mod) {
                    ...
                    try {
                        ...
                        if (wornl != null) {
                            for (Item i : wornl) {
                                if (i.getTemplateId() != 297 || i.getMaterial() != 7) continue;
                                performer.achievement(93);
                                found = true;
                                break;
                            }
                        }
                        if (!found && (wornr = (rhand = performer.getBody().getBodyPart(14)).getItems()) != null) {
                            for (Item i : wornr) {
                                if (i.getTemplateId() != 297 || i.getMaterial() != 7) continue;
                                performer.achievement(93);
                            }
                ...
                if (!target.isPregnant()) {
                    if (!breeder.isPregnant()) {
                        ...
                        if (counter == 1.0f) {
                            ...
                            double d = breeder.getBaseCombatRating();
                            float f = performer.getPower() >= 5 ? 20.0f : counter;
                            if (breeding.skillCheck(d, 0.0, false, f) > 0.0) {
                                if (breeder.getSex() == 1) {
                    ...
                    if (action == 331) {
                        ...
                        if (VehicleBehaviour.mayDriveVehicle(performer, target)) {
                            if (VehicleBehaviour.canBeDriverOfVehicle(performer, vehicle)) {
                                ...
                                for (int x = 0; x < vehicle.seats.length; ++x) {
                                    if (vehicle.seats[x].isOccupied() || vehicle.seats[x].type != 0) continue;
                                    ...
                                    BlockingResult result = Blocking.getBlockerBetween(performer, performer.getPosX(), performer.getPosY(), newposx, newposy, performer.getPositionZ(), target.getPositionZ(), performer.isOnSurface(), target.isOnSurface(), false, 4, -1L, performer.getBridgeId(), target.getBridgeId(), false);
                                    if (result != null) {
                                    ...
                                    if (target.getTemplateId() != 21) break;
                                ...
                                performer.getCommunicator().sendNormalServerMessage(StringUtil.format("You are not agile enough to control the %s. You need %.2f in %s.", vehicName, Float.valueOf(vehicle.skillNeeded), SkillSystem.getNameFor(104)), (byte)3);
                            }
                        } else {
                        ...
                        if (VehicleBehaviour.mayEmbarkVehicle(performer, target)) {
                            if (performer.getDraggedItem() == null) {
                                ...
                                for (int x = 0; x < vehicle.seats.length; ++x) {
                                    if (vehicle.seats[x].isOccupied() || vehicle.seats[x].type != 1) continue;
                                    float r = -(target.getStatus().getRotation() + 180.0f) * (float)Math.PI / 180.0f;
                                    float s = (float)Math.sin(r);
                                    float c = (float)Math.cos(r);
                                    float xo = s * -vehicle.seats[x].offx - c * -vehicle.seats[x].offy;
    ...
    public static final float getStealthTerrainModifier(Creature performer, int tilex, int tiley, boolean surfaced) {
        ...
        for (int x = -1; x <= 1; ++x) {
            for (int y = -1; y <= 1; ++y) {
                ...
                if (!surfaced || tilex <= 0 || tilex >= Zones.worldTileSizeX - 1 || tiley <= 0 || tiley >= Zones.worldTileSizeY - 1) continue;
                int tt = Server.surfaceMesh.getTile(tilex + x, tiley + y);
                byte type = Tiles.decodeType(tt);
                byte data = Tiles.decodeData(tt);
                Tiles.Tile theTile = Tiles.getTile(type);
                if (!theTile.isTree() || FoliageAge.getAgeAsByte(data) <= FoliageAge.MATURE_TWO.getAgeId()) continue;
                mod += 5.0f;
            }
        }
        if (!surfaced) {
    ...
    public static final boolean groom(Creature performer, Creature target, Item groomTool, short action, Action act, float counter) {
        ...
        if (target.canBeGroomed()) {
            ...
            if (act.currentSecond() % 15 == 0) {
                ...
                toReturn = true;
                double d = target.getBaseCombatRating();
                float f = performer.getPower() >= 5 ? 20.0f : counter / 2.0f;
                if (breeding.skillCheck(d, groomTool, 0.0, false, f) > 0.0) {
                    target.setLastGroomed(System.currentTimeMillis());
