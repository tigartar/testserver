...
public final class BridgePartBehaviour extends TileBehaviour implements QuestionTypes {
   ...
   public List<ActionEntry> getBehavioursFor(Creature performer, Item source, boolean onSurface, BridgePart bridgePart) {
      ...
      if (MethodsStructure.mayModifyStructure(performer, structure, floorTile, (short)169)) {
         if (bpState.isBeingBuilt()) {
            ...
            toReturn.add(
               new ActionEntry(
                  (short)(20000 + bridgePart.getMaterial().getCode()),
                  "Build " + bridgePart.getMaterial().getName().toLowerCase() + " " + bridgePart.getType().getName(),
                  bridgePart.getMaterial().getName() + " " + bridgePart.getType().getName(),
                  emptyIntArr
               )
            );
         }
      ...
      if (!source.isTraded()) {
         ...
         if (bridgePart.isFinished()
            && bridgePart.getMaterial() != BridgeConstants.BridgeMaterial.ROPE
            && bridgePart.getMaterial() != BridgeConstants.BridgeMaterial.WOOD) {
            byte roadType = bridgePart.getRoadType();
            if ((
                  source.getTemplateId() == 492
               ...
               toReturn.add(new ActionEntry((short)155, "Prepare", "preparing the floor"));
                  ...
                  source.getTemplateId() == 97
                     || WurmPermissions.mayUseDeityWand(performer) && source.getTemplateId() == 176
                     || WurmPermissions.mayUseGMWand(performer) && source.getTemplateId() == 315
               )
               && roadType == Tiles.Tile.TILE_PREPARED_BRIDGE.id) {
               ...
               toReturn.add(Actions.actionEntrys[155]);
                  ...
                  source.getTemplateId() == 1115
                     || WurmPermissions.mayUseDeityWand(performer) && source.getTemplateId() == 176
                     || WurmPermissions.mayUseGMWand(performer) && source.getTemplateId() == 315
               )
               && Tiles.isRoadType(roadType)) {
   ...
   private boolean buildAction(Action act, Creature performer, Item source, BridgePart bridgePart, short action, float counter) {
      if (bridgePart.getBridgePartState() == BridgeConstants.BridgeState.PLANNED) {
            ...
            performer.getCommunicator()
               .sendNormalServerMessage(
                  "You Cannot build a "
                     + bridgePart.getName()
                     + " made of "
                     + BridgeConstants.BridgeMaterial.fromByte((byte)(action - 20000)).getName().toLowerCase()
                     + " as It's already planned to be made of "
                     + bridgePart.getMaterial().getName().toLowerCase()
                     + "."
               );
            return true;
         }
         ...
         BuildAllMaterials bam = getRequiredMaterials(bridgePart);
   ...
   static boolean advanceNextState(Creature performer, BridgePart bridgePart, Action act, boolean justCheckIfItemsArePresent) {
      BuildAllMaterials mats = getRequiredMaterials(bridgePart);
      if (takeItemsFromCreature(performer, bridgePart, act, mats, justCheckIfItemsArePresent)) {
         return true;
      } else if (performer.getPower() >= 2 && !justCheckIfItemsArePresent && Servers.isThisATestServer()) {
   ...
   public static BuildAllMaterials getRequiredMaterials(BridgePart bridgePart) {
      BuildAllMaterials toReturn = getRequiredMaterials(bridgePart.getType(), bridgePart.getMaterial(), bridgePart.getNumberOfExtensions());
      toReturn.setNeeded(bridgePart.getState(), bridgePart.getMaterialCount());
      return toReturn;
   }

   public static BuildAllMaterials getRequiredMaterials(
      BridgeConstants.BridgeType bridgeType, BridgeConstants.BridgeMaterial bridgeMaterial, int bridgeExtensions
   ) {
      ...
      try {
         ...
         logger.log(Level.WARNING, "BridgePartBehaviour.getRequiredMaterials trying to use material that have a non existing template.", (Throwable)var12);
      }
   ...
   static final boolean isOkToBuild(Creature performer, Item tool, BridgePart bridgePart) {
      if (bridgePart == null) {
         ...
         return false;
      } else if (!hasValidTool(bridgePart.getMaterial(), tool)) {
         ...
         Skill buildSkill = getBuildSkill(bridgePart.getType(), bridgePart.getMaterial(), performer);
         if (buildSkill.getKnowledge(0.0) < (double)bridgePart.minRequiredSkill()) {
            ...
            return false;
         } else if (performer.getTileX() == bridgePart.getTileX() && performer.getTileY() == bridgePart.getTileY()) {
            performer.getCommunicator().sendNormalServerMessage("You cannot work on the bridge part when standing on the same tile.");
            return false;
         } else if (performer.getBridgeId() == bridgePart.getStructureId()) {
            return true;
         } else if (!bridgePart.hasAnExit()) {
   ...
   private static final boolean bridgePartBuilding(Action act, Creature performer, Item source, BridgePart bridgePart, short action, float counter) {
      if (!isOkToBuild(performer, source, bridgePart)) {
         ...
         int time = 10;
         boolean insta = (Servers.isThisATestServer() && performer.getPower() > 1 || performer.getPower() >= 4)
         ...
         if (bridgePart.isFinished()) {
            ...
            return true;
         } else if (!Methods.isActionAllowed(performer, (short)116, bridgePart.getTileX(), bridgePart.getTileY())) {
            return true;
         } else {
            if (counter == 1.0F) {
               try {
                  Structures.getStructure(bridgePart.getStructureId());
               } catch (NoSuchStructureException var24) {
            ...
            if (!(counter * 10.0F > (float)time) && !insta) {
               ...
               if (!advanceNextState(performer, bridgePart, act, false) && !insta) {
                  ...
                  double bonus = 0.0;
                  Skill toolSkill = getToolSkill(bridgePart, performer, source);
                  if (toolSkill != null) {
                  ...
                  float divider = (float)bam.getTotalQuantityRequired();
                  float oldql = bridgePart.getQualityLevel();
                  float qlevel = Math.max(
                     1.0F, (float)Math.min((double)(act.getPower() + oldql), (double)oldql + buildSkill.getKnowledge(0.0) / (double)divider)
                  );
                  ...
                  if (bsm.isStageComplete(bridgePart)) {
                     ...
                     if (bam.getStageCount() - 1 >= bridgePart.getState() + 1) {
                        ...
                        if (insta) {
                           ...
                           if (!Servers.isThisATestServer()) {
                              logger.info(
                                 "Insta Bridge: " + performer.getName() + " completed bridge part at " + bridgePart.getTileX() + "," + bridgePart.getTileY()
                              );
                           }
                        }
   ...
   private static boolean hasValidTool(BridgeConstants.BridgeMaterial bridgeMaterial, Item source) {
      if (bridgeMaterial == BridgeConstants.BridgeMaterial.ROPE) {
         return true;
      } else if (source != null && bridgeMaterial != null) {
   ...
   public static boolean actionDestroyBridgePart(Action act, Creature performer, Item source, BridgePart bridgePart, short action, float counter) {
      if (!Methods.isActionAllowed(performer, (short)116, bridgePart.getTileX(), bridgePart.getTileY())) {
         return true;
      } else if (MethodsHighways.onHighway(bridgePart)) {
         performer.getCommunicator().sendNormalServerMessage("That bridge part is protected by the highway.");
         return true;
      } else if (bridgePart.getBridgePartState().isBeingBuilt()) {
         return (!WurmPermissions.mayUseDeityWand(performer) || source.getTemplateId() != 176)
               && (!WurmPermissions.mayUseGMWand(performer) || source.getTemplateId() != 315)
            ? MethodsStructure.destroyFloor(action, performer, source, bridgePart, counter)
            : maybeDestroy(performer, bridgePart);
      } else if (bridgePart.getBridgePartState() != BridgeConstants.BridgeState.COMPLETED) {
         return bridgePart.getBridgePartState() == BridgeConstants.BridgeState.PLANNED ? maybeDestroy(performer, bridgePart) : true;
      } else {
         return (!WurmPermissions.mayUseDeityWand(performer) || source.getTemplateId() != 176)
               && (!WurmPermissions.mayUseGMWand(performer) || source.getTemplateId() != 315)
            ? MethodsStructure.destroyFloor(action, performer, source, bridgePart, counter)
   ...
   public boolean action(Action act, Creature performer, boolean onSurface, BridgePart bridgePart, int encodedTile, short action, float counter) {
      if (action == 1) {
         ...
         if (action == 59) {
            ...
            if (bridge == null) {
               ...
               TextInputQuestion tiq = new TextInputQuestion(
                  performer, "Setting description for " + bridge.getName() + ".", "Set the new description:", 1, bridge.getWurmId(), 40, false
               );
         ...
         return super.action(
            ...
            bridgePart.getTileY(),
            onSurface,
            Zones.getTileIntForTile(bridgePart.getTileX(), bridgePart.getTileY(), 0),
            action,
            counter
         );
      }
   ...
   public boolean action(Action act, Creature performer, Item source, boolean onSurface, BridgePart bridgePart, int encodedTile, short action, float counter) {
      ...
      if (action == 1) {
         ...
         if ((source.getTemplateId() == 315 || source.getTemplateId() == 176) && performer.getPower() >= 2) {
            ...
            logger.log(
               Level.WARNING, performer.getName() + " hacking the protocol by trying to set the restrictions of " + bridgePart + ", counter: " + counter + '!'
            );
         }
         ...
         if (!this.isConstructionAction(action)) {
            return super.action(
               ...
               bridgePart.getTileY(),
               onSurface,
               bridgePart.getHeightOffset(),
               Zones.getTileIntForTile(bridgePart.getTileX(), bridgePart.getTileY(), 0),
               action,
               counter
            );
         } else if (action == 524 && !bridgePart.isIndestructible()) {
            return actionDestroyBridgePart(act, performer, source, bridgePart, action, counter);
         } else if (action == 193 && !bridgePart.isNoRepair()) {
            return MethodsStructure.repairFloor(performer, source, bridgePart, counter, act);
         } else if (action == 192 && !bridgePart.isNoImprove()) {
            return MethodsStructure.improveFloor(performer, source, bridgePart, counter, act);
         } else if (action != 169 && action - 20000 < 0) {
            if (action == 82
               && (source.getTemplateId() == 176 || source.getTemplateId() == 315)
               ...
               try {
                  ...
                  performer.getLogger()
                     .log(
                        Level.INFO,
                        performer.getName() + " destroyed bridge " + struct.getName() + " at " + bridgePart.getTileX() + ", " + bridgePart.getTileY()
                     );
   ...
   private boolean examine(Creature performer, BridgePart bridgePart) {
      ...
      if (bpState.isBeingBuilt()) {
         ...
         performer.getCommunicator()
            .sendNormalServerMessage(
               "You see a " + bridgePart.getName() + " under construction. The " + bridgePart.getName() + " requires " + allmaterials + " to be finished."
            );
         performer.getCommunicator()
            .sendNormalServerMessage(
               "The current stage being constructed is "
                  + bridgePart.getFloorStageAsString()
                  + "of "
                  + bam.getStageCountAsString()
                  + " and requires "
                  + stagematerials
                  + " to be completed."
            );
         ...
         performer.getCommunicator()
            .sendNormalServerMessage(
               "You see plans for a " + bridgePart.getName() + ". The " + bridgePart.getName() + " requires " + allmaterials + " to be finished."
            );
         performer.getCommunicator()
            .sendNormalServerMessage(
               "There are " + bam.getStageCountAsString() + " stages and the initial stage requires " + stagematerials + " to be completed."
            );
         sendQlString(performer, bridgePart);
      } else {
         ...
         performer.getCommunicator()
            .sendNormalServerMessage(
               "id: "
                  ...
                  + " ("
                  + bridgePart.getBridgePartState().toString().toLowerCase()
                  + ")."
            );
      }
   }
         ...
         if (pavingItem != 492 && (!insta || roadType != 0)) {
            if (pavingItem == 153) {
               if (source.getWeightGrams() < source.getTemplate().getWeightGrams()) {
                  performer.getCommunicator()
                     .sendNormalServerMessage(
                        "It takes " + source.getTemplate().getWeightGrams() / 1000 + "kg of " + source.getName() + " to colour the floorboards."
                     );
                  return true;
               }
               ...
               if (pavingItem != 1115 && (!insta || !Tiles.isRoadType(roadType))) {
                  if (source.getWeightGrams() < source.getTemplate().getWeightGrams()) {
                     performer.getCommunicator()
                        .sendNormalServerMessage(
                           "The amount of "
            ...
            if (!insta && source.getWeightGrams() < source.getTemplate().getWeightGrams()) {
               performer.getCommunicator()
                  .sendNormalServerMessage(
                     "It takes " + source.getTemplate().getWeightGrams() / 1000 + "kg of " + source.getName() + " to prepare the bridge part."
                  );
               return true;
            }
