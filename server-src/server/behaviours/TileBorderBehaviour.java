...
implements MiscConstants {
    ...
    public List<ActionEntry> getBehavioursFor(Creature performer, Item subject, int tilex, int tiley, boolean onSurface, Tiles.TileBorderDirection dir, boolean border, int heightOffset) {
        ...
        if (TileBorderBehaviour.hasHoleEachSide(tilex, tiley, onSurface, dir)) {
            return toReturn;
        }
        ...
        if (onSurface && (subject.isMineDoor() || subject.getTemplateId() == 315 || subject.getTemplateId() == 176) && (opening = Terraforming.getCaveOpeningCoords(tilex, tiley))[0] != -1 && opening[1] != -1 && !TileBorderBehaviour.isWideEntrance(opening[0], opening[1])) {
            toReturn.add(Actions.actionEntrys[363]);
        }
        if (!(hasMarker = TileBorderBehaviour.hasMarker(tilex, tiley, onSurface, dir)) && MethodsStructure.isCorrectToolForBuilding(performer, templateId)) {
            VolaTile vt;
            boolean ok = onSurface;
            if (!onSurface && (vt = Zones.getOrCreateTile(tilex, tiley, onSurface)).getVillage() != null) {
                ok = true;
            }
            if (ok) {
                toReturn.add(new ActionEntry(-12, "Fence", "Fence options"));
                toReturn.add(new ActionEntry(-5, "Iron", "Fence options"));
                ...
                toReturn.add(new ActionEntry(-2, "Log", "Fence options"));
                toReturn.add(Actions.actionEntrys[165]);
                toReturn.add(Actions.actionEntrys[167]);
                toReturn.add(new ActionEntry(-8, "Marble", "Fence options"));
                ...
                toReturn.add(new ActionEntry(-4, "Plank", "Fence options"));
                ...
                toReturn.add(new ActionEntry(-8, "Pottery", "Fence options"));
                ...
                toReturn.add(new ActionEntry(-2, "Rope", "Rope options"));
                toReturn.add(Actions.actionEntrys[544]);
                toReturn.add(Actions.actionEntrys[543]);
                toReturn.add(new ActionEntry(-8, "Rounded stone", "Fence options"));
                ...
                toReturn.add(new ActionEntry(-8, "Sandstone", "Fence options"));
                ...
                toReturn.add(new ActionEntry(-5, "Stone", "Fence options"));
                ...
                toReturn.add(new ActionEntry(-1, "Woven", "Fence options"));
    ...
    public boolean action(Action act, Creature performer, Item source, int tilex, int tiley, boolean onSurface, int heightOffset, Tiles.TileBorderDirection dir, long borderId, short action, float counter) {
        ...
        if (action == 1) {
        ...
        } else if (!TileBorderBehaviour.hasHoleEachSide(tilex, tiley, onSurface, dir)) {
            if (Actions.isActionBuildFence(action)) {
                ...
                if (ok) {
                    if (!TileBorderBehaviour.hasMarker(tilex, tiley, onSurface, dir)) {
                ...
                if (!TileBorderBehaviour.hasMarker(tilex, tiley, onSurface, dir)) {
                    ...
                    if (opening[0] != -1 && opening[1] != -1) {
                        if (!TileBorderBehaviour.isWideEntrance(opening[0], opening[1])) {
                ...
                if (performer.getPower() >= 5 && source.getTemplateId() == 176) {
                    try {
                        ...
                        for (x = z.getStartX(); x < z.getStartX() + z.getSize(); ++x) {
                            try {
                        ...
                        for (x = z.getStartY(); x < z.getStartY() + z.getSize(); ++x) {
                            if (x == z.getStartY()) continue;
                            try {
                ...
                int tile = Server.surfaceMesh.getTile(digTilex, digTiley = (int)performer.getStatus().getPositionY() + 2 >> 2);
    ...
    public boolean action(Action act, Creature performer, int tilex, int tiley, boolean onSurface, Tiles.TileBorderDirection dir, long borderId, short action, float counter) {
        if (action == 1) {
            TileBorderBehaviour.handle_EXAMINE(performer, tilex, tiley, onSurface, dir);
        } else {
            if (TileBorderBehaviour.hasHoleEachSide(tilex, tiley, onSurface, dir)) {
                return true;
            }
