...
extends Behaviour {
    ...
    public List<ActionEntry> getBehavioursFor(Creature performer, Item target, int tilex, int tiley, boolean onSurface, Tiles.TileBorderDirection dir, boolean border, int heightOffset) {
        ...
        toReturn.add(Actions.actionEntrys[607]);
        boolean hasMarker = StructureBehaviour.hasMarker(tilex, tiley, onSurface, dir, heightOffset);
        ...
        if (structure != null && structure.isActionAllowed(performer, (short)116)) {
            if (!onSurface) {
                ...
                if (dir == Tiles.TileBorderDirection.DIR_HORIZ ? (neCeil = (short)CaveTile.decodeCeilingHeight(neCorner = Server.caveMesh.getTile(tilex + 1, tiley))) < minHeight : (swCeil = (short)CaveTile.decodeCeilingHeight(swCorner = Server.caveMesh.getTile(tilex, tiley + 1))) < minHeight) {
                    return toReturn;
                }
            }
            boolean hasArch = false;
            if (!MethodsStructure.doesTileBorderContainWallOrFence(tilex, tiley, heightOffset, dir, onSurface, false)) {
                toReturn.add(new ActionEntry(-1, "Plan", "planning"));
                toReturn.add(new ActionEntry((short)(20000 + StructureTypeEnum.SOLID.ordinal()), "Wall", "planning wall", emptyIntArr));
            } else {
                hasArch = true;
            }
            if (!hasMarker) {
                toReturn.add(new ActionEntry(-11, "Fence", "Fence options"));
                LinkedList<ActionEntry> iron = new LinkedList<ActionEntry>();
                ...
                LinkedList<ActionEntry> marble = new LinkedList<ActionEntry>();
                ...
                LinkedList<ActionEntry> plank = new LinkedList<ActionEntry>();
                ...
                LinkedList<ActionEntry> pottery = new LinkedList<ActionEntry>();
                ...
                LinkedList<ActionEntry> rope = new LinkedList<ActionEntry>();
                ...
                LinkedList<ActionEntry> round = new LinkedList<ActionEntry>();
                ...
                LinkedList<ActionEntry> sandstone = new LinkedList<ActionEntry>();
                ...
                LinkedList<ActionEntry> shaft = new LinkedList<ActionEntry>();
                ...
                LinkedList<ActionEntry> slate = new LinkedList<ActionEntry>();
                ...
                LinkedList<ActionEntry> stone = new LinkedList<ActionEntry>();
                ...
                LinkedList<ActionEntry> woven = new LinkedList<ActionEntry>();
    ...
    public boolean action(Action act, Creature performer, Item source, int tilex, int tiley, boolean onSurface, int heightOffset, Tiles.TileBorderDirection dir, long borderId, short action, float counter) {
        ...
        if (!hasMarker && StructureBehaviour.canBuildFenceOnFloor(action) && structure != null && structure.isActionAllowed(performer, (short)116)) {
    ...
    static boolean hasMarker(int tilex, int tiley, boolean onSurface, Tiles.TileBorderDirection dir, int heightOffset) {
        ...
        if (floors != null && floors.length == 1) {
            ...
            if (bridgePart == null) return false;
        ...
        if (dir != Tiles.TileBorderDirection.DIR_HORIZ || !MethodsHighways.containsMarker(highwaypos, (byte)4)) return dir == Tiles.TileBorderDirection.DIR_DOWN && MethodsHighways.containsMarker(highwaypos, (byte)16);
        return true;
    }
}
