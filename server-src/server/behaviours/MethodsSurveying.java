...
implements MiscConstants {
    ...
    public static boolean planBridge(Action act, Creature performer, Item source, Creature helper, @Nullable Item target, short action, float counter) {
        ...
        if (helper != null && !insta && act.getTickCount() > 5 && act.getTickCount() < 12 && !MethodsSurveying.isHoldingRangePole(helper)) {
        ...
        if (!done && act.currentSecond() % timePerStage == 0) {
            ...
            double rotRads = Math.atan2(performer.getStatus().getPositionY() - hY, performer.getStatus().getPositionX() - hX);
            ...
            Point near = helper != null ? MethodsSurveying.getBridgeEnd(performer) : MethodsSurveying.getBridgeEnd(performer, null);
            Point far = helper != null ? MethodsSurveying.getBridgeEnd(helper) : MethodsSurveying.getBridgeEnd(performer, target);
            ...
            switch (act.getTickCount()) {
                case 1: {
                    ...
                    if (!MethodsSurveying.isBorderLevel(performer)) {
                        pMsg = "The (" + MethodsSurveying.dirAsString(performer.getStatus().getDir()) + ") tile border in front of you must be level.";
                        done = true;
                        break;
                    }
                    performer.getCommunicator().sendNormalServerMessage("The tile border to your " + MethodsSurveying.dirAsString(performer.getStatus().getDir()) + " is level");
                ...
                case 4: {
                    if (helper != null) {
                        ...
                        if (odir != performer.getStatus().getDir()) {
                            pMsg = "You (facing " + MethodsSurveying.dirAsString(performer.getStatus().getDir()) + ") and " + helper.getName() + " (facing " + MethodsSurveying.dirAsString(helper.getStatus().getDir()) + ") are not facing in opposite directions.";
                            hMsg = "You need to face in opposite directions to plan the bridge.";
                            done = true;
                            break;
                        }
                        if (!insta && !MethodsSurveying.isHoldingRangePole(helper)) {
                ...
                case 5: {
                    if (helper != null && !MethodsSurveying.isBorderLevel(helper)) {
                        pMsg = "The (" + MethodsSurveying.dirAsString(helper.getStatus().getDir()) + ") tile border in front of " + helper.getName() + " is not level.";
                        hMsg = "The (" + MethodsSurveying.dirAsString(helper.getStatus().getDir()) + ") tile border in front of you must be level.";
                        done = true;
                        break;
                    }
                    if (target != null && !MethodsSurveying.isBorderLevel(performer, target)) {
                        pMsg = "The (" + MethodsSurveying.dirAsString(MethodsSurveying.getDir(performer, target)) + ") tile border in front of the range pole is not level.";
                        done = true;
                        break;
                    }
                    if (helper != null) {
                        performer.getCommunicator().sendNormalServerMessage("The tile border to your helpers " + MethodsSurveying.dirAsString(helper.getStatus().getDir()) + " is level");
                    } else {
                        performer.getCommunicator().sendNormalServerMessage("The tile border to your rangepole's " + MethodsSurveying.dirAsString(MethodsSurveying.getDir(performer, target)) + " is level");
                    }
                    pMsg = "You check the heights of the plan ends" + qMsg + ".";
                    break;
                }
                case 6: {
                    PlanBridgeCheckResult ans = PlanBridgeChecks.calcHeights(performer, helper != null ? helper.getName() : "rangepole", near, far, diff, len, insta);
                    done = ans.failed();
                    qMsg = ans.qMsg();
                    pMsg = ans.pMsg();
                    if (done) break;
                ...
                case 7: {
                    ...
                    if (wid > 1) {
                        ans = PlanBridgeChecks.checkPlanWidth(start, end, dir, performer.getLayer());
                        done = ans.failed();
                        qMsg = ans.qMsg();
                        pMsg = ans.pMsg();
                    }
                ...
                case 9: {
                    PlanBridgeCheckResult ans = performer.isOnSurface() ? PlanBridgeChecks.checkForFolliage(start, end, dir, insta) : PlanBridgeChecks.checkCeilingClearance(start, end, dir, insta, performer.getLayer());
                    done = ans.failed();
                    qMsg = ans.qMsg();
                    pMsg = ans.pMsg();
                    if (done) break;
                ...
                case 14: {
                    ...
                    if (pMsg.length() == 0) {
                        ...
                        pMsg = ans.pMsg();
                        if (pMsg.length() == 0) {
                            ans = helper != null ? PlanBridgeChecks.checkForBuildingPermissions(performer, helper.getTileX(), helper.getTileY(), insta, performer.isOnSurface()) : PlanBridgeChecks.checkForBuildingPermissions(performer, target.getTileX(), target.getTileY(), insta, performer.isOnSurface());
                            done = ans.failed();
                            qMsg = ans.qMsg();
                            pMsg = ans.pMsg();
                        }
    ...
    public static boolean survey(Action act, Creature performer, Item source, Creature helper, @Nullable Item target, short action, float counter) {
        ...
        if (helper != null && performer.isOnSurface() != helper.isOnSurface()) {
            ...
            if (!done && act.currentSecond() % timePerStage == 0) {
                ...
                Point far = helper != null ? new Point(helper.getTileX(), helper.getTileY(), helper.getPosZDirts()) : new Point(target.getTileX(), target.getTileY(), (int)(target.getPosZ() * 10.0f));
                String rp = "the range pole";
                if (helper != null) {
                    rp = helper.getName();
                }
                ...
                switch (act.getTickCount()) {
                    ...
                    case 5: {
                        ...
                        if (diff.getY() != 0) {
                            pMsg = diff.getX() != 0 ? "The range pole is " + Math.abs(diff.getY()) + " tiles " + (diff.getY() > 0 ? "North" : "South") + " and " + Math.abs(diff.getX()) + " tiles " + (diff.getX() > 0 ? "West" : "East") + " of you." : "The range pole is " + Math.abs(diff.getY()) + " tiles straight " + (diff.getY() > 0 ? "North" : "South") + " of you.";
                        } else if (diff.getX() != 0) {
                            pMsg = "The range pole is " + Math.abs(diff.getX()) + " tiles straight " + (diff.getX() > 0 ? "West" : "East") + " of you.";
                        }
                        pMsg = diff.getH() != 0 ? (near.getH() > far.getH() ? pMsg + " Also you appear to be " + diff.getH() + " dirt higher than the range pole." : pMsg + " Also you appear to be " + diff.getH() + " dirt lower than the range pole.") : pMsg + " Also both ends are the same height, Nice!";
                        if (far.getH() > 0 || near.getH() < 0) break;
                        if (insta) {
                            rMsg = "(" + act.getTickCount() + ") ";
                        }
                        if (pMsg.length() > 0) {
    ...
    private static boolean isHoldingRangePole(Creature target) {
        try {
            return ((Player)target).getCurrentAction().getNumber() == 636 && MethodsSurveying.subjectItem(target).getTemplateId() == 901;
        }
    ...
    private static Item subjectItemOrNull(Creature target) {
        try {
            ...
            Item item = Items.getItem(subject);
            return item;
        }
        catch (NoSuchActionException e) {
            return null;
    ...
    private static Point getBridgeEnd(Creature creature, Item item) {
        byte dir = MethodsSurveying.getDir(creature, item);
    ...
    private static boolean isBorderLevel(Creature creature) {
        Point pt = MethodsSurveying.getBridgeEnd(creature);
        byte dir = creature.getStatus().getDir();
    ...
    private static boolean isBorderLevel(Creature creature, Item item) {
        Point pt = MethodsSurveying.getBridgeEnd(creature, item);
        byte dir = creature.getStatus().getDir();
        if (creature.getFloorLevel() == 0) {
            int tileX2 = pt.getX();
            int tileY2 = pt.getY();
