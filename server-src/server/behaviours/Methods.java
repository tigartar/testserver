...
public final class Methods
   ...
   static void sendTeleportQuestion(Creature performer, Item source) {
      TeleportQuestion dq = new TeleportQuestion(
         performer,
         "Teleportation coordinates",
         "Set coordinates: x, 0-" + ((1 << Constants.meshSize) - 1) + " y, 0-" + ((1 << Constants.meshSize) - 1) + " or provide a player name.",
         source.getWurmId()
      );
      dq.sendQuestion();
   }
   ...
   static void sendSummonQuestion(Creature performer, Item source, int tilex, int tiley, long structureId) {
      CreatureCreationQuestion cq = new CreatureCreationQuestion(
         performer, "Summon creature", "Summon the creature of your liking:", source.getWurmId(), tilex, tiley, performer.getLayer(), structureId
      );
      cq.sendQuestion();
   }
   ...
   static void sendFoundVillageQuestion(Creature performer, Item deed) {
      if (!performer.isOnSurface()) {
         ...
         Map<Village, String> decliners = Villages.canFoundVillage(5, 5, 5, 5, tile.tilex, tile.tiley, 0, true, null, performer);
               ...
               try {
                  if (village != null && village.getCitizen(performer.getWurmId()).getRole() == village.getRoleForStatus((byte)2)) {
                     performer.getCommunicator()
                        .sendNormalServerMessage("You cannot found another settlement while being mayor in one. Give away one of the deeds.");
                  } else {
                     VillageFoundationQuestion vf = new VillageFoundationQuestion(
                        performer, "Settlement Application Form", "Welcome to the Settlement Application Form", deed.getWurmId()
                     );
                     if (vf != null) {
   ...
   static void sendExpandVillageQuestion(Creature performer, @Nullable Item deed) {
      try {
            ...
            if (coolDown < village.getToken().getLastOwnerId()) {
               performer.getCommunicator()
                  .sendNormalServerMessage(
                     "The settlement has been attacked, or been under siege recently. You need to wait "
                        + Server.getTimeFor(village.getToken().getLastOwnerId() - coolDown)
                        + "."
                  );
               return;
            }
         ...
         if (village.isActionAllowed((short)76, performer)) {
            ...
            performer.getCommunicator()
               .sendNormalServerMessage(
                  "You are not allowed to use this deed here. Ask the mayor to set the permissions on the management deed to allow you to expand."
               );
         }
   ...
   static void setVillageToken(Creature performer, Item token) {
      ...
      if (village == null) {
         ...
         try {
            ...
            logger.log(Level.WARNING, "Village: " + village.getId() + ", token: " + token.getWurmId() + " - " + var10.getMessage(), (Throwable)var10);
            ...
            logger.log(Level.WARNING, "Village: " + village.getId() + ", token: " + token.getWurmId() + " - " + var11.getMessage(), (Throwable)var11);
            performer.getCommunicator().sendNormalServerMessage("A server error occured. Please report this.");
            return;
         }
            ...
            if (nextDrain < 0L) {
               ...
               if (moneyDrained > 1L) {
                  ...
                  if (counter == 1.0F) {
                     ...
                     Server.getInstance()
                        .broadCastAction(
                           performer.getName() + " starts to rummage through the coffers of " + village.getName() + ", looking for coins.", performer, 5
                        );
                  ...
                  if (counter * 10.0F > (float)time || insta) {
                     ...
                     performer.getCommunicator()
                        .sendNormalServerMessage("You find " + change.getChangeString() + " in the coffers of " + village.getName() + ".");
                     Server.getInstance()
                        .broadCastAction(
                           performer.getName()
                     ...
                     Item[] coins = Economy.getEconomy().getCoinsFor(moneyDrained / 2L);

                     for(Item lCoin : coins) {
                     ...
                     if (village.plan != null
                        && village.plan.getNumHiredGuards() >= 5
                        && performer.isChampion()
                        && village.kingdom != performer.getKingdomId()) {
                     ...
                     boolean enemyHomeServer = Servers.localServer.isChallengeOrEpicServer()
                        ...
                        && Servers.localServer.KINGDOM != performer.getKingdomId();
               ...
               performer.getCommunicator()
                  .sendNormalServerMessage("The coffer timelock has been activated. Try again in " + Server.getTimeFor(nextDrain) + ".");
            }
         } else {
   ...
   public static void sendJoinVillageQuestion(Creature performer, Creature invited) {
      if (performer.getKingdomId() == invited.getKingdomId()) {
         ...
         if (vill != null) {
            Citizen citiz = vill.getCitizen(invited.getWurmId());
            VillageRole role = citiz.getRole();
            if (role.getStatus() == 2) {
               performer.getCommunicator()
                  .sendNormalServerMessage(invited.getName() + " is the mayor of " + vill.getName() + ". He can't join another settlement.");
               return;
            }
         }
         ...
         try {
            ...
            if (village != null && village.acceptsNewCitizens()) {
               ...
               VillageJoinQuestion vj = new VillageJoinQuestion(
                  performer, "Settlement invitation", "Invitation to become citizen of a settlement.", invited.getWurmId()
               );
               vj.sendQuestion();
            ...
            logger.log(Level.WARNING, "Failed to locate creature " + invited.getName(), (Throwable)var5);
            ...
            logger.log(Level.WARNING, "Failed to locate player " + invited.getName(), (Throwable)var6);
   ...
   static void sendVillagePeaceQuestion(Creature performer, Creature invited) {
      ...
      if (vill != null) {
         if (!vill.mayDoDiplomacy(invited)) {
            performer.getCommunicator()
               .sendNormalServerMessage(invited.getName() + " may not do diplomacy in the name of " + vill.getName() + ". He cannot give you peace.");
            return;
         }
         ...
         if (village != null) {
            if (!village.mayDoDiplomacy(performer)) {
               performer.getCommunicator()
                  .sendNormalServerMessage("You may not do diplomacy in the name of " + village.getName() + ". You cannot offer peace.");
               return;
            }
   ...
   static void sendWarDeclarationQuestion(Creature performer, Village targetVillage) {
      if (targetVillage != null) {
         ...
         if (village != null) {
            if (!village.mayDoDiplomacy(performer)) {
               performer.getCommunicator()
                  .sendNormalServerMessage("You may not do diplomacy in the name of " + village.getName() + ". You cannot declare war.");
               return;
            }

            try {
               DeclareWarQuestion pq = new DeclareWarQuestion(performer, "War declaration", "Will you declare war?", (long)targetVillage.getId());
               pq.sendQuestion();
            } catch (NoSuchCreatureException var4) {
               logger.log(Level.WARNING, "Failed to locate creature " + performer.getName(), (Throwable)var4);
               ...
               logger.log(Level.WARNING, "Failed to locate player " + performer.getName(), (Throwable)var5);
               ...
               logger.log(Level.WARNING, "Failed to locate village " + targetVillage.getName(), (Throwable)var6);
   ...
   static void sendShutdownQuestion(Creature performer, Item wand) {
      ShutDownQuestion vs = new ShutDownQuestion(
         performer, "Shutting down the server", "Select the number of minutes and seconds to shutdown as well as the reason for it.", wand.getWurmId()
      );
      vs.sendQuestion();
   }
   ...
   static void sendPaymentQuestion(Creature performer, Item wand) {
      PaymentQuestion vs = new PaymentQuestion(
         performer, "Setting payment expiretime for a player.", "Select the number of days and months before the subscription expires.", wand.getWurmId()
      );
      vs.sendQuestion();
   }
   ...
   static void sendPowerManagementQuestion(Creature performer, Item wand) {
      PowerManagementQuestion vs = new PowerManagementQuestion(
         performer, "Setting the power status for a player.", "Set the power of the player to the selected level.", wand.getWurmId()
      );
      vs.sendQuestion();
   }
   ...
   static void sendTileDataQuestion(Creature performer, Item wand, int tilex, int tiley) {
      TileDataQuestion dq = new TileDataQuestion(
         performer, "Setting data for tile at " + tilex + ", " + tiley, "Set the data:", tilex, tiley, wand.getWurmId()
      );
      dq.sendQuestion();
   }
   ...
   static final void sendAllianceQuestion(Creature performer, Creature target) {
      if (!target.isFighting() && target.hasLink()) {
         ...
         aq.sendQuestion();
         performer.getCommunicator()
            .sendNormalServerMessage("You send an elaborate invitation to form a high and mighty alliance to " + target.getName() + ".");
      } else {
   ...
   static final void sendFriendQuestion(Creature performer, Creature target) {
      if (performer.getKingdomId() == target.getKingdomId()) {
         if (!target.isFighting() && target.hasLink()) {
            ...
            fq.sendQuestion();
            performer.getCommunicator()
               .sendNormalServerMessage("You ask " + target.getName() + " for permission to add " + target.getHimHerItString() + " to your friends list.");
         } else {
            performer.getCommunicator().sendNormalServerMessage(target.getName() + " does not answer questions right now.");
         }
      }
   ...
   public static final boolean discardSellItem(Creature performer, Action act, Item discardItem, float counter) {
      ...
      if (!discardItem.isNoDiscard() && !discardItem.isTemporary()) {
         if (discardItem.getOwnerId() != performer.getWurmId()) {
            ...
            return true;
         } else if (discardItem.isInstaDiscard()) {
         ...
         } else if (performer.getMoneyEarnedBySellingLastHour() > 500L) {
         ...
         if (!vill.isDisbanding()) {
            if (settings) {
               ...
               if (counter == 1.0F && !insta) {
                  ...
                  vill.broadCastMessage(
                     new Message(performer, (byte)3, "Village", "<" + performer.getName() + "> starts to disband the village of " + vill.getName() + ".")
                  );
                  ...
                  if (insta) {
                     ...
                     if (vill.getMayor().getId() == performer.getWurmId() && vill.getDiameterX() < 30 && vill.getDiameterY() < 30) {
                        ...
                        if (!Servers.localServer.isFreeDeeds() || Servers.localServer.isUpkeep()) {
                           performer.getCommunicator()
                              .sendAlertServerMessage("Do not change server during this process. You may not receive the money from the coffers in that case.");
                        }

                        if (Servers.localServer.isFreeDeeds()
                           && Servers.localServer.isUpkeep()
                           && vill.getCreationDate() < System.currentTimeMillis() + 2419200000L) {
                           performer.getCommunicator()
                        ...
                        vill.broadCastMessage(
                           new Message(performer, (byte)3, "Village", "<" + performer.getName() + "> has set " + vill.getName() + " to disband immediately.")
                        );
                        Village[] allies = vill.getAllies();

                        for(Village lAllie : allies) {
                           lAllie.broadCastMessage(
                              new Message(
                                 performer, (byte)3, "Village", "<" + performer.getName() + "> has set " + vill.getName() + " to disband immediately."
                              )
                           );
                        }
                     } else {
                        ...
                        Server.getInstance()
                           .broadCastAction(performer.getName() + " has set " + vill.getName() + " to disband in " + "24 hours" + ".", performer, 5);
                        vill.broadCastMessage(
                           new Message(
                              performer, (byte)3, "Village", "<" + performer.getName() + "> has set " + vill.getName() + " to disband in " + "24 hours" + "."
                           )
                        );
                        Village[] allies = vill.getAllies();
                        ...
                        for(Village lAllie : allies) {
                           lAllie.broadCastMessage(
                              new Message(
                                 ...
                                 (byte)3,
                                 "Village",
                                 "<" + performer.getName() + "> has set " + vill.getName() + " to disband in " + "24 hours" + "."
                              )
                           );
                        }
                     }
         ...
         if (vill.isDisbanding()) {
            if (counter == 1.0F) {
               ...
               if (citizVill != null && citizVill.equals(vill)) {
                  ...
                  vill.broadCastMessage(
                     new Message(performer, (byte)3, "Village", "<" + performer.getName() + "> starts to salvage the settlement of " + vill.getName() + ".")
                  );
            ...
            if (counter > 30.0F || insta) {
               ...
               try {
                  ...
                  vill.broadCastMessage(
                     new Message(performer, (byte)3, "Village", "<" + performer.getName() + "> has salvaged the settlement of " + vill.getName() + ".")
                  );
               } catch (IOException var10) {
                  logger.log(Level.WARNING, performer.getName() + " " + var10.getMessage(), (Throwable)var10);
               }
            }
         } else {
            toReturn = true;
   ...
   public static final boolean aspireKing(Creature performer, byte kingdom, @Nullable Item item, Creature elector, Action act, float counter) {
      ...
      if (performer.getKingdomId() != kingdom) {
         performer.getCommunicator()
            .sendNormalServerMessage(
               "You may not aspire to become the " + King.getRulerTitle(performer.getSex() == 0, kingdom) + " of " + Kingdoms.getNameFor(kingdom) + "."
            );
         done = true;
         ...
         if (counter == 1.0F) {
            ...
            if (act.currentSecond() == 5) {
                  ...
                  if (sk.getKnowledge(0.0) > 21.0) {
                     ...
                     if (elector != null) {
                        if (kingdom == 1) {
                           ...
                           performer.getCommunicator()
                              .sendNormalServerMessage(
                                 elector.getName() + " hisses and sways " + elector.getHisHerItsString() + " head back and forth in excitement!"
                              );
                           Server.getInstance()
                              .broadCastAction(
                                 elector.getName() + " hisses and sways " + elector.getHisHerItsString() + " head back and forth in excitement!", performer, 5
                              );
                        }
                     }
                  }
               ...
               if (done) {
                  ...
                  if (elector != null) {
                     if (kingdom == 1) {
                        ...
                        Server.getInstance()
                           .broadCastAction(elector.getName() + " thwarts " + performer.getName() + " with some threatening moves.", performer, 5);
                     }
                  }
               }
               ...
               if (item != null) {
                  ...
                  Server.getInstance()
                     .broadCastAction(performer.getName() + " exerts all " + performer.getHisHerItsString() + " force on the sword.", performer, 5);
               }

               if (elector != null) {
                  performer.getCommunicator()
                     .sendNormalServerMessage(elector.getName() + " seems to make up " + elector.getHisHerItsString() + " mind about you.");
                  Server.getInstance().broadCastAction(elector.getName() + " watches " + performer.getName() + " intensely again.", performer, 5);
                  elector.turnTowardsCreature(performer);
               }
               ...
               if (Server.rand.nextInt(randomint) != 0 && performer.getPower() < 3) {
                  if (item != null) {
                     ...
                     Server.getInstance()
                        .broadCastAction(performer.getName() + " shrugs in disappointment as the " + item.getName() + " does not budge.", performer, 5);
                  }
                  ...
                  if (elector != null) {
                     if (kingdom == 1) {
                        elector.submerge();
                        performer.getCommunicator()
                           .sendNormalServerMessage("For no obvious reason you are rejected. " + elector.getName() + " silently disappears into the depths.");
                        Server.getInstance().broadCastAction(elector.getName() + " disappears into the depths in silence.", performer, 5);
                     } else if (kingdom == 3) {
                        performer.getCommunicator()
                           .sendNormalServerMessage(
                              "For no obvious reason you are rejected. " + elector.getName() + " ushers you away with some really threatening moves."
                           );
                        Server.getInstance()
                           .broadCastAction(elector.getName() + " thwarts " + performer.getName() + " with some threatening moves.", performer, 5);
                     }
                     ...
                     performer.getCommunicator()
                        .sendNormalServerMessage(
                           "The sword gets loose and disappears! You are the new "
                              + King.getRulerTitle(performer.getSex() == 0, kingdom)
                              + " of "
   ...
   public static void rewardRegalia(Creature creature) {
      ...
      if (inventory != null) {
         try {
            ...
            logger.log(Level.WARNING, creature.getName() + " " + var7.getMessage(), (Throwable)var7);
         }
      }
   }
   ...
   public static Item[] getBestReports(Creature creature, @Nullable Item container) {
      Map<Byte, Item> reports = new HashMap<>();
      if (container == null) {
      ...
      Item[] reportArr = reports.values().toArray(new Item[reports.size()]);
      Arrays.sort((Object[])reportArr);
      return reportArr;
   }
   ...
   public static boolean isActionAllowed(
      Creature performer, short action, boolean setHunted, int tileX, int tileY, @Nullable Item item, int encodedTile, int dir
   ) {
      ...
      if (Actions.isActionDestroy(action) && village != null && village.isActionAllowed(action, performer)) {
         ...
         && (Actions.isActionBuild(action) || Actions.isActionDestroy(action))
         && structure.isActionAllowed(performer, action)) {
         canDo = true;
         ...
         if (!Actions.isActionBuildingPermission(action) && village != null && village.isActionAllowed(action, performer)) {
            canDo = true;
         } else if (!structure.isActionAllowed(performer, action)) {
            canDo = isNotAllowedMessage(performer, village, structure, action, false);
         } else {
            canDo = true;
         }
      } else if (village != null) {
         if (!village.isActionAllowed(action, performer, false, encodedTile, dir)) {
            canDo = isNotAllowedMessage(performer, village, structure, action, false);
         } else {
            canDo = true;
         }
      ...
      if (village == null && Actions.actionEntrys[action].isPerimeterAction()) {
         ...
         if (villagePerim != null && !villagePerim.isCitizen(performer) && !villagePerim.isAlly(performer)) {
            ...
            if (!skipOthers) {
               if (!villagePerim.isActionAllowed(action, performer, false, 0, 0)) {
                  canDo = isNotAllowedMessage(performer, villagePerim, structure, action, true);
               } else {
                  canDo = true;
               }
            }
         ...
         if (twoCheck == null) {
            Village vill = Villages.getVillageWithPerimeterAt(item.getTileX(), item.getTileY(), item.isOnSurface());
         ...
         if (twoCheck != null) {
            if (!twoCheck.isActionAllowed(action, performer, false, 0, 0)) {
               canDo = isNotAllowedMessage(performer, twoCheck, structure, action, true);
            } else {
               canDo = true;
            }
         }
   ...
   private static boolean isNotAllowedMessage(Creature performer, Village village, Structure structure, short action, boolean inPerimeter) {
      if (performer.isOnPvPServer() && !Servers.isThisAChaosServer()) {
         if (village != null) {
            ...
            return true;
         } else if (structure != null && structure.isFinished()) {
