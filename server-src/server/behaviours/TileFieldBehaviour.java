...
public final class TileFieldBehaviour extends TileBehaviour {
   ...
   private static final String[] harvestStrings = new String[]{
      "The ground is freshly sown and the seeds are evolving.",
      ...
      "The field is at full height and ready to harvest!",
      "The field is barren, with weeds rotting in the dirt."
   };
         ...
         if (performer.getPower() > 2) {
            ...
            if (t != null) {
               ...
               if (t.getVillage() > 0) {
                  ...
                  } catch (NoSuchVillageException var35) {
                  }
               }
      ...
      if (action == 151 && source.isFieldTool() && !Zones.protectedTiles[tilex][tiley]) {
         ...
         if (tileAge == 7) {
               ...
               if (counter * 10.0F > (float)time) {
                  ...
                  try {
                     ...
                     performer.getCommunicator()
                        .sendNormalServerMessage("You find " + (result.getName().endsWith("s") ? "some " : "a ") + result.getName() + " in amongst the weeds.");
                     if (!performer.getInventory().insertItem(result, true)) {
                        performer.getCommunicator()
                           .sendNormalServerMessage("You can't carry the " + result.getName() + ". It falls to the ground and is ruined!");
                     }
                  } catch (NoSuchTemplateException | FailedException var31) {
                  }
            ...
            if (counter * 10.0F > (float)time) {
               ...
               if (farmedCount < 5) {
                  ++farmedCount;
                  farmedChance = (int)Math.min(
                     (double)farmedChance + power * 2.0 + (double)(act.getRarity() * 110) + (double)(source.getRarity() * 10), 2047.0
                  );
               }
