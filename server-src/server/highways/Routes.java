...
import com.wurmonline.server.highways.PlayerMessageToSend;
...
implements HighwayConstants {
    ...
    public static final void generateAllRoutes() {
        ...
        for (Item waystone : Items.getWaystones()) {
            Routes.makeNodeFrom(waystone);
        }
        for (Item waystone : Items.getWaystones()) {
            Routes.checkForRoutes(waystone, false, null);
        }
        for (Item waystone : Items.getWaystones()) {
            Node startNode = Routes.getNode(waystone);
            HighwayFinder.queueHighwayFinding(null, startNode, null, (byte)0);
        }
        logger.log(Level.INFO, "Calculated " + allRoutes.size() + " routes and " + allNodes.size() + " nodes.That took " + (float)(System.nanoTime() - start) / 1000000.0f + " ms.");
        Players.getInstance().sendGmMessage(null, "Roads", "Calculated " + allRoutes.size() + " routes and " + allNodes.size() + " nodes. That took " + (float)(System.nanoTime() - start) / 1000000.0f + " ms.", false);
    }

    private static final boolean checkForRoutes(Item waystone, boolean tellGms, Item marker) {
        boolean foundRoute = false;
        foundRoute |= Routes.checkForRoute(waystone, (byte)1, tellGms, marker);
        foundRoute |= Routes.checkForRoute(waystone, (byte)2, tellGms, marker);
        foundRoute |= Routes.checkForRoute(waystone, (byte)4, tellGms, marker);
        foundRoute |= Routes.checkForRoute(waystone, (byte)8, tellGms, marker);
        foundRoute |= Routes.checkForRoute(waystone, (byte)16, tellGms, marker);
        foundRoute |= Routes.checkForRoute(waystone, (byte)32, tellGms, marker);
        foundRoute |= Routes.checkForRoute(waystone, (byte)64, tellGms, marker);
        return foundRoute |= Routes.checkForRoute(waystone, (byte)-128, tellGms, marker);
    }
    ...
    private static final boolean checkForRoute(Item waystone, byte checkdir, boolean tellGms, Item planted) {
        ...
        while (checking) {
            ...
            Item marker = MethodsHighways.getMarker(highwayPos = MethodsHighways.getNewHighwayPosLinked(highwayPos, linkdir));
            if (marker == null) {
            ...
            if (!MethodsHighways.hasLink(marker.getAuxData(), fromdir)) {
                logger.info("Missing Link! " + MethodsHighways.getLinkAsString(lastdir) + " from:x:" + lastx + ",y:" + lasty + ",bp:" + lastbp + ",fl:" + lastfl + "  to:x" + highwayPos.getTilex() + ",y:" + highwayPos.getTiley() + "Surf:" + highwayPos.isOnSurface() + ",bp:" + highwayPos.getBridgeId() + ",fl:" + highwayPos.getFloorLevel());
                return false;
            }
            if (marker.getTemplateId() == 1114) {
                ...
                if (MethodsHighways.numberOfSetBits(todir) != 1) {
                    if (Servers.isThisATestServer()) {
                        logger.info("End of road! @" + marker.getTileX() + "," + marker.getTileY() + " (from:" + MethodsHighways.getLinkAsString(fromdir) + ",to:" + MethodsHighways.getLinkAsString(todir) + ")");
                    }
                    return false;
                }
                linkdir = todir;
            ...
            if (marker.getTemplateId() == 1112) {
                Node endNode = Routes.getNode(marker);
    ...
    public static final Node getNode(Item waystone) {
        ...
        return Routes.makeNodeFrom(waystone);
    }
    ...
    public static final void remove(Item marker) {
        if (marker.getTemplateId() == 1114) {
            for (Map.Entry<Integer, Route> entry : allRoutes.entrySet()) {
                if (!entry.getValue().containsCatseye(marker)) continue;
                Routes.removeRoute(entry.getValue(), marker);
                break;
            }
        } else {
            Node node = allNodes.remove(marker.getWurmId());
            if (node != null) {
                Routes.removeRoute(node, (byte)1, marker);
                Routes.removeRoute(node, (byte)2, marker);
                Routes.removeRoute(node, (byte)4, marker);
                Routes.removeRoute(node, (byte)8, marker);
                Routes.removeRoute(node, (byte)16, marker);
                Routes.removeRoute(node, (byte)32, marker);
                Routes.removeRoute(node, (byte)64, marker);
                Routes.removeRoute(node, (byte)-128, marker);
            }
        }
    }
    ...
    private static final void removeRoute(Node node, byte checkdir, Item marker) {
        ...
        if (route != null) {
            Routes.removeRoute(route, marker);
        }
    }
    ...
    private static final void removeRoute(Route route, Item marker) {
        ...
        if (oppRoute != null) {
            Item[] oppStart = oppRoute.getStartNode();
        ...
        if (!marker.isReplacing()) {
            ...
            Routes.sendToTrello(title, description);
        }
    }
    ...
    public static final boolean checkForNewRoutes(Item marker) {
        byte startdir;
        if (marker.getTemplateId() == 1112) {
            Routes.getNode(marker);
            return Routes.checkForRoutes(marker, true, marker);
        }
        if (MethodsHighways.numberOfSetBits(marker.getAuxData()) == 2 && (startdir = Routes.getStartdir(marker)) != 0) {
            HashSet<Item> markersDone = new HashSet<Item>();
            ...
            while (checking) {
                ...
                int lastfl = highwayPos.getFloorLevel();
                byte lastdir = linkdir;
                Item nextMarker = MethodsHighways.getMarker(highwayPos = MethodsHighways.getNewHighwayPosLinked(highwayPos, linkdir));
                if (nextMarker == null) {
                ...
                if (nextMarker.getTemplateId() == 1114) {
                    ...
                    if (MethodsHighways.numberOfSetBits(todir) != 1) {
                        if (Servers.isThisATestServer()) {
                            logger.info("End of road! @" + nextMarker.getTileX() + "," + nextMarker.getTileY() + " (from:" + MethodsHighways.getLinkAsString(fromdir) + ",to:" + MethodsHighways.getLinkAsString(todir) + ")");
                        }
                        return false;
                    }
                    linkdir = todir;
                ...
                if (nextMarker.getTemplateId() == 1112) {
                    checking = false;
                    return Routes.checkForRoute(nextMarker, fromdir, true, marker);
                }
                return false;
            }
        }
    ...
    private static final byte getStartdir(Item marker) {
        ...
        return (byte)startdir;
    }

    public static final Item[] getMarkers() {
        ConcurrentHashMap<Long, Item> markers = new ConcurrentHashMap<Long, Item>();
    ...
    public static final Item[] getRouteMarkers(Item marker) {
        ConcurrentHashMap<Long, Object> markers = new ConcurrentHashMap<Long, Object>();
        if (marker.getTemplateId() == 1114) {
            for (Route route : allRoutes.values()) {
                if (!route.containsCatseye(marker)) continue;
                ...
                for (Item item : route.getCatseyes()) {
                    markers.put(item.getWurmId(), item);
                }
            ...
            for (Route route : allRoutes.values()) {
                ...
                if (route.getStartNode().getWurmId() == marker.getWurmId()) {
                    startWaystone = route.getStartNode().getWaystone();
                    markers.put(startWaystone.getWurmId(), startWaystone);
                    for (Item item : route.getCatseyes()) {
                        markers.put(item.getWurmId(), item);
                    }
                    endWaystone = route.getEndNode().getWaystone();
                    markers.put(((Item)endWaystone).getWurmId(), endWaystone);
                }
                if (route.getEndNode().getWurmId() != marker.getWurmId()) continue;
                startWaystone = route.getStartNode().getWaystone();
                markers.put(startWaystone.getWurmId(), startWaystone);
                for (Item item : route.getCatseyes()) {
                    markers.put(item.getWurmId(), item);
    ...
    public static final boolean isCatseyeUsed(Item catseye) {
        for (Route route : allRoutes.values()) {
            if (!route.containsCatseye(catseye)) continue;
            return true;
        }
        return false;
    }
    ...
    public static final boolean isMarkerUsed(Item marker) {
        if (marker.getTemplateId() == 1114) {
            return Routes.isCatseyeUsed(marker);
        }
        for (Route route : allRoutes.values()) {
            if (route.getStartNode().getWaystone().getWurmId() == marker.getWurmId()) {
                return true;
            }
            if (route.getEndNode() == null || route.getEndNode().getWaystone().getWurmId() != marker.getWurmId()) continue;
            return true;
        }
        return false;
    }
    ...
    public static final Village[] getVillages() {
        ConcurrentHashMap<Integer, Village> villages = new ConcurrentHashMap<Integer, Village>();
        for (Node node : allNodes.values()) {
            Village vill = node.getVillage();
            if (vill == null || !vill.isHighwayFound()) continue;
    ...
    public static final Village[] getVillages(long waystoneId) {
        ...
        for (Node node : allNodes.values()) {
            Village vill = node.getVillage();
            if (vill == null || !vill.isHighwayFound() || !PathToCalculate.isVillageConnected(waystoneId, vill)) continue;
            villages.put(vill.getId(), vill);
        }
        return villages.values().toArray(new Village[villages.size()]);
    }
    ...
    public static final Node[] getNodesFor(Village village) {
        ConcurrentHashMap<Long, Node> nodes = new ConcurrentHashMap<Long, Node>();
        for (Node node : allNodes.values()) {
            Village vill = node.getVillage();
            if (vill == null || !vill.equals(village)) continue;
