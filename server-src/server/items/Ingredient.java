...
public class Ingredient implements MiscConstants {
   ...
   public boolean matches(Item item) {
      return (
            this.itemTemplate.isFoodGroup() && item.getTemplate().getFoodGroup() == this.itemTemplate.getTemplateId()
               || this.getTemplateId() == item.getTemplateId()
         )
         && this.checkState(item)
         && this.checkMaterial(item)
         && this.checkRealTemplate(item)
         && this.checkCorpseData(item);
   }
   ...
   String getSubMenuName() {
      if (this.resultName.length() > 0) {
         ...
         return this.hasPState() ? this.pstateName + " " + this.itemTemplate.getName() : this.itemTemplate.getName();
      }
   }
   ...
   public byte getXState() {
      if (this.hasCState()) {
         return this.hasPState() ? (byte)(this.getCState() + this.getPState()) : this.getCState();
      } else {
   ...
   public int getRealTemplateId() {
      return this.realItemTemplate == null ? -10 : this.realItemTemplate.getTemplateId();
   }
   ...
   boolean checkState(Item target) {
      return !this.hasCState() && !this.hasPState() ? true : target.isCorrectFoodState(this.getCState(), this.getPState());
   }
   ...
   String generateMaterialName(byte mat) {
      return mat != -1 ? Materials.convertMaterialByteIntoString(mat) : "";
   }
