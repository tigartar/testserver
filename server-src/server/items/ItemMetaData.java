...
implements MiscConstants {
    ...
    private static final String INSERT_ITEMKEYS = DbConnector.isUseSqlite() ? "INSERT OR IGNORE INTO ITEMKEYS (LOCKID,KEYID)VALUES(?,?)" : "INSERT IGNORE INTO ITEMKEYS (LOCKID,KEYID)VALUES(?,?)";
        ...
        try {
            ...
            for (int x = 0; x < this.keys.length; ++x) {
                try {
                    ...
                    logger.log(Level.INFO, "Failed to insert key id " + this.keys[x] + ": " + ex.getMessage(), ex);
                }
            }
        }
        catch (SQLException ex) {
            try {
                throw new IOException(this.itemId + " " + ex.getMessage(), ex);
            }
            catch (Throwable throwable) {
                DbUtilities.closeDatabaseObjects(ps, null);
                DbConnector.returnConnection(dbcon);
