...
public enum EpicMissionEnum {
   ...
   public static EpicMissionEnum getRandomMission(int difficulty, boolean battlegroundServer, boolean homeServer, boolean enemyHomeServer) {
      ...
      for(EpicMissionEnum f : values()) {
         if (f.minDifficulty <= difficulty
            && f.maxDifficulty >= difficulty
            && (battlegroundServer && f.battlegroundServer || homeServer && f.homeServer || enemyHomeServer && f.enemyHomeServer)) {
   ...
   private EpicMissionEnum(
      ...
      int minDifficulty,
      int maxDifficulty,
      int maxDifficultyPvp,
      boolean friendlyTerritory,
      boolean enemyTerritory,
      boolean battlegroundServer,
      boolean homeServer,
      boolean enemyHomeServer,
      int baseKarma,
      int karmaBonusDiffMult,
      int baseSleep,
      int sleepBonusDiffMult,
      boolean isKarmaMultProgress,
      boolean isSleepMultNearby,
      String... missionNames
   ) {
   ...
   public static final long getTimeReductionForMission(byte missionType, int missionDifficulty) {
      ...
      return toReturn + 7200000L * (long)missionDifficulty;
   }
}
