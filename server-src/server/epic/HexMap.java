...
public class HexMap implements MiscConstants, CreatureTemplateIds, TimeConstants {
   ...
   public final void pollAllEntities(boolean testing) {
      EpicEntity[] entityArr = this.getAllEntities();
      ...
      if (entity != null) {
         ...
         switch(rand.nextInt(7)) {
            case 0:
               ...
               entity.setSkill(
                  102, current + (100.0F - current) / ((float)(500 + (7 - missionDifficulty) * 50) + rand.nextFloat() * (float)((7 - missionDifficulty) * 50))
               );
               break;
            case 1:
               current = entity.getCurrentSkill(103);
               entity.setSkill(
                  103, current + (100.0F - current) / ((float)(500 + (7 - missionDifficulty) * 50) + rand.nextFloat() * (float)((7 - missionDifficulty) * 50))
               );
               break;
            case 2:
               current = entity.getCurrentSkill(104);
               entity.setSkill(
                  104, current + (100.0F - current) / ((float)(500 + (7 - missionDifficulty) * 50) + rand.nextFloat() * (float)((7 - missionDifficulty) * 50))
               );
               break;
            case 3:
               current = entity.getCurrentSkill(100);
               entity.setSkill(
                  100, current + (100.0F - current) / ((float)(500 + (7 - missionDifficulty) * 50) + rand.nextFloat() * (float)((7 - missionDifficulty) * 50))
               );
               break;
            case 4:
               current = entity.getCurrentSkill(101);
               entity.setSkill(
                  101, current + (100.0F - current) / ((float)(500 + (7 - missionDifficulty) * 50) + rand.nextFloat() * (float)((7 - missionDifficulty) * 50))
               );
               break;
            case 5:
               current = entity.getCurrentSkill(105);
               entity.setSkill(
                  105, current + (100.0F - current) / ((float)(500 + (7 - missionDifficulty) * 50) + rand.nextFloat() * (float)((7 - missionDifficulty) * 50))
               );
               break;
            case 6:
               current = entity.getCurrentSkill(106);
               entity.setSkill(
                  106, current + (100.0F - current) / ((float)(500 + (7 - missionDifficulty) * 50) + rand.nextFloat() * (float)((7 - missionDifficulty) * 50))
               );
         }
   ...
   final boolean winCondition(boolean isWurm, int currentCollectibles, boolean isAtSpawn, int currentHex) {
      if (!this.currentScenario.isSpawnPointRequiredToWin() && currentHex == this.currentScenario.getHexNumRequiredToWin()
         || isAtSpawn && this.currentScenario.isSpawnPointRequiredToWin()) {
         if (isWurm) {
   ...
   public void addDemigod(
      ...
      if (companion != 0L) {
         EpicEntity compa = this.getEntity(companion);
         if (compa != null) {
         ...
         if (e != null) {
            ...
            if (name != null && !e.getName().toLowerCase().equals(name.toLowerCase())) {
               ...
               Servers.ascend(
                  ...
                  d.sex,
                  (byte)3,
                  e.getCurrentSkill(102),
                  e.getCurrentSkill(103),
                  e.getCurrentSkill(104),
                  e.getCurrentSkill(100),
                  e.getCurrentSkill(101),
                  e.getCurrentSkill(105),
                  e.getCurrentSkill(106)
               );
               return true;
            }
         } else {
      ...
      if (maxCollectables == 1) {
         ...
         if (rand.nextBoolean()) {
            missionDescription = instigator
               ...
               + questHolder.getName()
               + "'s "
               + getNameForNumber(maxCollectables)
               + ' '
               + firstPartOfName
               + ' '
               + secondPartOfName
   ...
   EpicEntity getRandomEntityMonster() {
      ...
      return mons.size() > 0 ? mons.get(rand.nextInt(mons.size())) : null;
   }
   ...
   String generateHideWord() {
      ...
      if (prep == 0) {
         ...
         return prep == 2 ? "recently " + secondPart : secondPart;
      }
   }
   ...
   final void setWinCondition(
      int collectiblesRequired,
      int collectiblesRequiredForWurm,
      boolean atSpawnPointRequired,
      int hexNumRequired,
      String newScenarioName,
      String newScenarioQuest,
      int reasonAndEffect
   ) {
   ...
   final void generateCollectables(int nums, String cname, int type) {
      for(int x = -1; x >= -nums; --x) {
         ...
         EpicEntity collectable = new EpicEntity(this, (long)id, cname, type);
         MapHex hex = this.getRandomHex();
   ...
   final void loadControllers() {
      ...
      try {
         ...
         logger.log(Level.WARNING, "Problem loading entity controllers due to " + var12.getMessage(), (Throwable)var12);
      } finally {
   ...
   final void initializeCreatureController(int creatureTemplateId) {
      ...
      try {
         ...
         logger.log(
            Level.WARNING, "Problem creating entity controller for creature template " + creatureTemplateId + " due to " + var8.getMessage(), (Throwable)var8
         );
      } finally {
   ...
   final void setCreatureController(int creatureTemplateId, long controller) {
      for(Entry<Long, LinkedList<Integer>> me : this.controllers.entrySet()) {
      ...
      try {
         ...
         logger.log(
            Level.WARNING,
            "Problem updating entity controller for creature template " + creatureTemplateId + " due to " + var11.getMessage(),
            (Throwable)var11
         );
      } finally {
         DbUtilities.closeDatabaseObjects(ps, null);
   ...
   final void loadVisitedHexes() {
      ...
      try {
         ...
         logger.log(Level.WARNING, "Problem loading visited hexes due to " + var17.getMessage(), (Throwable)var17);
      } finally {
         DbUtilities.closeDatabaseObjects(ps, rs);
         DbConnector.returnConnection(dbcon);
