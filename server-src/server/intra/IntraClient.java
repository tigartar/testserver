...
MiscConstants {
    ...
    public void reconnectAsynch(String serverIp, int serverPort, IntraServerConnectionListener aServerConnectionListener) {
    ...
    public void login(String password, boolean dev) {
        if (this.retryInSeconds <= 0) {
            try {
                byte[] passwordBytes = password.getBytes(CHARSET_ENCODING_FOR_COMMS);
                ...
                logger.log(Level.WARNING, "Failed to login", e);
    ...
    public synchronized void update() throws IOException {
        ...
        try {
            ...
            if (logger.isLoggable(Level.FINE)) {
                logger.log(Level.FINE, "Failed to update on connection: " + this.connection, e);
            }
            this.serverConnectionListener.commandFailed(this);
        }
    }
    ...
    public void reallyHandle(int num, ByteBuffer bb) {
        try {
            ...
            if (cmd == 2) {
                ...
                byte[] bytes = new byte[bb.getShort() & 0xFFFF];
                bb.get(bytes);
                String sessionKey = new String(bytes, CHARSET_ENCODING_FOR_COMMS);
                this.retryInSeconds = bb.getShort() & 0xFFFF;
                long targetNow = bb.getLong();
                this.timeDifference = targetNow - System.currentTimeMillis();
                if (this.retryInSeconds > 0) {
                ...
                WurmCalendar.currentTime = wurmTime = bb.getLong();
                logger.log(Level.INFO, "The server just synched wurm clock. New wurm time=" + wurmTime + ". Difference was " + (oldTime - wurmTime) + " wurm seconds.");
            ...
            logger.log(Level.WARNING, "Problem handling Block: " + bb, e);
            e.printStackTrace();
        }
    }
    ...
    void executeMoneyUpdate(long playerId, long currentMoney, long moneyAdded, String detail) throws IOException {
        ...
        byte[] det = detail.getBytes(CHARSET_ENCODING_FOR_COMMS);
    ...
    void executePasswordUpdate(long playerId, String currentHashedPassword, long timestamp) throws IOException {
        ...
        buf.put((byte)18);
        buf.putLong(playerId);
        byte[] pw = currentHashedPassword.getBytes(CHARSET_ENCODING_FOR_COMMS);
    ...
    static /* synthetic */ SocketConnection access$002(IntraClient x0, SocketConnection x1) {
        x0.connection = x1;
        return x0.connection;
    }

    static /* synthetic */ SocketConnection access$000(IntraClient x0) {
        return x0.connection;
    }

    static /* synthetic */ boolean access$102(IntraClient x0, boolean x1) {
        x0.disconnected = x1;
        return x0.disconnected;
    }
}
