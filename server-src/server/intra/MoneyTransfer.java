...
implements MiscConstants {
    ...
    private static final Map<Long, Set<MoneyTransfer>> batchTransfers = new HashMap<Long, Set<MoneyTransfer>>();
    ...
    public boolean poll() {
        block21: {
            ...
            try {
                ...
                logger.log(Level.WARNING, "Failed to load player info for wurmid " + this.wurmid + ".", eex);
            }
            if (info.wurmId <= 0L) {
            ...
            if (this.client != null && !this.done) {
                ...
                if (this.client.loggedIn && !this.done && !this.sentTransfer) {
                    try {
                        ...
                        logger2.log(Level.WARNING, "Problem calling IntraClient.executeMoneyUpdate() for " + this + " due to " + iox.getMessage(), iox);
                        this.done = true;
                    }
                }
                if (!this.done) {
                    try {
                        ...
                        logger.log(Level.FINE, "Problem calling IntraClient.update() but hopefully not serious", ex);
                    }
                }
            }
        }
