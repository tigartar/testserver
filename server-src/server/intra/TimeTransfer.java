...
implements MiscConstants {
    ...
    public static final List<TimeTransfer> transfers = new LinkedList<TimeTransfer>();
    private boolean done = false;
    private IntraClient client = null;
    private boolean started = false;
    public boolean deleted = false;
    private boolean sentTransfer = false;
    private static final Map<Long, Set<TimeTransfer>> batchTransfers = new HashMap<Long, Set<TimeTransfer>>();
        ...
        if (this.client != null && !this.done) {
            ...
            if (this.client.loggedIn && !this.done && !this.sentTransfer) {
                try {
                    ...
                    logger.log(Level.WARNING, this + ", " + iox.getMessage(), iox);
                    this.done = true;
                }
            }
            if (!this.done) {
