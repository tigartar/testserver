...
TimeConstants {
    ...
    public void setCreatureName(String val) {
        this.creatureName = val == null ? "" : val;
    }
    ...
    private static void destroyInventoryItems(Item inventory, Creature performer) {
        ...
        for (int i = 0; i < inventoryItems.length; ++i) {
            ...
            if (!inventoryItems[i].isInventoryGroup()) continue;
            TriggerEffect.destroyInventoryItems(inventoryItems[i], performer);
        }
    }
    ...
    public void effect(Creature performer, MissionPerformed perf, long target, boolean setFinished, boolean triggerEpicHelper) {
        ...
        if (!this.inActive) {
            ...
            if (performer.isPlayer() && this.creatorType != 2) {
                if (this.sendTriggerDescription(performer)) {
            ...
            if (this.getMissionStateChange() != 0.0f) {
                ...
                if (this.creatorType == 2) {
                    ...
                    if (em != null) {
                        if (em.isCurrent()) {
                            performer.sendToLoggers("Effect " + this.name + " state effect " + this.getMissionStateChange() + " on Epic mission " + em.getEntityName(), (byte)2);
                            ...
                            if (!(Deities.getFavoredKingdom(deityNum) != performer.getKingdomTemplateId() && Servers.localServer.EPIC || missionEnum == null || EpicMissionEnum.isMissionKarmaGivenOnKill(missionEnum))) {
                                int karmaGained;
                                int karmaSplit = missionEnum.getKarmaBonusDiffMult() * em.getDifficulty();
                                if (missionEnum.isKarmaMultProgress() && (karmaGained = (int)Math.ceil((float)karmaSplit / 100.0f * this.getMissionStateChange())) > 0) {
                                    ...
                                    if (performer.isPaying()) {
                                        ...
                                        if (Servers.localServer.EPIC) {
                                            WcEpicKarmaCommand wcek = new WcEpicKarmaCommand(WurmId.getNextWCCommandId(), new long[]{performer.getWurmId()}, new int[]{performer.getScenarioKarma()}, em.getEpicEntityId());
                                            wcek.sendToLoginServer();
                                        }
                                    }
                                    if (this.getMissionStateChange() * 10.0f > 10.0f) {
                                        logger.log(Level.INFO, "Added karma " + karmaGained + " to " + performer.getName() + " for mission " + em.getMissionId() + " for " + em.getEntityName() + " state change=" + this.getMissionStateChange() + " to " + em.getMissionProgress());
                                    }
                                }
                            }
                            ...
                            if (em.isCompleted()) {
                                int deityNumber;
                                logger.log(Level.INFO, "Mission is complete! " + performer.getName() + " mission " + em.getMissionId() + " for " + em.getEntityName() + ", performer mission is " + perf.getMissionId());
                                ...
                                if (m != null) {
                                    if (Deities.getFavoredKingdom(deityNum) == performer.getKingdomTemplateId() || !Servers.localServer.EPIC) {
                                        ...
                                        ArrayList<Player> totalNearby = new ArrayList<Player>();
                                        for (Player p : Players.getInstance().getPlayers()) {
                                            if (Deities.getFavoredKingdom(deityNum) != p.getKingdomTemplateId() && Servers.localServer.EPIC || !p.isWithinDistanceTo(performer, 300.0f)) continue;
                                            totalNearby.add(p);
                                        }
                                        ArrayList<PlayerInfo> allParticipants = new ArrayList<PlayerInfo>();
                                        for (MissionPerformer mp : allperfs = MissionPerformed.getAllPerformers()) {
                                            MissionPerformed thisMission = mp.getMission(m.getId());
                                            if (thisMission == null) continue;
                                        ...
                                        if (karmaSplit > 0 && EpicMissionEnum.isKarmaSplitNearby(missionEnum)) {
                                            karmaGainedNearby = (int)Math.ceil((float)karmaSplit / (float)totalNearby.size());
                                            karmaSplit = 0;
                                        }
                                        if (sleepSplit > 0 && missionEnum.isSleepMultNearby()) {
                                            sleepGainedNearby = (int)Math.ceil((float)sleepSplit / (float)totalNearby.size());
                                            sleepSplit = 0;
                                        }
                                        for (Player p : totalNearby) {
                                            ...
                                            if (karmaGainedNearby <= 0) continue;
                                        ...
                                        if (karmaSplit > 0) {
                                            karmaGained += (int)Math.ceil((float)karmaSplit / (float)allParticipants.size());
                                        }
                                        int sleepBonusGain = 0;
                                        if (sleepSplit > 0) {
                                            sleepBonusGain += (int)Math.ceil((float)sleepSplit / (float)allParticipants.size());
                                        }
                                        for (PlayerInfo pinf : allParticipants) {
                                            ...
                                            if (Servers.localServer.PVPSERVER) {
                                                try {
                                                    ...
                                                    logger.log(Level.WARNING, this.getName() + ": failed to reset rank decay timer " + pinf.getRank(), iox);
                                                }
                                            }
                                        ...
                                        if (Servers.localServer.EPIC) {
                                            ...
                                            for (int x = 0; x < idsToSend.length; ++x) {
                                                idsToSend[x] = (Long)ids.get(x);
                                                valuesToSend[x] = (Integer)values.get(x);
                                            }
                                    ...
                                    for (MissionTrigger t : triggers) {
                                        if (t.getMissionRequired() != m.getId()) continue;
                                        logger.log(Level.INFO, performer.getName() + "Destroying triggers for  mission " + em.getMissionId() + " for " + em.getEntityName() + ", performer mission is " + perf.getMissionId());
                                        TriggerEffects.destroyEffectsForTrigger(t.getId());
                                        t.destroy();
                                    }
                                }
                                logger.log(Level.INFO, performer.getName() + "Destroying effect for  mission " + em.getMissionId() + " for " + em.getEntityName() + ", performer mission is " + perf.getMissionId());
                                this.destroy();
                                logger.log(Level.INFO, performer.getName() + " Sending report for " + em.getMissionId() + " for " + em.getEntityName() + ", performer mission is " + perf.getMissionId());
                                WcEpicStatusReport wce = new WcEpicStatusReport(WurmId.getNextWCCommandId(), true, em.getEpicEntityId(), em.getMissionType(), em.getDifficulty());
                                wce.sendToLoginServer();
                                EpicServerStatus.storeLastMissionForEntity(em.getEpicEntityId(), em);
                                if (!Servers.localServer.EPIC && (deityNumber = em.getEpicEntityId()) > 0 && deityNumber <= 4) {
                                    ...
                                    es.generateNewMissionForEpicEntity(deityNumber, entityName, em.getDifficulty() + 1, 604800, em.getScenarioName(), EpicServerStatus.getCurrentScenario().getScenarioNumber(), "You must really do this for " + entityName + " because yeah you know.", true);
                                }
                            }
                        } else {
            ...
            if (this.destroysInventory()) {
                TriggerEffect.destroyInventoryItems(performer.getInventory(), performer);
                ...
                for (int i = 0; i < boditems.length; ++i) {
                    ...
                    if (boditems[i].isNoDrop() || boditems[i].isCoin()) continue;
            ...
            if (this.rewardItem > 0 && this.creatorType != 3) {
                ...
                for (int x = 0; x < this.rewardNumbers; ++x) {
                    try {
                        Item toCreate = ItemFactory.createItem(this.rewardItem, this.rewardQl, this.itemMaterial, (byte)0, performer.getName());
                        if (this.newbieItem) {
            ...
            if (this.rewardSkillNum > 0 && this.creatorType != 3) {
                ...
                if (skills != null) {
                    try {
                        ...
                        double reward = this.rewardSkillVal;
            ...
            if ((this.getModifyTileX() > 0 || this.getModifyTileY() > 0) && this.getNewTileType() != 0) {
                if (this.getNewTileType() == Tiles.Tile.TILE_CAVE_WALL.id || this.getNewTileType() == Tiles.Tile.TILE_CAVE_WALL_ORE_IRON.id) {
                    ...
                    if (this.getNewTileType() == Tiles.Tile.TILE_CAVE_WALL_ORE_IRON.id && Tiles.decodeType(ntile = Server.caveMesh.getTile(this.getModifyTileX(), this.getModifyTileY())) == Tiles.Tile.TILE_CAVE_WALL.id) {
                        Server.caveMesh.setTile(this.getModifyTileX(), this.getModifyTileY(), Tiles.encode(Tiles.decodeHeight(ntile), Tiles.Tile.TILE_CAVE_WALL_ORE_IRON.id, Tiles.decodeData(ntile)));
                    ...
                    if (Tiles.decodeType(otile) != Tiles.Tile.TILE_HOLE.id && (t = Zones.getOrCreateTile(this.getModifyTileX(), this.getModifyTileY(), true)) != null) {
                        if (t.getStructure() == null) {
                            Server.setSurfaceTile(this.getModifyTileX(), this.getModifyTileY(), Tiles.decodeHeight(otile), (byte)this.getNewTileType(), this.getNewTileData());
            ...
            if (this.spawnTileX > 0 || this.spawnTileY > 0) {
                try {
                    if (this.getCreatureSpawn() > 0) {
                        ...
                        byte sex = ct.getSex();
                        if (!ct.keepSex) {
                            sex = Server.rand.nextBoolean() ? (byte)1 : 0;
                        }
                        byte ttype = this.getCreatureType();
                        byte age = (byte)this.getCreatureAge();
                        try {
                            Creature reward = Creature.doNew(ct.getTemplateId(), true, this.spawnTileX * 4 + 2, this.spawnTileY * 4 + 2, performer.getStatus().getRotation() - 180.0f, performer.getLayer(), ct.getName(), sex, (byte)0, ttype, false, age);
                        }
                        catch (Exception cex) {
                            logger.log(Level.WARNING, performer.getName() + " " + cex.getMessage(), cex);
                        }
                    }
                }
            ...
            if (this.getMissionToActivate() > 0 && (mis = Missions.getMissionWithId(this.getMissionToActivate())) != null) {
                mis.setInactive(false);
            }
            if (this.getMissionToDeActivate() > 0 && (mis = Missions.getMissionWithId(this.getMissionToDeActivate())) != null) {
                mis.setInactive(true);
            }
            if (this.getTriggerToActivate() > 0 && (trg = MissionTriggers.getTriggerWithId(this.getTriggerToActivate())) != null) {
                trg.setInactive(false);
            }
            if (this.getTriggerToDeActivate() > 0 && (trg = MissionTriggers.getTriggerWithId(this.getTriggerToDeActivate())) != null) {
                trg.setInactive(true);
            }
            if (this.getEffectToActivate() > 0 && (eff = TriggerEffects.getTriggerEffect(this.getEffectToActivate())) != null) {
                eff.setInactive(false);
            }
            if (this.getEffectToDeActivate() > 0 && (eff = TriggerEffects.getTriggerEffect(this.getEffectToDeActivate())) != null) {
    ...
    void destroyTarget(long targetId) {
        ...
        if (WurmId.getType(targetId) == 5 && (wall = Wall.getWall(targetId)) != null) {
            wall.setAsPlan();
        }
        ...
        } else if (WurmId.getType(targetId) == 7 && (fence = Fence.getFence(targetId)) != null) {
            fence.destroy();
        }
    }
    ...
    public void update() {
        ...
        try {
            ...
            ps = dbcon.prepareStatement(UPDATE_EFFECT);
    ...
    public void create() {
        ...
        try {
            dbcon = DbConnector.getPlayerDbCon();
            ps = dbcon.prepareStatement(CREATE_EFFECT, 1);
            ps.setString(1, this.name);
            ps.setString(2, this.description);
            ps.setInt(3, this.rewardItem);
            ps.setInt(4, this.rewardNumbers);
