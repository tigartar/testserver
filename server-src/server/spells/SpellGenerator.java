...
public final class SpellGenerator implements SpellTypes {
   ...
   public static void createSpells() {
      ...
      Spell[] SPELLS_COMMON = new Spell[]{
         Spells.SPELL_BLESS,
         Spells.SPELL_BREAK_ALTAR,
         Spells.SPELL_DISPEL,
         Spells.SPELL_LOCATE_ARTIFACT,
   ...
   private static int rollTamingGroup(Deity deity, Random rand) {
      SpellGenerator.SpellGroup taming = new SpellGenerator.SpellGroup(deity);
   ...
   private static int rollAreaHealingGroup(Deity deity, Random rand) {
      SpellGenerator.SpellGroup areaHealing = new SpellGenerator.SpellGroup(deity);
      if (!deity.isHateGod()) {
         if (deity.isWarrior()) {
   ...
   private static int rollAreaCombatGroup(Deity deity, Random rand) {
      SpellGenerator.SpellGroup areaCombat = new SpellGenerator.SpellGroup(deity);
   ...
   private static int rollWeaponEnchantGroup(Deity deity, Random rand) {
      SpellGenerator.SpellGroup weaponEnchant = new SpellGenerator.SpellGroup(deity);
   ...
   private static int rollWeaponAugmentGroup(Deity deity, Random rand) {
      SpellGenerator.SpellGroup weaponAugment = new SpellGenerator.SpellGroup(deity);
      if (deity.isHealer()) {
   ...
   private static int rollIndustrialEnchantGroup(Deity deity, Random rand) {
      SpellGenerator.SpellGroup industrialEnchant = new SpellGenerator.SpellGroup(deity);
      if (deity.isLearner()) {
   ...
   private static int rollCreatureEnchantGroup(Deity deity, Random rand) {
      SpellGenerator.SpellGroup creatureEnchant = new SpellGenerator.SpellGroup(deity);
      if (deity.isForestGod()) {
   ...
   private static int rollJewelryOffenseGroup(Deity deity, Random rand) {
      SpellGenerator.SpellGroup jewelryOffense = new SpellGenerator.SpellGroup(deity);
      if (deity.isForestGod()) {
   ...
   private static int rollJewelryDefenseGroup(Deity deity, Random rand) {
      SpellGenerator.SpellGroup jewelryDefense = new SpellGenerator.SpellGroup(deity);
   ...
   private static int rollCombatDamageGroup(Deity deity, Random rand) {
      SpellGenerator.SpellGroup combatDamage = new SpellGenerator.SpellGroup(deity);
      if (deity.isMountainGod()) {
   ...
   private static int rollMajorDamageGroup(Deity deity, Random rand) {
      SpellGenerator.SpellGroup majorDamage = new SpellGenerator.SpellGroup(deity);
      if (deity.isMountainGod()) {
   ...
   private static int rollTileChangeGroup(Deity deity, Random rand) {
      SpellGenerator.SpellGroup tileChange = new SpellGenerator.SpellGroup(deity);
      if (deity.isHateGod()) {
   ...
   private static int rollRiteGroup(Deity deity, Random rand) {
      SpellGenerator.SpellGroup rite = new SpellGenerator.SpellGroup(deity);
      if (deity.isHateGod()) {
   ...
   private static int rollDisintegrate(Deity deity, Random rand) {
      SpellGenerator.SpellGroup disintegrate = new SpellGenerator.SpellGroup(deity);
      if (deity.isWarrior()) {
   ...
   private static int rollStrongwall(Deity deity, Random rand) {
      SpellGenerator.SpellGroup strongwall = new SpellGenerator.SpellGroup(deity);
   ...
   private static void addDeitySpell(String group, Deity deity, int spellId) {
      if (spellId > 0) {
         ...
         logger.info(String.format("%s obtains spell %s from %s group.", deity.getName(), Spells.getSpell(spellId).getName(), group));
      } else {
         logger.info(String.format("%s obtains no spell from %s group.", deity.getName(), group));
      }
   }
               ...
               if (!spell.isKarmaSpell()) {
                  ...
                  if (spell == Spells.SPELL_FIRE_PILLAR) {
                     if (!deity.hasSpell(Spells.SPELL_ICE_PILLAR) && !deity.hasSpell(Spells.SPELL_PAIN_RAIN)) {
                        if (spell.isSpellBlocked(deity.getNumber(), Spells.SPELL_ICE_PILLAR.getNumber())) {
                           blocked = true;
   ...
   private static class SpellGroup {
      ...
      public int getRandomSpell(Random rand) {
         if (this.spells.size() > 0) {
            ...
            SpellGenerator.logger
               .warning(String.format("Attempting to obtain a random spell for deity %s failed because no spells available.", this.deity.getName()));
            return -10;
         }
      }
   }
