                     ...
                     if (!ok && System.currentTimeMillis() - v.plan.getLastDrained() > 7200000L) {
                        performer.getCommunicator()
                           .sendNormalServerMessage(
                              "The settlement has not been drained during the last two hours and the wall still stands this time.", (byte)3
                           );
                        return false;
                     }
                  }
      ...
      try {
         ...
         if (Tiles.isReinforcedCave(type)) {
            ...
            if (performer.getDeity().isWarrior()) {
               num = Math.max(1, 100 - (int)power);
            } else {
               num = Math.max(1, 200 - 2 * (int)power);
            }
            ...
            if (Server.rand.nextInt(Servers.localServer.testServer ? 1 : num) == 0) {
               ...
               if (t != null) {
                  logger.info(
                     performer.getName() + " (" + performer.getWurmId() + ") disintegrated a " + t.getName() + " at [" + tilex + "," + tiley + ",false]"
                  );
               }
            ...
            performer.getCommunicator()
               .sendNormalServerMessage("You fail to find a weak spot to direct the power to. The wall still stands this time.", (byte)3);
            dis = false;
         }

         if (dis && TileRockBehaviour.createInsideTunnel(tilex, tiley, tile, performer, 145, dir, true, act)) {
            performer.getCommunicator()
               .sendNormalServerMessage("You disintegrate the " + Tiles.getTile(Tiles.decodeType(tile)).tiledesc.toLowerCase() + ".", (byte)2);
         }
