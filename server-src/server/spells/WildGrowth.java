      ...
      for(int x = sx; x <= ex; ++x) {
         for(int y = sy; y <= ey; ++y) {
            ...
            if (performer.isOnSurface()) {
               ...
               if (t != null) {
                  if (t.getVillage() != null && !t.getVillage().isActionAllowed((short)468, performer, false, 0, 0)) {
                     if (!sentMessage) {
                        performer.getCommunicator()
                           .sendNormalServerMessage("You are not allowed to affect the hedges in " + t.getVillage().getName() + ".", (byte)3);
                        sentMessage = true;
                     }
                  } else {
                     for(Fence fence : t.getFences()) {
                        if (fence.isHedge()
                           && !fence.isHighHedge()
                           && fence.getType() != StructureConstantsEnum.HEDGE_FLOWER1_LOW
                           ...
                           try {
                              ...
                              logger.log(Level.WARNING, x + "," + y + " " + var25.getMessage(), (Throwable)var25);
                           }
                        }
                     }
                  }
            ...
            if (type == Tiles.Tile.TILE_FIELD.id || type == Tiles.Tile.TILE_FIELD2.id) {
               ...
               int age = Tiles.decodeData(tile) >> 4 & 15;
               int halfdata = Tiles.decodeData(tile) & 15;
