...
public class Tornado extends DamageSpell implements AttitudeConstants {
   ...
   void doEffect(Skill castSkill, double power, Creature performer, int tilex, int tiley, int layer, int heightOffset) {
      ...
      Shape circle = new Float((float)sx, (float)sy, (float)(ex - sx), (float)(ey - sy));

      for(int x = sx; x < ex; ++x) {
         for(int y = sy; y < ey; ++y) {
            if (circle.contains((double)x, (double)y)) {
               ...
               if (type == Tiles.Tile.TILE_FIELD.id || type == Tiles.Tile.TILE_FIELD2.id) {
                  ...
                  if (treeAge == 15 || Server.rand.nextInt(16 - treeAge) != 0) {
                     ...
                     double lNewRotation = Math.atan2((double)((y << 2) + 2 - ((y << 2) + 2)), (double)((x << 2) + 2 - ((x << 2) + 2)));
                     float rot = (float)(lNewRotation * (180.0 / Math.PI));

                     try {
                        Item newItem = ItemFactory.createItem(
                           templateId,
                           (float)power / 5.0F,
                           (float)(x * 4 + Server.rand.nextInt(4)),
                           (float)(y * 4 + Server.rand.nextInt(4)),
                           rot,
                           performer.isOnSurface(),
                           treeType.getMaterial(),
                           (byte)0,
                           -10L,
                           null,
                           treeAge
                     ...
                     if (!crets[c].isGhost() && !crets[c].isDead() && crets[c].getAttitude(performer) == 2) {
                        try {
                           ...
                           CombatEngine.addWound(
                              performer, crets[c], (byte)0, pos, damage, 1.0F, "assault", performer.getBattle(), 0.0F, 0.0F, false, false, false, true
                           );
